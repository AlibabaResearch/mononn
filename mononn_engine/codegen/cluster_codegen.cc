...
namespace codegen {
    ...
    std::string ClusterCodegen::generate(std::shared_ptr<const CUDAContext> cuda_context, std::shared_ptr<const ClusterOp> cluster_op) {

        return cluster_op->generate_cluster_invocation();
    }

    void ClusterCodegen::setup_codegen(std::shared_ptr<const CUDAContext> cuda_context,
                                       std::shared_ptr<ClusterOp> cluster_op) {
        cluster_op->setup_codegen();
    }

    std::string ClusterCodegen::generate_function_declaration(std::shared_ptr<const CUDAContext> cuda_context,
        ...
        int output_node_count = (int)cluster_op->get_graph()->get_output_nodes().size();
        for (int idx = 0; idx < output_node_count; ++idx) {
            std::string node_name = cluster_op->get_graph()->get_output_node(idx);
            std::shared_ptr<const Op> node = cluster_op->get_graph()->get_node(node_name);

            if (node->get_output_specs_count() > 1) {
                if (node->get_type() != OpType::reduce) {
                    LOG(FATAL) << "Reduce node expected, however " << node->get_name() << " has type " << node->get_type().to_string();
                }

                for (int tuple_index = 0; tuple_index < node->get_output_specs_count(); ++tuple_index) {
                    ss << "void *" << BufferManager::get_buffer_name(node_name) + "_tuple_index_" + std::to_string(tuple_index) + "_output";
    ...
    std::string ClusterCodegen::generate_function_definition(std::shared_ptr<const CUDAContext> cuda_context,
                                                             std::shared_ptr<const ClusterOp> cluster_op) {
        std::stringstream ss;
