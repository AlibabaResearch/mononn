...
class CUDAEmitter {
 public:
  CUDAEmitter(const Graph* _graph,
              const CodegenStateSpace* _codegens_state_space)
  ...
  // void emit(const Op *node, const std::map<std::string, std::string>
  ...
  void emit_elementwise_unary(const Op* node,
                              const CodegenState& codegen_state);
  void emit_elementwise_binary(const Op* node,
                               const CodegenState& codegen_state);
...
class CodegenStateSampler {
 public:
  CodegenStateSampler(
      std::function<CodegenState(const CodegenState&)> _sample_func)
      : sample_func(_sample_func) {}
...
class CodegenStateSpace {
 public:
  ...
  std::vector<CodegenState> generate_codegen_state(
