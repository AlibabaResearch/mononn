...
namespace codegen {
    ...
    std::string CUDAEmitter::emit_code_use_new_code_emitter(
        ...
        std::set<std::pair<std::string, std::string>> visit; // pair<node_name, traced_by>;
        ...
        if (graph->get_instruction_parallel_factor() != 1) {
            ...
            symbolic_index_initializer = {
                {IndexSymbols::linear_index, mononn_engine::helpers::string_format("((%s) + (%s * {ilp_index_id}))", linear_loop_key.c_str(), linear_loop_stride.c_str())},
        ...
        std::function<void(const Op *, const std::string &, const Op *)> codegen_dfs 
                = [&](const Op *node, const std::string &source_index, const Op *source_node) -> void {
            ...
            for (auto const &symbolic_index : node->get_symbolic_index()) {
                ...
                if (symbolic_index.index_before_trace == source_index || (source_node && source_node->get_type() == OpType::gather)) { // This is a hack
                    visit.insert(visit_mark);
                    
                    // This reverse ordered operand traversal is a trick.
                    // Need a better way to efficiently manage DFS context while respect control dependency.
                    // Just reverse the order of operand here is only gather operator will introduce control dependency.
                    for (int operand_id = node->get_operand_count() - 1; operand_id >= 0; --operand_id) {
                        codegen_dfs(node->get_operand(operand_id).get(), symbolic_index.index_after_trace, node);
                    }

                    // for (auto const &control_edge : graph->get_node_input_control_edges(node->get_name())) {
                    //     codegen_dfs(control_edge->get_src().get(), symbolic_index.index_after_trace, node);
                    // }
                    
                    codege_state_mgr.recall_op(node);

                    cuda_emitter.emit(node, symbolic_index.instantiate(symbolic_index_initializer));
                }
            }
            ...
            if (node->get_type() == OpType::reduce) {
                ...
                for (auto const &symbolic_index : node->get_symbolic_index()) {
                    cuda_emitter.emit_output(node, symbolic_index.instantiate(symbolic_index_initializer));
                }
    ...
    void CUDAEmitter::emit(const Op *node, const ConcreteIndexStamp &concrete_index) {
    ...
    // void CUDAEmitter::emit(const Op *node, const std::map<std::string, std::string> &symbolic_index_initializer) {
    ...
    void CUDAEmitter::emit_codegen_state(const Op *node, const CodegenState &codegen_state) {
        if (node->get_type() == OpType::broadcast || node->get_type() == OpType::gather) {
        ...
        } else if (node->get_type() == OpType::compare || node->get_type() == OpType::multiply || node->get_type() == OpType::add) {
            ...
            LOG(FATAL) << node->get_name() << " have unsupported type " << node->get_type().to_string();
        }

        if (graph->is_output_node(node->get_name()) && node->get_type() != OpType::reduce) {
            this->emit_output(node, codegen_state);
        }
    }

    void CUDAEmitter::emit_constant(const Op *node, const CodegenState &codegen_state) {
        if (node->as<Constant>()->is_scalar()) {
            this->code_stream << mononn_engine::helpers::string_format("const %s %s = %s;\n"
    ...
    void CUDAEmitter::emit_elementwise_unary(const Op *node, const CodegenState &codegen_state) {
        // Convert 
        std::string functor_name = Functor::get_functor_name_for_op_type(node->get_type());
        Functor functor(functor_name, node->get_output_spec(0).get_dtype());
        FunctionInvocation invocation(functor.get_name());
        invocation.add_arg(node->get_operand(0)->get_name() + codegen_state.node_name_suffix);

        // this->code_stream << invocation.to_string();
        this->code_stream << mononn_engine::helpers::string_format("%s %s = %s;\n"
            , node->get_output_spec(0).get_dtype().to_string().c_str(),
            (node->get_name() + codegen_state.node_name_suffix).c_str(),
            invocation.to_string().c_str());
    }

    void CUDAEmitter::emit_elementwise_binary(const Op *node, const CodegenState &codegen_state) {
        // Compare Multiply
        std::string functor_name;

        if (node->get_type() == OpType::compare) {
            functor_name = Functor::get_functor_name_for_op_type(OpType::compare, node->as<Compare>()->get_comparator());
        } else {
            functor_name = Functor::get_functor_name_for_op_type(node->get_type());
        }

        Functor functor(functor_name, node->get_operand(0)->get_output_spec(0).get_dtype());
        FunctionInvocation invocation(functor.get_name());

        invocation.add_arg(node->get_operand(0)->get_name() + codegen_state.node_name_suffix);
        invocation.add_arg(node->get_operand(1)->get_name() + codegen_state.node_name_suffix);
    ...
    void CUDAEmitter::emit_parameter(const Op *node, const CodegenState &codegen_state) {
        std::string var_name = node->get_name() + codegen_state.node_name_suffix;
        std::string buffer_name = BufferManager::get_buffer_name(node->get_name()) + "_input";
        auto dtype = node->get_output_spec(0).get_dtype();
        this->code_stream << Memory::read(
            Memory::REGULAR, 
            dtype, var_name, 
            buffer_name, 
            codegen_state.index.index_after_trace, 
            true,
            codegen_state.index.pred_after_trace);
    }

    void CUDAEmitter::emit_reduce(const Op *node, const CodegenState &codegen_state) {
        this->code_stream << node->get_implementation()->generate();
    }

    void CUDAEmitter::emit_reduce_window(const Op *node, const CodegenState &codegen_state) {
        const ReduceWindow *reduce_window = node->as<ReduceWindow>();
        Dtype dtype = node->get_output_spec(0).get_dtype();

        std::string reduce_window_node_name = (reduce_window->get_name() + codegen_state.node_name_suffix);
        ...
        std::vector<std::vector<int>> window_positions = reduce_window->get_window_positions();

        for (auto const &window_position : window_positions) {
            std::string operand_name = node->get_operand(0)->get_name() + codegen_state.node_name_suffix + "_window_pos_"
                + mononn_engine::helpers::join<int>("_", window_position, [](const int& v) -> std::string { return std::to_string(v); });
            this->code_stream << mononn_engine::helpers::string_format("%s = %s(%s, %s);\n",
                reduce_window_node_name.c_str(), reduce_window->get_reduction_functor_generator()->instance_name().c_str(),
                reduce_window_node_name.c_str(), operand_name.c_str());
        }
    }

    void CUDAEmitter::emit_select(const Op *node, const CodegenState &codegen_state) {
        std::string functor_name = Functor::get_functor_name_for_op_type(node->get_type());
        Functor functor(functor_name, node->get_output_spec(0).get_dtype());
        FunctionInvocation invocation(functor.get_name());
        invocation.add_arg(node->get_operand(0)->get_name() + codegen_state.node_name_suffix);
        invocation.add_arg(node->get_operand(1)->get_name() + codegen_state.node_name_suffix);
        invocation.add_arg(node->get_operand(2)->get_name() + codegen_state.node_name_suffix);
    ...
    void CUDAEmitter::emit_non_op(const Op *node, const CodegenState &codegen_state) {
        this->code_stream << mononn_engine::helpers::string_format("%s %s = %s;\n"
            , node->get_output_spec(0).get_dtype().to_string().c_str(),
            (node->get_name() + codegen_state.node_name_suffix).c_str(),
            (node->get_operand(0)->get_name() + codegen_state.node_name_suffix).c_str());
    }

    void CUDAEmitter::emit_output(const Op *node, const CodegenState &codegen_state) {
        std::string var_name = node->get_name() + codegen_state.node_name_suffix;
        std::string buffer_name = BufferManager::get_buffer_name(node->get_name()) + "_output";
        auto dtype = node->get_output_spec(0).get_dtype();
        this->code_stream << Memory::write(
            Memory::REGULAR, 
            dtype, 
            var_name, 
            buffer_name, 
    ...
    void CUDAEmitter::emit_output(const Op *node, const ConcreteIndexStamp &concrete_index) {
        // std::vector<CodegenState> symbolic_codegen_state_list;
    ...
    void CUDAEmitter::emit_loop_end(const Op *node) {
        this->code_stream << "}\n";
    }

    void CUDAEmitter::emit_post_reduce_if(const Op *node) {
        this->code_stream << node->get_implementation()->as<ReduceImpl>()->get_post_reduce_if_statement() << "\n";
    }

    void CUDAEmitter::emit_post_reduce_if_end(const Op *node) {
        this->code_stream << node->get_implementation()->as<ReduceImpl>()->get_post_reduce_if_end() << "\n";
    }

    void CUDAEmitter::emit_reduce_broadcast(const Op *node) {
        this->code_stream << node->get_implementation()->as<ReduceImpl>()->generate_reduce();
    }
    ...
    CodegenState CodegenStateSampler::sample(const CodegenState &codegen_state) const {
    ...
    void CodegenStateSpace::pop() {
        this->space_array.pop_back();
    }

    std::vector<CodegenState> CodegenStateSpace::generate_codegen_state(const CodegenState &init_state) const {
        if (this->space_array.empty()) {
            return { init_state };
        }

        std::vector<std::vector<int>> space_selection_index(this->space_array[0].size());
        // space_selection_index.push_back({});

        for (int idx_space_sampler = 0; idx_space_sampler < this->space_array[0].size(); ++idx_space_sampler) {
            space_selection_index[idx_space_sampler].push_back(idx_space_sampler);
        }

        for (int idx_space_array = 1; idx_space_array < this->space_array.size(); ++idx_space_array) {
            std::vector<int> space_sampiler_index(this->space_array[idx_space_array].size());

            std::iota(space_sampiler_index.begin(), space_sampiler_index.end(), 0);

            space_selection_index = mononn_engine::helpers::cartesian_join<std::vector<int>, int, std::vector<int>>
                (space_selection_index, space_sampiler_index, [](const std::vector<int> &vec, const int &value) -> std::vector<int> {
    ...
    std::vector<CodegenState> CodegenStateSpace::generate_codegen_state(const std::vector<CodegenState> &init_state_list) const {
        std::vector<CodegenState> result_state_list;

        for (auto const &init_state : init_state_list) {
            std::vector<CodegenState> partial_state_list = this->generate_codegen_state(init_state);
            result_state_list.insert(result_state_list.end(), partial_state_list.begin(), partial_state_list.end());
        }
    ...
    void CodegenStateSpaceMgr::emit_instruction_level_parallelism(int ilp_factor) {
        ...
        for (int ilp_id = 0; ilp_id < ilp_factor; ++ilp_id) {
            state_sampler_array.emplace_back([ilp_id](const CodegenState &codegen_state) -> CodegenState {
                CodegenState next_codegen_state;
                
                next_codegen_state.node_name_suffix = codegen_state.node_name_suffix + "__i" + std::to_string(ilp_id);
                next_codegen_state.index = codegen_state.index.instantiate({{"ilp_index_id", std::to_string(ilp_id)}});
    ...
    void CodegenStateSpaceMgr::emit_reduce_window(const Op *node) {
        ...
        for (auto const &window_position : window_positions) {
            state_sampler_array.emplace_back([window_position](const CodegenState &codegen_state) -> CodegenState {
                CodegenState next_codegen_state;
                ...
                next_codegen_state.node_name_suffix = codegen_state.node_name_suffix +  "_window_pos_"
                    + mononn_engine::helpers::join<int>("_", window_position, [](const int& v) -> std::string { return std::to_string(v); });
