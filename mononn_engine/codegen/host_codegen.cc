...
namespace codegen {
    ...
    std::string HostCodegen::generate(std::shared_ptr<CUDAContext> cuda_context, Graph *graph, const std::string &kernel_name) {
        std::stringstream ss;
        ...
        if (std::find(Config::get()->host_codegen_disabled_pass.begin(),
                      Config::get()->host_codegen_disabled_pass.end(), "generate_memory_initialization") ==
        ...
        if (std::count(Config::get()->host_codegen_disabled_pass.begin(),
                       Config::get()->host_codegen_disabled_pass.end(), "generate_parameter_initialization") == 0) {
            ss << HostCodegen::generate_parameter_declaration(cuda_context, graph);
        }

        ss << HostCodegen::generate_kernel_invocation(cuda_context, graph, kernel_name);
        ss << HostCodegen::generate_stream_synchronize(cuda_context->cuda_runtime_context.stream);
//        ss << HostCodegen::generate_print_output(cuda_context, graph);
    ...
    std::string HostCodegen::generate_memory_initialization(std::shared_ptr<CUDAContext> cuda_context,
                                                            Graph *graph) {
        std::stringstream ss;
        ...
        for (auto const &node_name : buffer_list) {
            ...
            if (node->get_type() == OpType::constant) {
                ...
                ss << mononn_engine::helpers::string_format("auto %s = cnpy::npy_load(\"%s\");",
                               node_name.c_str(), ("./" + node_name +".npy").c_str()) << "\n";
                ...
                cuda_memcpy.add_arg(mononn_engine::helpers::string_format("%s.data<%s>()", node_name.c_str(), dtype.to_cutlass_type().to_string().c_str()));
                cuda_memcpy.add_arg(std::to_string(node->get_output_buffer_size_in_bytes()));
                ...
                cuda_memset.add_arg(std::to_string(node->get_output_buffer_size_in_bytes()));
    ...
    std::string HostCodegen::generate_parameter_declaration(std::shared_ptr<CUDAContext> cuda_context,
                                                            Graph *graph) {
        ...
        for (auto const &node_name : buffer_list) {
            ...
            if (node->get_type() == OpType::parameter) {
                ...
                ss << mononn_engine::helpers::string_format("auto %s = cnpy::npy_load(\"%s\");",
                                                       node_name.c_str(), input_data_file.c_str()) << "\n";
            }
        }
    ...
    std::string HostCodegen::generate_parameter_initialization(std::shared_ptr<CUDAContext> cuda_context,
                                                              Graph *graph) {
        std::stringstream ss;
        ...
        for (auto const &node_name : buffer_list) {
            ...
            if (node->get_type() == OpType::parameter) {
                ...
                cuda_memcpy.add_arg(BufferManager::get_buffer_name(node_name));
                cuda_memcpy.add_arg(mononn_engine::helpers::string_format("%s.data<%s>()", node_name.c_str(),
    ...
    std::string HostCodegen::generate_stream(std::shared_ptr<CUDAContext> cuda_context,
                                             Graph *graph) {
        std::stringstream ss;
        ss << mononn_engine::helpers::string_format("cudaStream_t %s;", cuda_context->cuda_runtime_context.stream.c_str()) << "\n";
        ss << mononn_engine::helpers::string_format("checkCudaErrors(cudaStreamCreateWithFlags(&%s, cudaStreamNonBlocking));",
                                               cuda_context->cuda_runtime_context.stream.c_str()) << "\n";

        return ss.str();
    }

    std::string HostCodegen::generate_memory_allocation(std::shared_ptr<CUDAContext> cuda_context,
                                                        Graph *graph) {
        ...
        cuda_malloc.add_arg(std::to_string(onefuser_memory_buffer.get_total_size_in_bytes()));
        ...
        for (int idx = 0; idx < (int)buffer_node_list.size(); ++idx) {
            ...
            ss << mononn_engine::helpers::string_format("void *%s = %s;",
               buffer_name.c_str(), onefuser_memory_buffer.get_pointer_to_buffer(idx).c_str()) << "\n";
        }
    ...
    std::string
    HostCodegen::generate_kernel_invocation(std::shared_ptr<CUDAContext> cuda_context, Graph *graph, const std::string &kernel_name) {

        CUDAInvocation cuda_invocation(
            kernel_name,
        ...
        if (cuda_context->cuda_runtime_context.smem_size >= (48 << 10)) {
            ...
            cuda_func_set_attr.add_arg(std::to_string(cuda_context->cuda_runtime_context.smem_size));
        ...
        ss << mononn_engine::helpers::string_format("checkCudaErrors(cudaEventRecord(start, %s));", cuda_context->cuda_runtime_context.stream.c_str()) << "\n";
        ...
        ss << "}" << "\n";
        ss << mononn_engine::helpers::string_format("checkCudaErrors(cudaEventRecord(stop, %s));", cuda_context->cuda_runtime_context.stream.c_str()) << "\n";
...
printf("Time in us: %.3f\n", milliseconds * 1000 / 100);
    ...
    std::string HostCodegen::generate_print_output(std::shared_ptr<CUDAContext> cuda_context, Graph *graph) {
        std::stringstream ss;

        int num_output_nodes = graph->get_output_nodes().size();

        std::function<void(std::string, std::string, std::string, TensorSpec)> print_single_output =
            [&](std::string name, std::string dev_ptr, std::string host_ptr, TensorSpec tensor_spec) -> void {

            auto type = tensor_spec.get_dtype();

            ss << mononn_engine::helpers::string_format("std::array<%s, %s> %s;",
            ...
            ss << mononn_engine::helpers::string_format("std::cout << \"%s\" << \" \" << \"%s\" << \"%s\";\n", name.c_str(), type.to_string().c_str(), tensor_spec.to_string().c_str());
            ss << mononn_engine::helpers::string_format("std::cout << \"==========\" << std::endl;") << "\n";

            int row_count = tensor_spec.element_count() / tensor_spec.get_shape(-1);
            int col_count = tensor_spec.get_shape(-1);
            std::string offset = mononn_engine::helpers::string_format("c + r * %d", col_count);

            ss << mononn_engine::helpers::string_format("for (int r = 0; r < %d; ++r) {", row_count) << "\n";
            ss << "std::cout << \"{ \";" << "\n";
            ss << mononn_engine::helpers::string_format("for (int c = 0; c < %d; ++c) {", col_count) << "\n";
            ss << "if (c != 0) { std::cout << \", \"; }" << "\n";
            ss << mononn_engine::helpers::string_format("std::cout << %s[%s];", host_ptr.c_str(), offset.c_str()) << "\n";
            ss << "}" << "\n";
        ...
        for (auto const &output_node_name : graph->get_output_nodes()) {
            ...
            if (output_node->get_output_specs().size() > 1) { // cluster multiple output
                MultiBuffer multi_buffer(BufferManager::get_buffer_name(output_node_name));
                ...
                for (int idx = 0; idx < output_count; ++idx) {
                    ...
                    std::string host_buffer = output_node_name + "_output" + std::to_string(idx) + "_host";

                    print_single_output(name, dev_ptr, host_buffer, output_node->get_output_spec(idx));
                }
                ...
                print_single_output(name, dev_ptr, host_buffer, output_node->get_output_spec(0));
            }
        }
    ...
    std::string HostCodegen::generate_stream_synchronize(std::string stream) {
        return mononn_engine::helpers::string_format("checkCudaErrors(cudaStreamSynchronize(%s));\n", stream.c_str());
    }
}
}