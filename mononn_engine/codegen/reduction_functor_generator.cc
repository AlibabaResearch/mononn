...
ReductionFunctorGeneratorRegistry* ReductionFunctorGenerator::Registry() {
  if (!ReductionFunctorGenerator::registry) {
    ReductionFunctorGenerator::registry =
        std::make_unique<ReductionFunctorGeneratorRegistry>();
  }
...
std::string hlo_shape_to_type_string(const xla::Shape& shape) {
  if (shape.IsTuple()) {
    ...
    for (auto const& sub_shape : shape.tuple_shapes()) {
      ...
      type_list.push_back(
          hlo_primitive_type_to_string(sub_shape.element_type()));
    }
...
std::string get_inst_definition(const xla::HloInstruction* inst,
                                const std::string& value) {
  return hlo_shape_to_type_string(inst->shape()) + " " +
         mononn_engine::helpers::get_canonicalized_node_name(inst->name()) +
         " = " + value;
}

std::string emit_parameter(const xla::HloInstruction* inst) { return ""; }

std::string emit_fusion(const xla::HloInstruction* inst) {
  std::string invocaiton = mononn_engine::helpers::get_canonicalized_node_name(
      inst->fused_instructions_computation()->name());
...
std::string emit_get_tuple_element(const xla::HloInstruction* inst) {
  const xla::HloGetTupleElementInstruction* gte_inst =
      static_cast<const xla::HloGetTupleElementInstruction*>(inst);

  std::string invocaiton = "cuda::std::get<" +
                           std::to_string(gte_inst->tuple_index()) + ">(" +
...
std::string emit_compare(const xla::HloInstruction* inst) {
  ...
  switch (inst->comparison_direction()) {
    ...
    case xla::ComparisonDirection::kNe:
      ...
      break;
    default:
      LOG(FATAL) << "Not supported comparator: "
                 << (int)inst->comparison_direction();
  }

  std::string param_a = mononn_engine::helpers::get_canonicalized_node_name(
      inst->operand(0)->name());
  std::string param_b = mononn_engine::helpers::get_canonicalized_node_name(
      inst->operand(1)->name());
...
std::string emit_elewise_binary(const xla::HloInstruction* inst) {
  std::string operand_a = mononn_engine::helpers::get_canonicalized_node_name(
      inst->operand(0)->name());
  std::string operand_b = mononn_engine::helpers::get_canonicalized_node_name(
      inst->operand(1)->name());
  ...
  switch (inst->opcode()) {
    ...
    case xla::HloOpcode::kMaximum:
      value = mononn_engine::helpers::string_format(
          "%s > %s ? %s : %s", operand_a.c_str(), operand_b.c_str(),
          operand_a.c_str(), operand_b.c_str());
      break;
    ...
    default:
      LOG(FATAL) << "Unsupported opcode "
                 << xla::HloOpcodeString(inst->opcode()) << " of instruciton "
                 << inst->name() << " not supported.";
  }
...
std::string emit_select(const xla::HloInstruction* inst) {
  std::string pred = mononn_engine::helpers::get_canonicalized_node_name(
      inst->operand(0)->name());
  std::string on_true = mononn_engine::helpers::get_canonicalized_node_name(
      inst->operand(1)->name());
  std::string on_false = mononn_engine::helpers::get_canonicalized_node_name(
      inst->operand(2)->name());
...
std::string emit_tuple(const xla::HloInstruction* inst) {
  ...
  return get_inst_definition(
      inst, "cuda::std::make_tuple(" +
                mononn_engine::helpers::join(", ", tuple_elements) + ")");
}

std::string generate_fused_computation(
    const xla::HloComputation* fused_computation) {
  std::stringstream computation_definition;

  computation_definition << "__device__ __forceinline__\n";
  computation_definition << hlo_shape_to_type_string(
                                fused_computation->root_instruction()->shape())
                         << mononn_engine::helpers::get_canonicalized_node_name(
                                fused_computation->name())
                         << "(\n";
  ...
  computation_definition << "return "
                         << mononn_engine::helpers::get_canonicalized_node_name(
                                fused_computation->root_instruction()->name())
                         << ";\n";
  computation_definition << "}\n";
...
void ReductionFunctorGeneratorRegistry::add_generator(
  ...
  std::string functor_type_name =
      mononn_engine::helpers::get_canonicalized_node_name(computation->name());
  ...
  for (auto const& inst : computation->MakeInstructionPostOrder()) {
    if (inst->opcode() == xla::HloOpcode::kFusion) {
      if (inst->fusion_kind() != xla::HloInstruction::FusionKind::kLoop) {
        LOG(FATAL) << "Fusion inst: " << inst->name() << " inside computation "
                   << computation->name() << " is not loop fusion.";
      }

      functor_definition << generate_fused_computation(
  ...
  functor_definition << hlo_shape_to_type_string(
                            computation->root_instruction()->shape())
                     << " operator () ";

  std::vector<std::string> init_value_type;
  std::vector<std::string> new_value_type;
  // parameter order remapping from hlo computation parameter to functor
  // parameter.
  for (int64_t param_idx = 0; param_idx < computation->num_parameters();
       ++param_idx) {
    auto param_inst = computation->parameter_instruction(param_idx);

    if (param_idx < computation->num_parameters() / 2) {
      new_value_type.push_back(
          hlo_primitive_type_to_string(param_inst->shape().element_type()));
    } else {
      init_value_type.push_back(
          hlo_primitive_type_to_string(param_inst->shape().element_type()));
    }
  }
  ...
  functor_definition << "(const cuda::std::tuple<"
                     << mononn_engine::helpers::join(", ", init_value_type)
                     << "> &a, "
                     << "const cuda::std::tuple<"
                     << mononn_engine::helpers::join(", ", init_value_type)
                     << "> &b) const {\n";
      ...
      case xla::HloOpcode::kParameter: {
        ...
        if (param_idx < computation->num_parameters() / 2) {
          functor_definition
              << get_inst_definition(
                     inst,
                     "cuda::std::get<" + std::to_string(param_idx) + ">(a)")
              << ";\n";
        } else {
          functor_definition
              << get_inst_definition(
  ...
  if (computation->num_parameters() == 2) {  // Scalar reduction, not tuple
    ...
    functor_definition << hlo_shape_to_type_string(
                              computation->root_instruction()->shape())
                       << " operator () ";
    functor_definition << "(const " << init_value_type[0] << " &a, "
                       << "const " << init_value_type[0] << " &b) const {\n";
    functor_definition << "return (*this)(cuda::std::make_tuple(a), "
                          "cuda::std::make_tuple(b));\n";
    functor_definition << "}\n";
  }
  ...
  this->registry[computation->name()] =
      std::move(std::make_unique<ReductionFunctorGenerator>(
          reduction_node_name, computation->name(), functor_type_name,
          functor_instance_name, functor_definition.str()));
}

const std::unordered_map<ReductionFunctorGeneratorRegistry::ComputationName,
                         std::unique_ptr<ReductionFunctorGenerator>>&
ReductionFunctorGeneratorRegistry::get_generator() const {
  return this->registry;
}

const ReductionFunctorGenerator*
ReductionFunctorGeneratorRegistry::get_generator(
...
thread_local std::unique_ptr<ReductionFunctorGeneratorRegistry>
    ReductionFunctorGenerator::registry = nullptr;
}  // namespace codegen
}  // namespace mononn_engine