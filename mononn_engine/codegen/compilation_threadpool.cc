...
namespace codegen {
using MonoNNModule = mononn_engine::module::MonoNNModule;
...
CompilationThreadpool::CompilationThreadpool(int n_threads)
...
CompilationThreadpool::CompilationThreadpool()
    : CompilationThreadpool(
          EnvVar::defined("TF_MONONN_THREADPOOL_SIZE")
              ? std::stoi(EnvVar::get("TF_MONONN_THREADPOOL_SIZE"))
              : std::thread::hardware_concurrency()) {}

void CompilationThreadpool::post(
    const xla::HloModule* hlo_module, std::unique_ptr<Graph> graph,
  ...
  this->thread_pool.enqueue([compile_debug_option, &finish_queue, hlo_module,
                             graph = std::move(graph), tuning_spec_id,
                             tuning_spec = std::move(tuning_spec), kernel_name,
                             allocation_list, alias_analysis]() mutable {
    std::unique_ptr<MonoNNModule> mononn_module =
        std::make_unique<MonoNNModule>(hlo_module, std::move(graph),
                                       std::move(tuning_spec), kernel_name,
                                       allocation_list, alias_analysis);

    // Assume have already finish pre impl assignment optimizations, which is
    // time consuming and invariant across tuning specifications.
    ...
    if (compile_debug_option & CompileDebugOption::COMPILATION_SAVE_SOURCE) {
      std::vector<std::string> file_list =
          mononn_module->get_cuda_program()->get_file_list();

      for (auto const& file : file_list) {
        result.file_collection[file] =
            mononn_module->get_cuda_program()->file_ref(file).str();
      }
    }
...
void CompilationThreadpool::post(
    const xla::HloModule* hlo_module, TuningSpecId tuning_spec_id,
    std::unique_ptr<GraphSpecification> tuning_spec,
    const std::string& kernel_name,
    const std::vector<xla::BufferAllocation>* allocation_list,
    const HloAliasAnalysis* alias_analysis,
  ...
  this->thread_pool.enqueue([compile_debug_option, &finish_queue, hlo_module,
    ...
    std::unique_ptr<Graph> graph =
        IRParserFused::from_hlo_module_unique(hlo_module);
    std::unique_ptr<MonoNNModule> mononn_module =
        std::make_unique<MonoNNModule>(hlo_module, std::move(graph),
                                       std::move(tuning_spec), kernel_name,
                                       allocation_list, alias_analysis);
