...
namespace codegen {
    ...
    CUDAProgram ClusteredGraphCodegen::generate(std::shared_ptr<CUDAContext> cuda_context, std::shared_ptr<ClusteredGraph> graph) {
        ...
        std::stringstream ss;
        ss << mononn_engine::helpers::string_format("__launch_bounds__(%d, %d)", block_size, (block_count + sm_count - 1) / sm_count) << "\n";
        ss << "__global__" << "\n";
        ss << "void onefuser_kernel(" << "\n";

        bool first = true;
        for (auto const &buffer_name : BufferManager::get_buffered_nodes_in_global()) {
            if (!first) ss << ",\n";
            first = false;
            ss << mononn_engine::helpers::string_format("void * __restrict__ %s", buffer_name.c_str());
        }
    ...
    void ClusteredGraphCodegen::initialize_buffer_manager(std::shared_ptr<ClusteredGraph> graph) {
        BufferManager::reset();
        ...
        for (auto const &cluster_node_name : graph->get_output_nodes()) {
            std::shared_ptr<ClusterOp> cluster_node = graph->get_node(cluster_node_name);
            for (auto const &node_name : cluster_node->get_graph()->get_output_nodes()) {
                BufferManager::buffer_in_global(node_name);
            }
        }
    ...
    void ClusteredGraphCodegen::synchronization_analysis(std::shared_ptr<ClusteredGraph> graph) {
        graph->wave_front_order([&](std::shared_ptr<ClusterOp> op) -> void {
            std::string node_name = op->get_name();

            for (auto const &edge : graph->get_node_output_edges(node_name)) {
                std::shared_ptr<Synchronization> sync = std::make_shared<Synchronization>(Synchronization::Global);
