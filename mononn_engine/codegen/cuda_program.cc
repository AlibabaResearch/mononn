...
namespace codegen {
    ...
    void CUDAProgram::append_file(std::string const &file_name, std::string const &content) {
    ...
    const std::stringstream& CUDAProgram::file_ref(const std::string &file_name) const {
        if (this->files.find(file_name) == this->files.end()) {
    ...
    void CUDAProgram::generate(std::string const &directory, CompileOutputType type) const {
        ...
        std::ofstream ofs;
        std::string make_file = mononn_engine::helpers::Path::join(directory, "Makefile");
        ofs.open(make_file);
        ...
        for (auto const &[file_name, file_content] : this->files) {
            std::string full_file_name = mononn_engine::helpers::Path::join(directory, file_name);
        ...
        for (auto const &data_file : this->data_files) {
            std::string file_name = mononn_engine::helpers::string_split(data_file, '/').back();
            std::string dst_file_name = mononn_engine::helpers::Path::join(directory, file_name);
            if (fs::exists(dst_file_name)) {
                fs::remove(dst_file_name);
            }

            fs::copy(data_file, mononn_engine::helpers::Path::join(directory, file_name));
        }
    }
    ...
    std::string CUDAProgram::get_makefile(CompileOutputType type) const {
        ...
        std::string cutlass_include = mononn_engine::helpers::Path::join(MONONN_HOME, "cutlass_mononn/include");
        std::string cutlass_util_include = mononn_engine::helpers::Path::join(MONONN_HOME, "cutlass_mononn/tools/util/include");
        std::string onefuser_include = mononn_engine::helpers::Path::join(MONONN_HOME, "mononn_engine");
...
//         std::string cutlass_include = mononn_engine::helpers::Path::join(Config::get()->onefuser_home, "cutlass/include");
//         std::string cutlass_util_include = mononn_engine::helpers::Path::join(Config::get()->onefuser_home, "cutlass/tools/util/include");
//         std::string onefuser_include = mononn_engine::helpers::Path::join(Config::get()->onefuser_home, "tensorflow/onefuser");
...
//         std::string file_list_str = mononn_engine::helpers::join(std::string("\n"), file_list);

//         return mononn_engine::helpers::string_format(cmakefile_template, cuda_arch.c_str(), cutlass_include.c_str(), cutlass_util_include.c_str(), onefuser_include.c_str(), file_list_str.c_str());
//     }
}
}