...
ABSL_FLAG(std::string, graph_spec_file, "",
          "The input graph specification file");
ABSL_FLAG(std::string, output_dir, "", "The codegen directory");
ABSL_FLAG(std::string, format, "pb", "Input graph spec format");
ABSL_FLAG(std::vector<std::string>, host_codegen_disabled_pass, {},
          "Host codegen disabled pass name");
...
void override_codegen_allow_list(
    GraphSpecification* graph_spec,
    const std::vector<std::string>& codegen_allow_list) {
  std::vector<std::string> all_nodes;

  all_nodes.insert(all_nodes.end(), graph_spec->codegen_allow_list().begin(),
                   graph_spec->codegen_allow_list().end());
  all_nodes.insert(all_nodes.end(), graph_spec->codegen_reject_list().begin(),
                   graph_spec->codegen_reject_list().end());
  ...
  for (auto const& node_name : all_nodes) {
    if (std::find(codegen_allow_list.begin(), codegen_allow_list.end(),
                  node_name) != codegen_allow_list.end()) {
  ...
  LOG(INFO) << "Codegen allow list override: "
            << mononn_engine::helpers::join(",", codegen_allow_list);
  LOG(INFO) << "Oerrided: "
            << mononn_engine::helpers::join(",", overrided_allow_list);
...
int Main(Options const& options) {
  ...
  Config::get()->host_codegen_disabled_pass =
      options.host_codegen_disabled_pass;
  ...
  if (!fs::exists(options.graph_spec_file)) {
    LOG(FATAL) << "Graph specification file " << options.graph_spec_file
               << " do not exists";
  }
  ...
  Config::get()->feeds = std::vector<std::string>(graph_spec->feeds().begin(),
                                                  graph_spec->feeds().end());
  Config::get()->fetches = std::vector<std::string>(
      graph_spec->fetches().begin(), graph_spec->fetches().end());
  Config::get()->input_data_files =
      std::vector<std::string>(graph_spec->input_data_files().begin(),
                               graph_spec->input_data_files().end());

  std::unique_ptr<CUDAProgram> cuda_program =
      std::move(GraphSpecificationCodegen::generate(graph_spec.get()));
...
int main(int argc, char** argv) {
  ...
  options.host_codegen_disabled_pass =
      absl::GetFlag(FLAGS_host_codegen_disabled_pass);
  options.optimization_disabled_pass =
      absl::GetFlag(FLAGS_optimization_disabled_pass);
  options.codegen_allow_list = absl::GetFlag(FLAGS_codegen_allow_list);
  std::sort(options.codegen_allow_list.begin(),
            options.codegen_allow_list.end());
