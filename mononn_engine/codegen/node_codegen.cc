...
namespace codegen {
    ...
    std::string NodeCodegen::generate(std::shared_ptr<const CUDAContext> cuda_context, std::shared_ptr<const Op> node) {
        std::string result;

        if (Config::get()->print_hlo_text) result += "//" + node->get_hlo_text() + "\n";
        ...
        if (node->get_type() == OpType::custom_call ||
            (node->get_type() == OpType::get_tuple_element && !TF_MONONN_ENABLED) || // In TF_MONONN path, GET node will be generated in buffer pointer initialization.
...
////                Loop::Condition::less_than(key.get_name(), std::to_string(loop_shape.element_count())),
...
//            std::vector<std::string> multi_index = IndexTransform::offset_to_multi_index(node->get_output_spec(0).get_shape(), index_before_trace);
//            IndexTraceStamp its {loop.get_loop_key().get_name(), index_tracer.get_index(), "", "true", index_tracer.get_predictive()};
//
//            op_impl->set_traced_index({ its });
//            op_impl->set_need_generate_with_index(true);
//
////            ss << op_impl->as<PadImpl>()->generate_if_statement_begin(multi_index);
//            ss << op_impl->generate();
////            ss << op_impl->as<PadImpl>()->generate_if_statement_end();
////            ss << op_impl->as<PadImpl>()->generate_else();
//
//        } else {
//            IndexTraceStamp its {loop.get_loop_key().get_name(), index_tracer.get_index(), ""};
//
//            op_impl->set_traced_index({ its });
//            op_impl->set_need_generate_with_index(true);
//            ss << op_impl->generate();
