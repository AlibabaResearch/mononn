...
namespace optimization {
    ...
    void analyse(Op *node, NodeTraversalContext &context, std::set<std::string> &visit, std::string sub_cluster_tag) {
        ...
        if (node->get_type() == OpType::broadcast) {
            ...
            if (!dims.empty() && std::find(dims.begin(), dims.end(), node->get_output_spec(0).rank() - 1) == dims.end()) {
    ...
    bool AccessPatternAnalysisPass::run(Graph *graph, std::shared_ptr<CUDAContext> cuda_context) {
        for (auto const &cluster_node_name : graph->get_node_list_by_type(OpType::cluster)) {
            ...
            // Temporal buffer not larger than l1_cache_limit_in_bytes (20KB) to prevent cache thrashing.
            std::vector<Op *> parameter_temporal_access_list;

            for (auto const &node_name : cluster_node->get_graph_ptr()->get_node_list()) {
                auto node = cluster_node->get_graph_ptr()->get_node(node_name);

                if (node->has_attribute(OpAttribute::is_parameter_temporal_access) &&
                    node->get_attribute(OpAttribute::is_parameter_temporal_access) == "true") {
                    
                    parameter_temporal_access_list.push_back(node.get());
                }
            }

            std::sort(parameter_temporal_access_list.begin(), parameter_temporal_access_list.end(), [](const Op *a, const Op *b) -> bool {
                return a->get_output_spec(0).size_in_bytes() < b->get_output_spec(0).size_in_bytes();
            });

            int bufferred_size_in_bytes = 0;

            for (auto *node  : parameter_temporal_access_list) {
                if (bufferred_size_in_bytes + node->get_output_spec(0).size_in_bytes() <= l1_cache_limit_in_bytes) {
