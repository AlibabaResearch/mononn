...
namespace optimization {
    ...
    bool LocalityEscalationPass::run(Graph *graph, std::shared_ptr<CUDAContext> cuda_context) {
        for (auto const &cluster_node_name : graph->get_node_list_by_type(OpType::cluster)) {
            ...
            for (auto const &node_name : cluster_node->as<ClusterOp>()->get_graph()->get_node_list()) {
                std::shared_ptr<Op> node = cluster_node->as<ClusterOp>()->get_graph()->get_node(node_name);
                if (!node->has_attribute(OpAttribute::on_chip_transfer_from_node)) { continue; }
                std::string preceding_node_name = node->get_attribute(OpAttribute::on_chip_transfer_from_node);
                std::shared_ptr<Op> preceding_node = cluster_node->as<ClusterOp>()->get_graph()->get_node(preceding_node_name);

                LOG(INFO) << "In cluster " << cluster_node_name << " escalate locality for node " << preceding_node_name << " and " << node_name << ".";

                if (preceding_node->get_type() == OpType::reduce) {
                    LocalityTier::Tier tier = cluster_node->as<ClusterOp>()->get_schedule().get_locality_tier();
                    if (tier == LocalityTier::kT1) {
                        ParameterShflImpl::InputSpec input_spec;
                        input_spec.operand = Tensor(preceding_node_name, preceding_node->get_output_spec(0));
                        Tensor output(node_name, node->get_output_spec(0));
                        std::shared_ptr<OpImplBase> impl = ParameterShflImpl::get_available_implementations(cuda_context, input_spec, output)[0];
                        ...
                        input_spec.operand = Tensor(preceding_node_name, preceding_node->get_output_spec(0));
                        Tensor output(node_name, node->get_output_spec(0));
                        std::shared_ptr<OpImplBase> impl = ParameterSmemIMpl::get_available_implementations(cuda_context, input_spec, output)[0];
                        impl->set_hlo_text(node->get_hlo_text());
                        node->set_implementation(impl);
                        // node->propagate_index_to_implementation();
                    } else {
                        LOG(FATAL) << "Unsupported locality tier for reduce: " << tier.to_string();
                    }

                    cluster_node->as<ClusterOp>()->get_graph()->add_edge(preceding_node_name, node_name);
                } else {
                    std::string reg_buffer_name = preceding_node_name + "_reg_buffer";
                    std::string step_id = cluster_node->as<ClusterOp>()->get_schedule().get_inner_loop().get_loop_step_id();
                    std::string step_cnt = cluster_node->as<ClusterOp>()->get_schedule().get_inner_loop().get_loop_steps();
                    Tensor preceding_node_tensor(preceding_node_name, preceding_node->get_output_spec(0));
                    ...
                    std::shared_ptr<ParameterReadRegImpl> parameter_read_reg_impl =
                            std::make_shared<ParameterReadRegImpl>(cuda_context, parameter_read_reg_impl_spec,
                                                                   node_tensor);

                    output_reg_impl->set_hlo_text("//Write to reg buffer");
                    parameter_read_reg_impl->set_hlo_text(node->get_hlo_text());

                    preceding_node->add_auxiliary_impl(AuxiliaryImplType::buffer_in_register, std::static_pointer_cast<OpImplBase>(output_reg_impl));
                    node->set_implementation(std::static_pointer_cast<OpImplBase>(parameter_read_reg_impl));
                }
            }
        }
