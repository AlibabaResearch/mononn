...
namespace optimization {
    ...
    bool ImplementationAssignmentPass::run(Graph *graph, std::shared_ptr<CUDAContext> cuda_context) {
        for (auto const &graph_node_name : graph->get_node_list()) {
            ...
            if (graph_node->get_type() == OpType::cluster) {
                if (graph_node->is_cluster_elewise()) {
                    for (auto const node_name : std::static_pointer_cast<ClusterOp>(graph_node)->get_graph()->get_node_list()) {
                        std::shared_ptr<Op> node = std::static_pointer_cast<ClusterOp>(graph_node)->get_graph()->get_node(node_name);
                        std::vector<std::shared_ptr<OpImplBase>> impl_list = node->generate_candidate_implementation(cuda_context, LocalityTier::kT0);
                    ...
                    int reduce_implementation_id = this->graph_specification->cluster_reduce_spec().at(graph_node_name).reduce_implementation();
                    for (auto const node_name : std::static_pointer_cast<ClusterOp>(graph_node)->get_graph()->get_node_list()) {
                        std::shared_ptr<Op> node = std::static_pointer_cast<ClusterOp>(graph_node)->get_graph()->get_node(node_name);
                        if (node->get_type() == OpType::reduce) {
                            std::vector<std::shared_ptr<OpImplBase>> impl_list = node->generate_candidate_implementation(cuda_context, reduce_tier);
                ...
                if (graph_node->is_gemm()) {
                    std::unique_ptr<CutlassConfig> cutlass_config = std::make_unique<CutlassConfig>();
                    std::unique_ptr<GemmBackendConfig> gemm_backend_config = std::make_unique<GemmBackendConfig>();
                    auto const &gemm_spec = this->graph_specification->gemm_spec_list().at(graph_node_name);
                    ...
                    input_spec.A = Tensor(graph_node->get_operand(0)->get_name(), graph_node->get_operand(0)->get_output_spec(0));
                    input_spec.B = Tensor(graph_node->get_operand(1)->get_name(), graph_node->get_operand(1)->get_output_spec(0));
                    if (graph_node->get_operands().size() > 2) {
                        input_spec.C = std::make_shared<Tensor>(graph_node->get_operand(2)->get_name(), graph_node->get_operand(2)->get_output_spec(0));
                    }
                    ...
                    std::unique_ptr<CutlassConfig> cutlass_config = std::make_unique<CutlassConfig>();
                    std::unique_ptr<ConvBackendConfig> conv_backend_config = std::make_unique<ConvBackendConfig>();
                    auto const &conv_spec = this->graph_specification->conv_spec_list().at(graph_node_name);
                    ...
                    input_spec.filter_size = graph_node->as<CustomCall>()->get_filter_size();
                    input_spec.filter_stride = graph_node->as<CustomCall>()->get_filter_stride();
                    input_spec.padding_low = graph_node->as<CustomCall>()->get_padding_low();
                    input_spec.padding_high = graph_node->as<CustomCall>()->get_padding_high();

                    std::string output_tensor_name = EnvVar::is_true("TF_MONONN_ENABLED") ?
                        graph_node->as<CustomCall>()->get_conv_output_GTE_node_name() : // Output of conv is a tuple
                        graph_node->get_name();
                ...
                if (graph_node->get_type() == OpType::global_sync ||
                    ...
                    auto impl = graph_node->generate_candidate_implementation(cuda_context, LocalityTier::kT0)[0];
