...
namespace optimization {
    ...
    struct NodeTraversalContext {
        bool broadcast_on_highest_dimension = false; // Scalar broadcast is also considered broadcast on highest dimension.
    ...
    bool node_can_vectorized_as(Op *node, int vec_len, NodeTraversalContext &context, std::set<std::string> &visit, std::string sub_cluster_tag) {
        // For node does not belong to current sub cluster.
        if (node->get_attribute(OpAttribute::sub_cluster_tag) != sub_cluster_tag) return true;
        ...
        if (node->get_type() == OpType::broadcast) {
            ...
            if (dims.empty() /*Scalar broadcast*/ ||  std::find(dims.begin(), dims.end(), node->get_output_spec(0).rank() - 1) == dims.end() /*Highest dim not exists in origin operand*/ ) {
        ...
        for (int operand_id = 0; operand_id < node->get_operand_count(); ++operand_id) {
            NodeTraversalContext next_context = context;

            if ((node->get_type() == OpType::reduce || node->get_type() == OpType::reduce_window) && operand_id >= node->get_operand_count() / 2) {
            ...
            if (!node_can_vectorized_as(node->get_operand(operand_id).get(), vec_len, next_context, visit, sub_cluster_tag)) return false;
        }
        ...
        if (node->get_type() == OpType::parameter || node->get_type() == OpType::constant) {
            ...
            if (dim_continuous % vec_len != 0) {
                // LOG(DEBUG) << "Return false shape not valid " << node->get_name() << " " << dim_continuous << " " << vec_len << node->get_output_spec(0).to_string(); 
                return false;
            }
            
            // Widest memory access instruction in NVIDIA GPU 128 bit.
            int vector_size_in_bits = node->get_output_spec(0).get_dtype().get_primitive_type().size_in_bits() * vec_len;
            if (vector_size_in_bits > 128) {
                // LOG(DEBUG) << "Return false too wide access " << vector_size_in_bits << " " << node->get_name();
                return false;
            }
            
            for (int valid_vector_size_in_bits = 128; valid_vector_size_in_bits >= 8; valid_vector_size_in_bits >>= 1) {
                if (vector_size_in_bits == valid_vector_size_in_bits) return true;
            }

            // LOG(DEBUG) << "Return false size in bit not valid " << vector_size_in_bits << " " << node->get_name();
        ...
        //     if (dim_continuous % vec_len != 0) {
        //         LOG(DEBUG) << "Constant not valid " << node->get_output_spec(0).to_string() << " " << vec_len;
        //     }
        ...
        if (node->get_type() == OpType::transpose) {
            ...
            if (permute.back() != (int)permute.size() - 1) {
                ...
                // LOG(DEBUG) << "Return false Transpose check failed " << node->get_name();
                return false;
            }
        ...
        if (node->get_type() == OpType::pad) {
            ...
            if (highest_pad_high % vec_len != 0) {
                // LOG(DEBUG) << "Return false pad " << node->get_name();
                return false;
            }
            ...
            if ((highest_dimension - highest_pad_high - highest_pad_low) % vec_len != 0) {
                // LOG(DEBUG) << "Return false pad " << node->get_name();
        ...
        if (node->get_type() == OpType::reduce) {
            return node->get_operand(0)->get_output_spec(0).get_shape(-1) % vec_len == 0;
        }

        return true;
    }

    bool sub_cluster_can_vectorized_as(ClusterOp *cluster_node, int vec_len, std::string sub_cluster_tag) {

        // if (cluster_node->get_loop_shape().get_shape(-1) % vec_len != 0) return false;
        // if (cluster_node->get_schedule().get_inner_loop().get_loop_shape().element_count() % vec_len != 0) return false;

        std::vector<std::string> node_name_in_order;
        for (auto const &node_name : cluster_node->get_graph()->traverse_in_topology_order()) {
        ...
        for (auto node_name_it = node_name_in_order.rbegin(); node_name_it != node_name_in_order.rend(); ++node_name_it) {
            NodeTraversalContext context;
            if (!node_can_vectorized_as(cluster_node->get_graph()->get_node(*node_name_it).get(), vec_len, context, visit, sub_cluster_tag)) return false;
        }

        return true;
    }

    void do_node_vectorize(Op *node, int vec_len, NodeTraversalContext &context, std::set<std::string> &visit, std::string sub_cluster_tag) {
        if (node->get_attribute(OpAttribute::sub_cluster_tag) != sub_cluster_tag) { return; }

        std::string node_name = node->get_name();

        if (visit.count(node_name)) { return; }
        ...
        if (node->get_type() == OpType::broadcast) {
            ...
            // Broadcast dim do not contain highest dimension
            if (dims.empty() /*Scalar broadcast*/ || std::find(dims.begin(), dims.end(), node->get_output_spec(0).rank() - 1) == dims.end()) {
        ...
        for (int operand_id = 0; operand_id < node->get_operand_count(); ++operand_id) {
            ...
            NodeTraversalContext next_context = context;
            do_node_vectorize(operand.get(), vec_len, next_context, visit, sub_cluster_tag);
        }
        ...
        if (context.broadcast_on_highest_dimension && !first_semi_vectorized_broadcast) {
        ...
        if (std::any_of(output_specs.begin(), output_specs.end(), [](const TensorSpec &spec) -> bool {
        ...
        for (auto &spec : output_specs) {
            if (spec.get_shape(-1) % vec_len != 0 && node->get_type() != OpType::bitcast) {
                LOG(FATAL) << "Node " << node_name << node->get_output_spec(0).to_string() << " cannot be vectorized by " << vec_len;
            }
            ...
            if (node->get_type() == OpType::bitcast && spec.get_shape(-1) == 1) {
                if (spec.get_shape().rank() > 1 && spec.get_shape(-2) % vec_len == 0 && 
                    spec.get_shape(-2) == node->get_operand(0)->get_output_spec(0).get_shape(-1) * node->get_operand(0)->get_output_spec(0).get_dtype().get_elements_per_access()) {
                    ...
                    LOG(FATAL) << "Node " << node_name << node->get_output_spec(0).to_string() << " cannot be vectorized by " << vec_len
                        << ". Type: " << node->get_type().to_string() << " shape: " <<spec.to_string() 
                        << ", operand: " << node->get_operand(0)->get_name() << ", shape: " << node->get_operand(0)->get_output_spec(0).to_string();
                }
            } else {
    ...
    // currently we only use the same vectorization length for all sub-clusters in the same cluster.
    void do_vectorize(ClusterOp *cluster_node, int vec_len) {
        NodeTraversalContext context;

        for (auto const &sub_cluster_tag : cluster_node->get_sub_cluster_tag_order()) {
            std::vector<std::string> node_name_in_order;
            for (auto const &node_name : cluster_node->get_graph()->traverse_in_topology_order()) {
                auto node = cluster_node->get_graph()->get_node(node_name);
                if (node->get_attribute(OpAttribute::sub_cluster_tag) == sub_cluster_tag) {
    ...
    void vectorize_cluster(ClusterOp *cluster_node) {
        ...
        for (int vec_len = 16; vec_len > 1; vec_len >>= 1) {
            // LOG(DEBUG) << "=====Try vectorize " << cluster_node->get_name() << " with len " << vec_len; 
            bool can_vectorize = true;
            for (auto const& sub_cluster_tag : cluster_node->get_sub_cluster_tag_order()) {
                if (!sub_cluster_can_vectorized_as(cluster_node, vec_len, sub_cluster_tag)) {
                    can_vectorize = false;
                    break;
                }
            }
            ...
            if (can_vectorize) {
                LOG(INFO) << "Vectorize cluster node " << cluster_node->get_name() << " to " << vec_len << "...";
                do_vectorize(cluster_node, vec_len);
                return;
            }
        }

        LOG(INFO) << "Cluster node " << cluster_node->get_name() << " cannot be vectorized";
    }

    bool VectorizationPass::run(Graph *graph, std::shared_ptr<CUDAContext> cuda_context) {
        for (auto const &cluster_node_name : graph->get_node_list_by_type(OpType::cluster)) {
            // if (cluster_node_name != "fusion_256_MD_fusion_79_MD_fusion_77") continue;
