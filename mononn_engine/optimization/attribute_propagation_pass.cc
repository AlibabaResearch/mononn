...
namespace optimization {
    ...
    void propagate_parameter_attributes(Graph *graph) {
        ...
        for (auto const &cluster_node_name : graph->get_node_list_by_type(OpType::cluster)) {
            ...
            for (auto const &node_name : cluster_node->get_graph()->get_node_list_by_type(OpType::parameter)) {
                auto node = cluster_node->get_graph()->get_node(node_name);

                for (auto const &attr : attribute_propagate_list) {
                    if (node->has_attribute(attr)) {
                        node->get_implementation()->set_attribute(
                            attr, 
    ...
    bool AttributePropagationPass::run(Graph *graph, std::shared_ptr<CUDAContext> cuda_context) {
