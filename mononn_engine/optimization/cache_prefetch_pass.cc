...
namespace optimization {
    ...
    bool CachePrefetchPass::run(Graph *graph, std::shared_ptr<CUDAContext> cuda_context) {
        for (auto const &cluster_node_name : graph->get_node_list_by_type(OpType::cluster)) {
            ...
            for (auto const &node_name : cluster_node->get_graph_ptr()->get_node_list()) {
                ...
                if (node->get_type() == OpType::parameter && !node->has_attribute(OpAttribute::on_chip_transfer_from_node)) {
                    CachePrefetchImpl::InputSpec input_spec;
                    input_spec.operand = Tensor(node->get_name(), node->get_output_spec(0));
                    auto cache_prefetch_impl = CachePrefetchImpl::get_available_implementations(cuda_context, input_spec)[0];
                    cache_prefetch_impl->set_hlo_text("Cache prefetch");
                    node->add_auxiliary_impl(AuxiliaryImplType::cache_prefetch, cache_prefetch_impl);
