...
namespace optimization {
    ...
    bool sub_cluster_is_valid_for_smem_prefetch(ClusterOp *cluster_node, std::string sub_cluster_tag) {
        for (auto const &node_name : cluster_node->get_graph()->get_node_list()) {
            auto node = cluster_node->get_graph()->get_node(node_name);
            
            if (node->get_attribute(OpAttribute::sub_cluster_tag) != sub_cluster_tag) continue;

            if (node->get_type() == OpType::gather || 
                node->get_type() == OpType::dynamic_slice || // Dynamic slice cannot be vectorized, thus is unlikely profitable in prefetch.
                node->get_type() == OpType::dynamic_update_slice ||
                node->get_type() == OpType::reduce_window ||
                node->get_type() == OpType::pad) return false;
        }
    ...
    void do_sub_cluster_smem_prefetch(ClusterOp *cluster_node, std::string sub_cluster_tag, SmemManager *smem_manager, std::shared_ptr<CUDAContext> &cuda_context) {        
        const int async_pipeline_total_stage_count = 2;
        Loop inner_most_loop = cluster_node->get_schedule().get_inner_loop();

        cluster_node->set_attribute(OpAttribute::async_pipeline_total_stage_count, std::to_string(async_pipeline_total_stage_count));
        ...
        for (auto const node_name : cluster_node->get_graph()->traverse_in_topology_order()) {
            ...
            if (node->get_type() != OpType::parameter) { continue;}
            if (node->has_attribute(OpAttribute::on_chip_transfer_from_node)) continue;
            if (!node->has_attribute(OpAttribute::is_parameter_streaming_access)) { continue; }
            if (node->get_attribute(OpAttribute::sub_cluster_tag) != sub_cluster_tag) { continue; }
            auto type = node->get_output_spec(0).get_dtype();
            if (!type.is_vectorized()) { continue; }
            ...
            if (type.size_in_bits() == 128) {
                ...
                LOG(FATAL) << "Too large memory alignment of node " << node_name << " :" << type.size_in_bits() << " bits";
            }
        }

        auto get_smem_base_buf_size_in_bytes = [&](Op *node) -> int {
            int inner_loop_elem_count = inner_most_loop.get_loop_shape().element_count();
            int highest_dim_elem_count = node->get_output_spec(0).get_shape(-1);

            // For cluster constaining slice node we should use inner_loop_elem_count (which is smaller).
            // For cluster containing pad/concatenate we should use highest_dim_elem_count (which is smaller).
            ...
            int size_in_bytes = node->get_output_spec(0).get_dtype().size_in_bytes() * elem_count;
        ...
        auto get_smem_buf_size_in_bytes = [&](Op *node) -> int {
            ...
            // Warp locality require warp_per_block times smem as each warp works individually.
            if (cluster_node->get_schedule().get_locality_tier() == LocalityTier::kT1) {
                size_in_bytes *= cuda_context->cuda_runtime_context.block_dim.XYZ() / 32; // multiply by warp number per block.
            }
            ...
            if (node->get_symbolic_index().size() > 1) { // traced by multiple nodes.
                ...
                if (smem_manager->can_claim_buffer(buffer_size_list_in_bytes)) {
                    for (auto const &symbolic_index : node->get_symbolic_index()) {
                        ...
                        smem_manager->claim_smem_buffer(reuse_node_name, smem_buffer_name, smem_buf_size_in_bytes);
                        smem_manager->record_base_buffer_size(smem_buffer_name, get_smem_base_buf_size_in_bytes(node.get()));
                    }

                    node->set_attribute(OpAttribute::async_pipeline_total_stage_count, std::to_string(async_pipeline_total_stage_count));
                    ...
                    auto impl = SmemPrefetchImpl::get_available_implementations(cuda_context, input_spec, output)[0];
                ...
                if (smem_manager->can_claim_buffer(smem_buf_size_in_bytes)) {
                    ...
                    smem_manager->claim_smem_buffer(node_name, smem_buffer_name, smem_buf_size_in_bytes);
                    smem_manager->record_base_buffer_size(smem_buffer_name, get_smem_base_buf_size_in_bytes(node.get()));
    ...
    void do_cluster_smem_prefetch(ClusterOp *cluster_node, std::shared_ptr<CUDAContext> &cuda_context) {
        SmemManager *smem_manager = cluster_node->get_smem_manager();
        bool cluster_can_prefetch = false;

        if (cluster_node->get_name() == "input_fusion_reduce_43_MD_input_fusion_reduce_21_MD_fusion_105") {
            LOG(INFO) << "Cluster " << cluster_node->get_name() << " cannot be prefetched.";
            return;
        }

        if (cluster_node->get_cluster_type() == ClusterType::Reduce) {

            LOG_ONCE(WARNING, __log_once, "Only reduce cluster is supported at this stage");

            for (auto const &sub_cluster_tag : cluster_node->get_sub_cluster_tag_order()) {
                if (sub_cluster_is_valid_for_smem_prefetch(cluster_node, sub_cluster_tag)) {
                    LOG(INFO) << "Cluster: " << cluster_node->get_name() << " sub cluster: " << sub_cluster_tag << " is valid for asynchronous smem prefetch";
                    do_sub_cluster_smem_prefetch(cluster_node, sub_cluster_tag, smem_manager, cuda_context);
    ...
    bool SmemPrefetchPass::run(Graph *graph, std::shared_ptr<CUDAContext> cuda_context) {
        if (!mononn_engine::core::gpu::cutlass::Arch::newer_or_equal(cuda_context->cuda_device_context.get_cutlass_arch_tag(), mononn_engine::core::gpu::cutlass::Arch::Sm80)) {
            LOG(INFO) << "CUDA architecture: " << cuda_context->cuda_device_context.cuda_arch << " do not support LDGSTS instruction, skipping optimization pass " << this->name();
            return true; 
        }
