...
namespace optimization {
    ...
    bool ClusteringSingleNodePass::run(Graph *graph, std::shared_ptr<CUDAContext> cuda_context) {
        for (auto node_name : graph->get_node_list()) {
            // scalar constant may be already deleted
            if (!graph->has_node(node_name)) { 
                LOG(WARNING) << "ClusteringSingleNodePass::run not found node " << node_name;
                continue; 
            }
            ...
            if (node->get_type() == OpType::reduce) {
                cluster_node = std::make_shared<ClusterReduce>(new_cluster_name, cluster_operands, output_spec_list);
            } else {
                cluster_node = std::make_shared<ClusterElewise>(new_cluster_name, cluster_operands, output_spec_list);
            }
            ...
            for (int idx = 0; idx < (int)cluster_operands.size(); ++idx) {
                std::string parameter_node_name = "param_" + std::to_string(idx) + "_" + new_cluster_name;
                std::vector<TensorSpec> operand_output_spec = cluster_operands[idx]->get_output_specs();
                std::shared_ptr<Parameter> parameter_node = std::make_shared<Parameter>(parameter_node_name, std::vector<std::shared_ptr<Op>>{}, operand_output_spec);
                parameter_node->set_parameter_number(idx);

                parameter_node->set_attribute(OpAttribute::initial_cluster_tag, cluster_node->get_name());
                parameter_node->set_attribute(OpAttribute::sub_cluster_tag, cluster_node->get_name());
                parameter_node->set_attribute(OpAttribute::sub_cluster_type, cluster_node->get_cluster_type().to_string());
                parameter_node->set_attribute(OpAttribute::initial_cluster_type, cluster_node->get_cluster_type().to_string());
            ...
            for (int idx = 0; idx < node->get_operand_count(); ++idx) {
                if (node->get_operand(idx)->get_type() == OpType::constant &&
                    ...
                    std::string new_constant_node_name = node_constant_scalar->get_name() + "_" + node_name;
                    std::shared_ptr<Constant> new_constant_node = std::make_shared<Constant>(new_constant_node_name, std::vector<std::shared_ptr<Op>>{}, node_constant_scalar->get_output_specs());
                    ...
                    new_constant_node->set_attribute(OpAttribute::initial_cluster_tag, cluster_node->get_name());
                    new_constant_node->set_attribute(OpAttribute::sub_cluster_tag, cluster_node->get_name());
                    new_constant_node->set_attribute(OpAttribute::sub_cluster_type, cluster_node->get_cluster_type().to_string());
                    new_constant_node->set_attribute(OpAttribute::initial_cluster_type, cluster_node->get_cluster_type().to_string());
                    ...
                    node->replace_operand(node_constant_scalar->get_name(), new_constant_node);
                }
            }
            ...
            for (auto const &edge : graph->get_node_input_edges(node_name)) {
                if (edge->get_src()->get_type() == OpType::constant &&
                    ...
                    // We need to to keep scalar in the entry computation constant to build hlo instruction schedule
                    // This dead node will participate node codegen phase, but it seems ok.
                    // if (graph->get_node_output_edges(edge->get_src_name()).size() == 0) {
            ...
            LOG(INFO) << "Clustering node " << node_name << " into cluster " << new_cluster_name;
        }
