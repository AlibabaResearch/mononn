...
namespace mononn_engine {
    namespace optimization {
        ...
        bool ExplicitOutputPass::run(Graph *graph, std::shared_ptr<CUDAContext> cuda_context) {
            for (auto const &cluster_node_name : graph->get_node_list_by_type(OpType::cluster)) {
                ...
                for (int idx = 0; idx < cluster_node->get_graph()->get_output_node_count(); ++idx) {
                    ...
                    std::shared_ptr<OutputImpl> output_impl = std::make_shared<OutputImpl>(cuda_context, input_spec);
                    output_impl->set_hlo_text("// Explicit output node");
                    node->add_auxiliary_impl(AuxiliaryImplType::explicit_output_node, output_impl);
                }
            }
