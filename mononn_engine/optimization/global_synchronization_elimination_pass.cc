...
bool GlobalSynchronizationEliminationPass::run(
    Graph* graph, std::shared_ptr<CUDAContext> cuda_context) {
  std::vector<std::string> seed_node = mononn_engine::helpers::vector_concat(
      graph->get_input_nodes(), graph->get_extended_input_nodes());

  struct BFSNode {
    std::string name;
    int depth;

    bool operator<(const BFSNode& rhs) const { return this->depth > rhs.depth; }
  };
  ...
  while (!q.empty()) {
    ...
    for (auto const& control_edge :
         graph->get_node_output_control_edges(node.name)) {
      q.push({control_edge->get_dst_name(), node.depth});
    }
  }

  // Node
  // f_copy_42_MI_f_copy_31_MI_f_copy_52_MI_f_copy_55_MI_f_copy_37_MI_f_copy_36_MI_f_copy_58_MI_f_copy_60_MI_f_copy_33_MI_f_
  ...
  // f_copy_50_MI_f_copy_24_MI_f_copy_44_MI_f_copy_23_MI_f_copy_28_MI_f_copy_54_MI_f_copy_38_MI_f_copy_35_MI_f_copy_40
  // do not belong to any independent group

  for (auto const& node_name : graph->get_node_list()) {
    if (visit.find(node_name) == visit.end()) {
      LOG(ERROR) << "Input node list: "
                 << mononn_engine::helpers::join(", ",
                                                 graph->get_input_nodes());
      LOG(ERROR) << "Extended input node list: "
                 << mononn_engine::helpers::join(
  ...
  for (int step = 1; step <= max_step; ++step) {
    ...
    std::shared_ptr<GlobalSync> global_sync =
        std::make_shared<GlobalSync>(global_sync_node_name);
    ...
    for (auto const& node_name : node_by_step[step - 1]) {
      ...
      for (auto& edge : graph->get_node_output_edges(node_name)) {
        if (edge->get_sync() == Synchronization::Global) {
          if (visit[edge->get_dst_name()] < step)
            LOG(FATAL) << "Independent group topology error";
    ...
    EXPECT_TRUE(merged_edges.size() != 0,
                "No merged node for step " + std::to_string(step));
    if (merged_edges.size() > 1)
      LOG(INFO) << "Merge global sync: "
                << mononn_engine::helpers::join(" ", merged_edges) << " to "
                << global_sync_node_name;
  }

  //        LOG(DEBUG) << "------------------------";
  //
  //        for (auto const &edge :
  //        graph->get_node_output_control_edges("global_sync_1")) {
  //            LOG(DEBUG) << edge->to_string();
  //        }
  //        LOG(DEBUG) << "------------------------";
  //        for (auto const &edge :
  //        graph->get_node_input_control_edges("global_sync_1")) {
  //            LOG(DEBUG) << edge->to_string();
  //        }
  //
  ...
  //                if (step == max_step && node_type != OpType::parameter &&
  //                node_type != OpType::constant && node_type !=
  //                OpType::get_tuple_element) {
  //                    node_list.push_back(node_name);
  //                }
  //            }
  //
  //            LOG(DEBUG) << "step" << step << " node list: " <<
