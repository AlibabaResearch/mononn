...
namespace optimization {
    ...
    bool ElementWiseConcatenationPass::run(Graph *graph, std::shared_ptr<CUDAContext> cuda_context) {
        for (auto const &cluster_node_name : graph->get_node_list_by_type(OpType::cluster)) {
            auto cluster_node = std::static_pointer_cast<ClusterOp>(graph->get_node(cluster_node_name));
            ...
            if (cluster_node->is_cluster_elewise()) {
                ...
                if (!down_stream_nodes.empty()) {
                    ...
                    std::vector<std::string> path_nodes_name = graph->search_nodes_between_two_nodes(cluster_node->get_name(), end_cluster->get_name());
                    ...
                    std::string new_cluster_name = cluster_node->get_name() + "_" + this->name() + "_" + end_cluster->get_name();
                    std::shared_ptr<ClusterOp> new_cluster = ClusterUtil::merge_sequential<ClusterElewise>(
                    ...
                    LOG(INFO) << "Pass: " << this->name() << ". Concatenate " << cluster_node->get_name() << " and " << end_cluster->get_name() << " into " << new_cluster_name;
