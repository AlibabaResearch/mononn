...
namespace frontend {
    ...
    public:
        MonoNNModule() = delete;
        MonoNNModule(
            const xla::HloModule *_hlo_module, 
            ...
            const xla::HloAliasAnalysis *_alias_analysis)
                : hlo_module(_hlo_module), graph(nullptr), tuning_spec(std::move(_tuning_spec)), 
                    kernel_name(_kernel_name), allocation_list(_allocation_list), alias_analysis(_alias_analysis) {}
        ...
        MonoNNModule(
            ...
            const std::vector<xla::BufferAllocation> *_allocation_list,
            const xla::HloAliasAnalysis *_alias_analysis)
                : hlo_module(_hlo_module), graph(std::move(_graph)), tuning_spec(std::move(_tuning_spec)), 
                    kernel_name(_kernel_name), allocation_list(_allocation_list), alias_analysis(_alias_analysis) {}      
