  ...
  if (this->is_instruction_parallelized()) {
    for (int ilp_id = 0; ilp_id < this->get_instruction_parallel_factor();
      ...
      ss << mononn_engine::helpers::string_format(
          ...
          mononn_engine::helpers::get_node_ilp_name(operand_name, ilp_id),
          mononn_engine::helpers::get_node_ilp_name(update_name, ilp_id)
              .c_str());
    }
  } else {
    ss << mononn_engine::helpers::string_format(
        "%s %s = %s ? %s : %s;\n", type.to_string().c_str(), node_name.c_str(),
        this->concrete_index_list[0].pred_after_trace.c_str(),
        operand_name.c_str(), update_name.c_str());
  }
...
DynamicUpdateSliceImpl::get_available_implementations(
    std::shared_ptr<CUDAContext> cuda_context,
    DynamicUpdateSliceImpl::InputSpec input_spec, Tensor output) {
  std::shared_ptr<DynamicUpdateSliceImpl> slice_impl =
      std::make_shared<DynamicUpdateSliceImpl>(cuda_context, input_spec,
                                               output);
