...
std::string OutputRegImpl::generate_impl() const {
  ...
  if (this->is_instruction_parallelized()) {
    ...
    for (int ilp_id = 0; ilp_id < this->get_instruction_parallel_factor();
      ...
      //                                         mononn_engine::helpers::get_node_ilp_name(node_name,
      //                                         ilp_id).c_str());
      ...
      ss << Memory::write(
          ...
          this->input_spec.reg_buffer_name, index);
    }
    ...
    return Memory::write(Memory::AccessFlavor::STRONG_TYPED, type, node_name,
                         this->input_spec.reg_buffer_name,
                         this->input_spec.step_id);
...
std::vector<Tensor> OutputRegImpl::get_output_tensor() const { return {}; }

int OutputRegImpl::get_elements_per_access() const {
  return this->input_spec.operand.get_dtype().get_elements_per_access();
}

bool OutputRegImpl::need_pre_inner_loop_generation() const { return true; }
...
std::string OutputRegImpl::generate_pre_inner_loop() const {
  ...
  //         ss << mononn_engine::helpers::string_format("%s %s[%s];\n",
  //                 type.to_string().c_str(),
  //                 mononn_engine::helpers::get_node_ilp_name(this->input_spec.reg_buffer_name,
  //                 ilp_id).c_str(), this->input_spec.step_cnt.c_str());
  //     }
