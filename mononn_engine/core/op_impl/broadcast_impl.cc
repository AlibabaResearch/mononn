...
namespace op_impl {
    ...
    std::string BroadcastImpl::generate_impl() const {
        ...
        if (this->is_instruction_parallelized()) {
            for (int ilp_id = 0; ilp_id < this->get_instruction_parallel_factor(); ++ilp_id) {
                ss << type.to_string() << " " << mononn_engine::helpers::get_node_ilp_name(output_name, ilp_id) << " = " <<
    ...
    std::string BroadcastImpl::generate_with_index_impl() const {
        std::string operand_name = this->input_spec.operand.get_name();
        std::string operand_buffer_name = BufferManager::get_buffer_name(operand_name);
        std::string output_name = this->output.get_name();
        ...
        if (this->is_instruction_parallelized()) {
            ...
            for (int ilp_id = 0; ilp_id < this->get_instruction_parallel_factor(); ++ilp_id) {
                std::string ilp_output_name = mononn_engine::helpers::get_node_ilp_name(output_name, ilp_id);
                std::string ilp_index = this->ilp_concrete_index_list[ilp_id][0].index_after_trace;
                ss << Memory::read(Memory::AccessFlavor::REGULAR, type, ilp_output_name, operand_buffer_name, ilp_index, false);
            }

            return ss.str();
        } else {
            std::string index = this->concrete_index_list[0].index_after_trace;
            return Memory::read(Memory::AccessFlavor::REGULAR, type, output_name, operand_buffer_name, index, true);
        }
    }
    ...
    std::vector<std::shared_ptr<OpImplBase>> BroadcastImpl::get_available_implementations(
            std::shared_ptr<CUDAContext> cuda_context, 
            InputSpec input_spec, 
            Tensor output
    ) {
        std::shared_ptr<BroadcastImpl> broadcast_impl = std::make_shared<BroadcastImpl>(cuda_context, input_spec, output);
