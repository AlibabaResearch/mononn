...
namespace op_impl {
    ...
    public:
        ...
        ConvImpl(
                ...
                ConvBackendConfig _conv_backend_config,
                Tensor _output)
                : cuda_context(_cuda_context), input_spec(_input_spec), cutlass_config(_cutlass_config), conv_backend_config(_conv_backend_config), output(_output) {
            this->setup();
        }
