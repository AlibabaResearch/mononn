...
namespace op_impl {
        ...
        if (this->is_instruction_parallelized()) {
            for (int ilp_id = 0; ilp_id < this->get_instruction_parallel_factor(); ++ilp_id) {
                ss << mononn_engine::helpers::string_format("%s %s = %s;",
                       ...
                       mononn_engine::helpers::get_node_ilp_name(this->output.get_name().c_str(), ilp_id).c_str(),
                       this->input_spec.operand.get_name().c_str()) << "\n";
            }
        } else {
            ss << mononn_engine::helpers::string_format("%s %s = %s;",
               type.to_string().c_str(), this->output.get_name().c_str(), this->input_spec.operand.get_name().c_str()) << "\n";
        }
    ...
    ParameterSmemImpl::get_available_implementations(
        ...
        Tensor output) {
        std::shared_ptr<ParameterSmemImpl> impl = std::make_shared<ParameterSmemImpl>(cuda_context, input_spec, output);
