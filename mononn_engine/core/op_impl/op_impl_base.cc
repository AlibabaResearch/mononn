...
namespace op_impl {
    ...
    std::string OpImplBase::generate() const {
        ...
        if (this->need_generate_with_index) {
            ...
            if (this->is_instruction_parallelized() && this->ilp_concrete_index_list.empty()) {
    ...
    std::string OpImplBase::generate_impl() const {
        LOG(FATAL) << "OpImplBase::generate_impl need to be implemented in derived class. " << typeid(*this).name();
    }

    std::string OpImplBase::generate_with_index_impl() const {
        LOG(FATAL) << "OpImplBase::generate_with_index_impl need to be implemented in derived class." << typeid(*this).name();
    }
    ...
    void OpImplBase::propagate_attributes(const std::unordered_map<std::string, std::string> &attrs) {
    ...
    // void OpImplBase::propagate_attributes_impl(const std::unordered_map<std::string, std::string> &attrs) {
    ...
    std::string OpImplBase::get_hlo_text() const {
        return this->hlo_dumped_text;
    }

    int OpImplBase::get_smem_usage_in_bytes() const {
        return 0;
    }

//    void OpImplBase::set_concrete_index(std::vector<ConcreteIndexStamp> _concrete_index_list) {
...
//    }
    ...
    std::string OpImplBase::get_upstream_index_trace_node(std::string index) const {
        auto iter = std::find_if(this->concrete_index_list.begin(), this->concrete_index_list.end(), [&](ConcreteIndexStamp const &its) -> bool {
            return its.index_after_trace == index;
        });

        EXPECT_TRUE(iter != this->concrete_index_list.end(), "Cannot find traced index equal to " + index);

        return iter->traced_by;
    }

    std::string OpImplBase::get_upstream_ilp_index_trace_node(std::string index, int ilp_id) const {
        auto iter = std::find_if(this->ilp_concrete_index_list[ilp_id].begin(), this->ilp_concrete_index_list[ilp_id].end(),
             [&](ConcreteIndexStamp const &its) -> bool {
            return its.index_after_trace == index;
        });

        EXPECT_TRUE(iter != this->ilp_concrete_index_list[ilp_id].end(),
                    "Cannot find traced index equal to" + index +  " with ilp factor " + std::to_string(ilp_id));

        return iter->traced_by;
    }

    void OpImplBase::add_operand_reuse_mask(std::string origin_operand_name, std::string reuse_operand_name) {
    ...
    bool OpImplBase::has_operand_reuse_mask(std::string operand_name) const {
        return this->operand_reuse_mask.find(operand_name) != this->operand_reuse_mask.end();
    }
    ...
    void OpImplBase::add_auxiliary_impl(std::string key, std::shared_ptr<OpImplBase> _impl) {
    ...
    void OpImplBase::instantiate_concrete_index(
        ...
        this->instantiate_concrete_index_impl(symbolic_index_list, params, loop_stride);

        for (auto &[key, auxiliary_impl] : this->auxiliary_impls) {
            auxiliary_impl->instantiate_concrete_index(symbolic_index_list, params, loop_stride);
        }
    }
    ...
    void OpImplBase::instantiate_ilp_concrete_index(
        ...
        this->instantiate_ilp_concrete_index_impl(symbolic_index_list, params, loop_stride, ilp_stride);
        for (auto &[key, auxiliary_impl] : this->auxiliary_impls) {
            auxiliary_impl->instantiate_ilp_concrete_index(symbolic_index_list, params, loop_stride, ilp_stride);
        }
    }
    ...
    void OpImplBase::instantiate_concrete_index_impl(
        ...
        for (auto const &symbolic_index : symbolic_index_list) {
            this->concrete_index_list.push_back(symbolic_index.instantiate(complete_params));
        }
    }
    ...
    void OpImplBase::instantiate_ilp_concrete_index_impl(
        ...
        for (int ilp_id = 0; ilp_id < this->get_instruction_parallel_factor(); ++ilp_id) {
            ...
            for (auto const& [key, value] : params) {
                if (key == "linear_index") {
                    ilp_params[key] = mononn_engine::helpers::string_format("((%s) + (%s * %d))", value.c_str(), ilp_stride.c_str(), ilp_id);
                } else {
            ...
            for (auto const &symbolic_index : symbolic_index_list) {
                this->ilp_concrete_index_list[ilp_id].push_back(symbolic_index.instantiate(ilp_params));
            }
        }
    }

    // void OpImplBase::set_ilp_traced_index(int ilp_id, std::vector<IndexTraceStamp> _traced_index_list) {
