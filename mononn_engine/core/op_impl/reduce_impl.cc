...
ReduceImpl::ReduceImpl(std::shared_ptr<CUDAContext> _context,
  ...
  this->elements_per_access =
      this->input_spec.operands[0].get_dtype().get_elements_per_access();
  ...
  //     for (int param_idx = this->input_spec.operands.size() / 2; param_idx <
  //     this->input_spec.operands.size(); ++param_idx) {
  ...
  //     reduce_accum_init =
  //     Limits::get_min_negative(this->output.get_dtype().get_primitive_type());
  // } else if (this->input_spec.reducer == Reducer::Min) {
  //     reduce_accum_init =
  //     Limits::get_max_positive(this->output.get_dtype().get_primitive_type());
  ...
  // this->reduce_accum = Scalar(this->output.get_name() + "_accum",
  // init_type_list, init_value_list);

  if (this->tier == LocalityTier::kT0) {
    this->post_reduce_if_statement =
        mononn_engine::helpers::string_format("if (true) {\n");
  } else if (this->tier == LocalityTier::kT1) {
    this->post_reduce_if_statement =
        mononn_engine::helpers::string_format("if (lane_id == 0) {\n");
  } else if (this->tier == LocalityTier::kT2) {
    this->post_reduce_if_statement =
        mononn_engine::helpers::string_format("if (threadIdx.x == 0) {\n");
  } else {
...
std::string ReduceImpl::get_post_reduce_if_end() const { return "}"; }

int ReduceImpl::get_smem_usage_in_bytes() const {
  if (this->tier == LocalityTier::kT2) {
    int size_in_bytes;

    for (int operand_id = 0; operand_id < this->input_spec.operands.size();
         ++operand_id) {
      size_in_bytes +=
          this->input_spec.operands[operand_id].get_dtype().size_in_bytes();
    }
...
ReduceImpl::get_available_implementations(std::shared_ptr<CUDAContext> context,
  ...
  if (tier == LocalityTier::kT1) {
    std::shared_ptr<ReduceImpl> impl_warp_custom_reduce =
        std::make_shared<ReduceImpl>(context, input_spec, output);
    std::shared_ptr<ReduceImpl> impl_warp_reduce_cub =
        std::make_shared<ReduceImpl>(context, input_spec, output);
    ...
    warp_custom_reduce.add_template_arg(
        input_spec.reduce_accum.get_type_string());
    // warp_custom_reduce.add_template_arg(reducer.get_functor_type());
    warp_custom_reduce.add_template_arg(
        input_spec.reduction_functor_generator->type_name());
    warp_custom_reduce.add_template_arg(
    ...
    // input_spec.operands[0].get_dtype().is_vectorized();
    ...
    return {std::static_pointer_cast<OpImplBase>(impl_warp_custom_reduce),
            std::static_pointer_cast<OpImplBase>(impl_warp_reduce_cub)};
  }

  if (tier == LocalityTier::kT2) {
    std::shared_ptr<ReduceImpl> impl_block_custom_reduce =
        std::make_shared<ReduceImpl>(context, input_spec, output);
    std::shared_ptr<ReduceImpl> impl_block_reduce_cub_RAKING =
        std::make_shared<ReduceImpl>(context, input_spec, output);
    std::shared_ptr<ReduceImpl> impl_block_reduce_cub_RAKING_COMMUTATIVE_ONLY =
        std::make_shared<ReduceImpl>(context, input_spec, output);
    std::shared_ptr<ReduceImpl> impl_block_reduce_cub_WARP_REDUCTIONS =
        std::make_shared<ReduceImpl>(context, input_spec, output);
    ...
    FunctionInvocation block_reduce_cub_RAKING_COMMUTATIVE_ONLY(
        "block_reduce_cub_RAKING_COMMUTATIVE_ONLY");
    FunctionInvocation block_reduce_cub_WARP_REDUCTIONS(
    ...
    impl_block_reduce_cub_RAKING->set_attribute("name",
                                                "block_reduce_cub_RAKING");

    impl_block_reduce_cub_RAKING_COMMUTATIVE_ONLY->set_invocation(
        invocations[2]);
    impl_block_reduce_cub_RAKING_COMMUTATIVE_ONLY->set_attribute(
        "name", "block_reduce_cub_RAKING_COMMUTATIVE_ONLY");

    impl_block_reduce_cub_WARP_REDUCTIONS->set_invocation(invocations[3]);
    impl_block_reduce_cub_WARP_REDUCTIONS->set_attribute(
        "name", "block_reduce_cub_WARP_REDUCTIONS");

    return {std::static_pointer_cast<OpImplBase>(impl_block_custom_reduce),
            std::static_pointer_cast<OpImplBase>(impl_block_reduce_cub_RAKING),
            std::static_pointer_cast<OpImplBase>(
                impl_block_reduce_cub_RAKING_COMMUTATIVE_ONLY),
            std::static_pointer_cast<OpImplBase>(
                impl_block_reduce_cub_WARP_REDUCTIONS)};
  }

  LOG(FATAL) << "Unexpected locality tier: " << tier.to_string();
}

ReduceImpl::Tier ReduceImpl::get_tier() const { return this->tier; }

std::string ReduceImpl::generate_impl() const {
  std::stringstream code_stream;

  FunctionInvocation vector_reduce_invocation(
      this->input_spec.reduction_functor_generator->instance_name());
  vector_reduce_invocation.add_arg(this->reduce_accum.get_name());

  auto to_tuple_name_if_needed =
      [](const std::vector<std::string>& name_list) -> std::string {
    if (name_list.size() == 1) {
      return name_list[0];
    }

    return "tuple_util::make_tuple(" +
           mononn_engine::helpers::join(", ", name_list) + ")";
  };

  if (this->input_spec.operands[0].get_dtype().is_vectorized() ||
    ...
    if (this->input_spec.operands[0].get_dtype().get_elements_per_access() !=
        1) {
      to_scalar.add_template_arg(std::to_string(
          this->input_spec.operands[0].get_dtype().get_elements_per_access()));
    }

    to_scalar.add_template_arg(
        this->input_spec.reduction_functor_generator->type_name());

    // if (this->input_spec.reducer == Reducer::Max) {
    //     to_scalar.add_template_arg(mononn_engine::helpers::string_format("cutlass::maximum<%s>",
    //     output_primitive_type.to_string().c_str()));
    // } else if (this->input_spec.reducer == Reducer::Min) {
    //     to_scalar.add_template_arg(mononn_engine::helpers::string_format("minimum::maximum<%s>",
    //     output_primitive_type.to_string().c_str()));
    // } else if (this->input_spec.reducer == Reducer::Sum) {
    //     to_scalar.add_template_arg(mononn_engine::helpers::string_format("cutlass::plus<%s>",
    //     output_primitive_type.to_string().c_str()));
    // } else {
    //     LOG(FATAL) << "Unsupported reducer " <<
    //     (int)this->input_spec.reducer;
    // }
    ...
    if (this->is_instruction_parallelized()) {
      for (int ilp_id = 0; ilp_id < this->get_instruction_parallel_factor();
        ...
        for (int operand_id = 0;
             operand_id < (int)this->input_spec.operands.size(); ++operand_id) {
          name_list.push_back(mononn_engine::helpers::get_node_ilp_name(
    ...
    std::vector<std::string> name_list;
    for (int operand_id = 0; operand_id < (int)this->input_spec.operands.size();
         ++operand_id) {
      name_list.push_back(this->input_spec.operands[operand_id].get_name());
    }
  ...
  code_stream << this->reduce_accum.get_name() << " = "
              << vector_reduce_invocation.to_string() << ";\n";
...
//
//        if (this->is_instruction_parallelized()) {
//            for (int ilp_id = 0; ilp_id <
//            this->get_instruction_parallel_factor(); ++ilp_id) {
//                std::string index =
//                this->ilp_traced_index_list[ilp_id][0].index_before_trace;
//                TensorShape output_shape = this->output.get_shape();
//                std::string reduction_index =
//                mononn_engine::helpers::string_format("(%s) / (%d)",
//                index.c_str(), output_shape.get_shape(-1)); std::string
//                output_buffer =
//                BufferManager::get_buffer_name(this->output.get_name());
//                std::string ptr = mononn_engine::helpers::string_format("%s +
//                (%s)", output_buffer.c_str(), reduction_index.c_str());
//
...
//                + "[" + std::to_string(ilp_id) + "]"); ss <<
//                reduce_invocation.to_string() << ";\n";
//            }
//        } else {
...
std::string ReduceImpl::generate_reduce() const {
  ...
  //     for (int ilp_id = 0; ilp_id < this->get_instruction_parallel_factor();
  //     ++ilp_id) {
  //         code_stream << "const "<< type.to_string() << " &" <<
