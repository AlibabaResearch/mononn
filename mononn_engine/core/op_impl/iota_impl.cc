...
namespace op_impl {
    ...
    std::string IotaImpl::generate_with_index_impl() const {
        ...
        if (this->input_spec.iota_dimension == 0) { // iota on the lowest dimension
            if (this->output.rank() == 1) {
                if (this->is_instruction_parallelized()) {
                    for (int ilp_id; ilp_id < this->get_instruction_parallel_factor(); ++ilp_id) {
                ...
                if (this->is_instruction_parallelized()) {
                    for (int ilp_id; ilp_id < this->get_instruction_parallel_factor(); ++ilp_id) {
                        std::string index = this->ilp_concrete_index_list[ilp_id][0].index_after_trace;
                        std::string div = std::to_string(this->output.get_shape().slice_dim(1, -1).element_count());
                        ss << type.to_string() << " " << mononn_engine::helpers::get_node_ilp_name(node_name, ilp_id) << " = ";
        ...
        } else if (this->input_spec.iota_dimension == this->output.rank() - 1) { // iota on the highest dimension
            if (this->is_instruction_parallelized()) {
                for (int ilp_id; ilp_id < this->get_instruction_parallel_factor(); ++ilp_id) {
                    std::string index = this->ilp_concrete_index_list[ilp_id][0].index_after_trace;
                    ...
                    if (type.is_vectorized()) {
                        ...
                        for (int element_id = 0; element_id < type.get_elements_per_access(); ++element_id) {
                            std::string linear_index = mononn_engine::helpers::string_format("(%s %% %s)", index.c_str(), mod.c_str());
                            std::string iota_value = mononn_engine::helpers::string_format("%s * %d + %d", linear_index.c_str(), type.get_elements_per_access(), element_id);
                            ss << mononn_engine::helpers::string_format("%s[%d] = %s;\n", mononn_engine::helpers::get_node_ilp_name(node_name, ilp_id).c_str(), element_id, iota_value.c_str());
                        }
                    } else {
                        ss << type.to_string() << " " << mononn_engine::helpers::get_node_ilp_name(node_name, ilp_id) << " = ";
                ...
                if (type.is_vectorized()) {
                    ...
                    for (int element_id = 0; element_id < type.get_elements_per_access(); ++element_id) {
                        std::string linear_index = mononn_engine::helpers::string_format("(%s %% %s)", index.c_str(), mod.c_str());
                        std::string iota_value = mononn_engine::helpers::string_format("%s * %d + %d", linear_index.c_str(), type.get_elements_per_access(), element_id);
                        ss << mononn_engine::helpers::string_format("%s[%d] = %s;\n", node_name.c_str(), element_id, iota_value.c_str());
                    }
                } else {
                    ss << type.to_string() << " " << node_name << " = ";
                    ss << mononn_engine::helpers::string_format("(%s %% %s);\n", index.c_str(), mod.c_str());
            ...
            if (this->is_instruction_parallelized()) {
                for (int ilp_id; ilp_id < this->get_instruction_parallel_factor(); ++ilp_id) {
                    std::string index = this->ilp_concrete_index_list[ilp_id][0].index_after_trace;
                    std::string mod = std::to_string(this->output.get_shape().slice_dim(this->input_spec.iota_dimension, -1).element_count());
                    std::string div = std::to_string(this->output.get_shape().slice_dim(this->input_spec.iota_dimension + 1, -1).element_count());
    ...
    std::vector<Tensor> IotaImpl::get_input_tensor() const {
        return {};
    }
    ...
    IotaImpl::get_available_implementations(std::shared_ptr<CUDAContext> cuda_context, IotaImpl::InputSpec input_spec,
                                            Tensor output) {
        std::shared_ptr<IotaImpl> impl = std::make_shared<IotaImpl>(cuda_context, input_spec, output);
        return {
            std::static_pointer_cast<OpImplBase>(impl)
        };
    }
}
}
