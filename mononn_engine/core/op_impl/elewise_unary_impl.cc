...
namespace op_impl {
    ...
    std::string ElewiseUnaryImpl::generate_impl() const {
        ...
        if (this->is_instruction_parallelized()) {
            for (int ilp_id = 0; ilp_id < this->get_instruction_parallel_factor(); ++ilp_id) {
                ...
                ss << this->get_invocation().get_ilp_function_invocation(ilp_id).to_string();
                ss << ";\n";
            }
        } else {
    ...
    void ElewiseUnaryImpl::set_instruction_parallel_factor(int _ilp_factor) {
        ...
        Functor functor(this->get_invocation_functor().get_raw_name(), this->output.get_dtype());
    ...
    std::vector<std::shared_ptr<OpImplBase>> ElewiseUnaryImpl::get_available_implementations(
        std::shared_ptr<CUDAContext> cuda_context, 
        InputSpec input_spec, 
        Tensor output
    ) {
        std::string functor_name = Functor::get_functor_name_for_op_type(input_spec.op_type);
        ...
        std::shared_ptr<OpImplBase> op_impl = std::static_pointer_cast<OpImplBase>(std::make_shared<ElewiseUnaryImpl>(cuda_context, input_spec, output));
