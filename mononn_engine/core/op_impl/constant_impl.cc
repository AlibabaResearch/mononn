...
namespace op_impl {
    ...
    std::string ConstantImpl::generate_impl() const {
        ...
        if (this->output.element_count() != 1) {
            LOG(FATAL) << this->output.get_name() << " is not scalar, need index to fetch value";
        }
        ...
        if (this->input_spec.value == "inf") {
            ...
            ss << mononn_engine::helpers::string_format("half(%s);", this->input_spec.value.c_str());
        } else {
            ss << mononn_engine::helpers::string_format("%s;", this->input_spec.value.c_str());
        }
        ...
        if (this->is_instruction_parallelized()) {
            for (int ilp_id = 0; ilp_id < this->get_instruction_parallel_factor(); ++ilp_id) {
                ss << "const " << type.to_string() << " &" << mononn_engine::helpers::get_node_ilp_name(op_name, ilp_id) << " = " << op_name << ";\n";
            }
        }
    ...
    std::string ConstantImpl::generate_with_index_impl() const {
        LOG(FATAL) << "Deprecated, all non-scalar constant should be read as parameter.";
        ...
        if ((!this->is_instruction_parallelized() && this->concrete_index_list.size() > 1) ||
            (this->is_instruction_parallelized() && this->ilp_concrete_index_list[0].size() > 1)) { // need operand reuse mask
            std::stringstream ss;
            if (this->is_instruction_parallelized()) {
    ...
    std::vector<Tensor> ConstantImpl::get_input_tensor() const {
        return {};
    }
    ...
    std::vector<std::shared_ptr<OpImplBase>> ConstantImpl::get_available_implementations(
            std::shared_ptr<CUDAContext> cuda_context,
            InputSpec input_spec,
            Tensor output
    ) {
        std::shared_ptr<ConstantImpl> constant_impl = std::make_shared<ConstantImpl>(cuda_context, input_spec, output);
