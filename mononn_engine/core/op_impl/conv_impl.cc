...
namespace op_impl {
    ...
    std::string ConvImpl::generate_impl() const {
        ...
        epilogue.add_template_arg(this->output.get_dtype().to_cutlass_type().to_string());
        ...
        conv_kernel.add_template_arg(this->input_spec.A.get_dtype().to_cutlass_type().to_string());
        conv_kernel.add_template_arg(this->LayoutA.to_string());
        conv_kernel.add_template_arg(this->input_spec.B.get_dtype().to_cutlass_type().to_string());
        conv_kernel.add_template_arg(this->LayoutB.to_string());
        conv_kernel.add_template_arg(this->output.get_dtype().to_cutlass_type().to_string());
        ...
        conv_kernel.add_template_arg(this->cutlass_config.ThreadBlockShape.to_string());
        conv_kernel.add_template_arg(this->cutlass_config.WarpShape.to_string());
        conv_kernel.add_template_arg(this->cutlass_config.InstructionShape.to_string());
        conv_kernel.add_template_arg(epilogue.get_name());
        conv_kernel.add_template_arg(cutlass::Swizzle::GemmXThreadblockSwizzle.to_string());
        ...
        conv_kernel.add_template_arg(cutlass::IteratorAlgorithm::kOptimized.to_string());
        ...
        conv_codegen_argument.ptr_a = BufferManager::get_buffer_name(this->conv_arguments.ptr_a);
        conv_codegen_argument.ptr_b = BufferManager::get_buffer_name(this->conv_arguments.ptr_b);
        conv_codegen_argument.ptr_d = BufferManager::get_buffer_name(this->conv_arguments.ptr_d);

        if (this->has_bias()) {
            conv_codegen_argument.ptr_c = BufferManager::get_buffer_name(this->conv_arguments.ptr_c);
        }

        ss << conv_codegen_argument.define_variable(kernel_name, conv_argument_name);
        ss << mononn_engine::helpers::string_format("typename %s::Params %s(%s, nullptr);\n", kernel_name.c_str(), conv_parameter_name.c_str(), conv_argument_name.c_str());
        ss << mononn_engine::helpers::string_format("cutlass_wrapper::cutlass_device_gemm_universal<%s>(%s);\n", kernel_name.c_str(), conv_parameter_name.c_str());
    ...
    bool ConvImpl::has_bias() const {
        return this->input_spec.C != nullptr;
    }
    ...
    void ConvImpl::set_conv_backend_config(const ConvBackendConfig &_conv_backend_config) {
    ...
    ConvImpl::get_available_implementations(std::shared_ptr<CUDAContext> cuda_context, InputSpec input_spec,
        ...
        ConvBackendConfig conv_backend_config = ConvImpl::parse_conv_backend_config(backend_config_str);
        ...
        while (input_spec.A.get_tensor_spec().get_tensor_shape_with_ordered_memory_layout().get_shape(3) % alignment != 0) { alignment >>= 1; }
        ...
        for (auto const &desc : available_tile_description) {
            ...
            if (alignment == 1 && A_type.size_in_bytes() <= 2 && cutlass::Arch::newer_or_equal(desc.get_ArchTag(), cutlass::Arch::Sm80)) {
        ...
        for (auto const &desc : valid_tile_description) {
            ...
            cutlass_config.OperatorClass = desc.get_op_class(); // only support tensor cores at this moment;
            cutlass_config.ArchTag = desc.get_ArchTag();
            cutlass_config.stages = desc.get_stages();

            std::shared_ptr<ConvImpl> conv_impl = std::make_shared<ConvImpl>(cuda_context, input_spec, cutlass_config, conv_backend_config, output);
    ...
    void ConvImpl::setup() {
        // NHWC
        TensorShape tensor_a_ordered_shape = this->input_spec.A.get_tensor_spec().get_tensor_shape_with_ordered_memory_layout();
        // KRSC
        TensorShape tensor_b_ordered_shape = this->input_spec.B.get_tensor_spec().get_tensor_shape_with_ordered_memory_layout();
        // NPQK
        TensorShape tensor_d_ordered_shape = this->output.get_tensor_spec().get_tensor_shape_with_ordered_memory_layout();

        EXPECT_TRUE(tensor_a_ordered_shape.get_shape(3) == tensor_b_ordered_shape.get_shape(3), "Channel dim not match");
        EXPECT_TRUE(tensor_b_ordered_shape.get_shape(1) == this->input_spec.filter_size[0], "Filter H dim not match");
        EXPECT_TRUE(tensor_b_ordered_shape.get_shape(2) == this->input_spec.filter_size[1], "Filter W dim not match");
        EXPECT_TRUE(tensor_d_ordered_shape.get_shape(0) == tensor_a_ordered_shape.get_shape(0), "Batch dim not match");
        EXPECT_TRUE(tensor_d_ordered_shape.get_shape(3) == tensor_b_ordered_shape.get_shape(0), "Filter K dim not match");
        ...
        this->problem_size.stride_h = std::to_string(this->input_spec.filter_stride[0]);
        this->problem_size.stride_w = std::to_string(this->input_spec.filter_stride[1]);
        ...
        this->conv_arguments.alpha = std::to_string(this->conv_backend_config.conv_result_scale);
        this->conv_arguments.beta = std::to_string(this->conv_backend_config.side_input_scale);
        ...
        while (channel % this->alignmentA != 0) { this->alignmentA >>= 1; }

        this->alignmentB = this->alignmentA;

        while (std::stoi(this->problem_size.K) % this->alignmentC != 0) { this->alignmentC >>= 1; }
    }

    std::string ConvImpl::get_problem_size_name() const {
        std::string node_name = this->output.get_name();
        return mononn_engine::helpers::string_format("%s_conv_problem_size", node_name.c_str());
    }

    std::string ConvImpl::get_epilogue_name() const {
        std::string node_name = this->output.get_name();
        return mononn_engine::helpers::string_format("%s_conv_epilogue_linear_combination", node_name.c_str());
    }

    std::string ConvImpl::get_conv_argument_name() const {
        std::string node_name = this->output.get_name();
        return mononn_engine::helpers::string_format("%s_conv_arg", node_name.c_str());
    }

    std::string ConvImpl::get_conv_parameter_name() const {
        std::string node_name = this->output.get_name();
        return mononn_engine::helpers::string_format("%s_conv_params", node_name.c_str());
    }

    std::string ConvImpl::get_kernel_name() const {
        std::string node_name = this->output.get_name();
        return mononn_engine::helpers::string_format("%s_conv_kernel", node_name.c_str());
    }
}
}
}
