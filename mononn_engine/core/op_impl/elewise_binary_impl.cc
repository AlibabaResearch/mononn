...
namespace op_impl {
    ...
    std::string ElewiseBinaryImpl::generate_impl() const {
        ...
        if (this->is_instruction_parallelized()) {
            for (int ilp_id = 0; ilp_id < this->get_instruction_parallel_factor(); ++ilp_id) {
                auto ilp_invocation = this->get_invocation().get_ilp_function_invocation(ilp_id);
    ...
    void ElewiseBinaryImpl::set_instruction_parallel_factor(int _ilp_factor) {
        ...
        Dtype type = this->input_spec.op_type == OpType::compare ? this->input_spec.operand1.get_dtype() : this->output.get_dtype();
    ...
    std::vector<std::shared_ptr<OpImplBase>> ElewiseBinaryImpl::get_available_implementations(
        ...
        std::shared_ptr<OpImplBase> op_impl = std::static_pointer_cast<OpImplBase>(std::make_shared<ElewiseBinaryImpl>(cuda_context, input_spec, output));
    ...
    std::vector<std::shared_ptr<OpImplBase>> ElewiseBinaryImpl::get_available_implementations_for_compare(
        std::shared_ptr<CUDAContext> cuda_context, 
        InputSpec input_spec, 
        Tensor output,
        MathOp math_op
        ...
        OpType op_type = input_spec.op_type;
        std::string functor_name = Functor::get_functor_name_for_op_type(op_type, math_op);
    ...
    std::vector<std::shared_ptr<OpImplBase>> ElewiseBinaryImpl::get_available_implementations_for_select(
            std::shared_ptr<CUDAContext> cuda_context,
            InputSpec input_spec,
            Tensor output,
            Tensor pred) {
