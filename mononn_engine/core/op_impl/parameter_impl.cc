  ...
  if ((!this->is_instruction_parallelized() &&
    ...
    if (this->is_instruction_parallelized()) {  // ilp
      ...
      for (int ilp_id = 0; ilp_id < this->get_instruction_parallel_factor();
        ...
        for (auto const& traced_index : this->ilp_concrete_index_list[ilp_id]) {
          ...
          reuse_op_name =
              mononn_engine::helpers::get_node_ilp_name(reuse_op_name, ilp_id);
          ss << this->memory_read_wrapper(type, reuse_op_name, buffer_name,
                                          index, false, pred);
        }
      }
    } else {  // no ilp
      for (auto const& traced_index : this->concrete_index_list) {
        ...
        std::string reuse_op_name =
            op_name + "_reuse_" + traced_index.traced_by;
        ss << this->memory_read_wrapper(type, reuse_op_name, buffer_name, index,
                                        true, pred);
      }
    }
    ...
    if (this->is_instruction_parallelized()) {  // ilp
      ...
      for (int ilp_id = 0; ilp_id < this->get_instruction_parallel_factor();
        ...
        ss << this->memory_read_wrapper(type, ilp_op_name, buffer_name, index,
                                        true, pred);
      }
      ...
      return this->memory_read_wrapper(type, op_name, buffer_name, index, true,
                                       pred);
    }
  }
}

std::vector<Tensor> ParameterImpl::get_input_tensor() const { return {}; }
...
std::string ParameterImpl::memory_read_wrapper(
    Dtype access_type, std::string var_name, std::string src_ptr,
    std::string offset, bool define_variable, std::string pred) const {
  if (this->has_attribute(OpAttribute::is_parameter_temporal_access)) {
    return Memory::read(Memory::AccessFlavor::EXPLICIT_PTX_EVICT_LAST,
                        access_type, var_name, src_ptr, offset, define_variable,
                        pred);
  } else {
    return Memory::read(Memory::AccessFlavor::REGULAR, access_type, var_name,
                        src_ptr, offset, define_variable, pred);
  }
}

std::vector<std::shared_ptr<OpImplBase>>
ParameterImpl::get_available_implementations(
    std::shared_ptr<CUDAContext> cuda_context, Tensor output) {
  std::shared_ptr<ParameterImpl> impl =
      std::make_shared<ParameterImpl>(cuda_context, output);
