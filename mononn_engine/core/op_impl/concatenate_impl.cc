...
namespace op_impl {
    ...
    std::string ConcatenateImpl::generate_with_index_impl() const {
        ...
        if (this->is_instruction_parallelized()) {
            for (int ilp_id = 0; ilp_id < this->get_instruction_parallel_factor(); ++ilp_id) {
                EXPECT_TRUE(this->ilp_concrete_index_list[ilp_id].size() == 1, "Multiple traced index");
                std::string offset_index = this->ilp_concrete_index_list[ilp_id][0].index_before_trace;
                std::vector<std::string> per_dim_index = IndexTransform::offset_to_multi_index(shape, offset_index);
                int concat_dimension = this->input_spec.dimension;
                std::string concat_dimension_index = per_dim_index[concat_dimension];
                std::vector<std::string> predictions;

                for (int operand_id = 0; operand_id < (int)this->input_spec.operands.size() - 1; ++operand_id) {
                    std::string concat_dimension_range;

                    for (int idx = 0; idx <= operand_id; ++idx) {
                        if (idx == 0) concat_dimension_range = std::to_string(this->input_spec.operands[idx].get_shape(concat_dimension));
                        else concat_dimension_range = concat_dimension_range + " + " + std::to_string(this->input_spec.operands[idx].get_shape(concat_dimension));
                    }

                    predictions.push_back(mononn_engine::helpers::string_format("(%s < %s)", concat_dimension_index.c_str(), concat_dimension_range.c_str()));
                }

                FunctionInvocation concat(mononn_engine::helpers::string_format("cutlass::Concat<%s, %d>::do_concat", primitive_type.to_string().c_str(), type.get_elements_per_access()));

                for (int operand_id = 0; operand_id < (int)this->input_spec.operands.size() - 1; ++operand_id) {
                    concat.add_arg(predictions[operand_id]);
                    concat.add_arg(mononn_engine::helpers::get_node_ilp_name(this->input_spec.operands[operand_id].get_name(), ilp_id));
                }

                concat.add_arg(mononn_engine::helpers::get_node_ilp_name(this->input_spec.operands.back().get_name(), ilp_id));

                ss << type.to_string() << " " << mononn_engine::helpers::get_node_ilp_name(node_name, ilp_id) << " = " << concat.to_string() << ";\n";
            }
        } else {
    ...
    ConcatenateImpl::get_available_implementations(
        ...
        std::shared_ptr<ConcatenateImpl> impl = std::make_shared<ConcatenateImpl>(cuda_context, input_spec, output);
