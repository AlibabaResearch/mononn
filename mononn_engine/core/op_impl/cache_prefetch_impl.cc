    ...
    if (this->is_instruction_parallelized()) {  // ilp
      for (int ilp_id = 0; ilp_id < this->get_instruction_parallel_factor();
        ...
        for (auto const& traced_index : this->ilp_concrete_index_list[ilp_id]) {
          ...
          std::string prefetch_predicate =
              mononn_engine::helpers::string_format(
                  "(%s) && (%s)", pred.c_str(),
                  this->get_attribute(OpAttribute::prefetch_predicate).c_str());
          ss << Memory::prefetch_l1(type, buffer_name, index,
                                    prefetch_predicate);
        }
      }
    } else {  // no ilp
      for (auto const& traced_index : this->concrete_index_list) {
        auto const& index = traced_index.index_after_trace;
        auto const& pred = traced_index.pred_after_trace;
        std::string prefetch_predicate = mononn_engine::helpers::string_format(
            "(%s) && (%s)", pred.c_str(),
            this->get_attribute(OpAttribute::prefetch_predicate).c_str());
...
std::vector<Tensor> CachePrefetchImpl::get_output_tensor() const { return {}; }
...
void CachePrefetchImpl::instantiate_concrete_index_impl(
  ...
  for (auto const& [key, value] : params) {
    if (key == "linear_index") {
      prefetch_params[key] = mononn_engine::helpers::string_format(
          "(%s) + (%s)", value.c_str(), loop_stride.c_str());
    } else {
  ...
  for (auto const& symbolic_index : symbolic_index_list) {
    this->concrete_index_list.push_back(
        symbolic_index.instantiate(prefetch_params));
  }
}
  ...
  for (int ilp_id = 0; ilp_id < this->get_instruction_parallel_factor();
    ...
    for (auto const& [key, value] : params) {
      if (key == "linear_index") {
        ...
        ilp_prefetch_params[key] = mononn_engine::helpers::string_format(
            "((%s) + (%s) + (%s * %d))", value.c_str(), loop_stride.c_str(),
    ...
    for (auto const& symbolic_index : symbolic_index_list) {
      this->ilp_concrete_index_list[ilp_id].push_back(
          symbolic_index.instantiate(ilp_prefetch_params));
    }
  }
}

// void CachePrefetchImpl::propagate_attributes_impl(const
...
CachePrefetchImpl::get_available_implementations(
    std::shared_ptr<CUDAContext> cuda_context, InputSpec input_spec) {
  std::shared_ptr<OpImplBase> impl =
      std::make_shared<CachePrefetchImpl>(cuda_context, input_spec);
