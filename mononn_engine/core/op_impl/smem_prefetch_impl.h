...
namespace op_impl {
    ...
    public:
        ...
        SmemPrefetchImpl(std::shared_ptr<CUDAContext> _cuda_context, InputSpec _input_spec, Tensor _output)
    ...
    private:
        ...
        std::vector<ConcreteIndexStamp> async_pipeline_initialization_concrete_index_list;
        ...
        std::vector<std::vector<ConcreteIndexStamp>> ilp_async_pipeline_initialization_concrete_index_list;
        std::vector<std::vector<ConcreteIndexStamp>> ilp_async_pipeline_prefetch_concrete_index_list;
        std::vector<std::vector<ConcreteIndexStamp>> ilp_smem_access_concrete_index_list;
        ...
        std::string access_smem_buf(
            const std::string &smem_buf_name, 
            const std::string &stage_id, 
            const std::string &index,
            const std::string &pred,
            const std::string &default_value = "0") const;
        std::string access_global_mem_buf(
            const std::string &buf_name,
            const std::string &index,
            const Dtype &type) const;

        std::string generate_async_copy_invocation(int bytes_per_access, std::string smem_ptr, std::string global_mem_ptr, std::string pred) const;
    };
