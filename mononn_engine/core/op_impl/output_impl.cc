...
std::string OutputImpl::generate_impl() const {
  ...
  std::string node_buffer_name =
      BufferManager::get_buffer_name(node_name) + "_output";
  ...
  if (this->is_instruction_parallelized()) {
    ...
    for (int ilp_id = 0; ilp_id < this->get_instruction_parallel_factor();
         ++ilp_id) {
      EXPECT_TRUE(
          this->ilp_concrete_index_list[ilp_id].size() == 1,
          "Output node should have exactly index, where as " +
              this->input_spec.operand.get_name() + " have " +
              std::to_string(this->ilp_concrete_index_list[ilp_id].size()));
      std::string index =
      ...
      ss << Memory::write(Memory::AccessFlavor::REGULAR, type, ilp_node_name,
                          node_buffer_name, index);
    }

    return ss.str();
  } else {
    EXPECT_TRUE(this->concrete_index_list.size() == 1,
                "Output node should have exactly one index, where as " +
                    this->input_spec.operand.get_name() + " have " +
                    std::to_string(this->get_concrete_index_count()));
    std::string index = this->get_concrete_index(0).index_before_trace;
    return Memory::write(Memory::AccessFlavor::REGULAR, type, node_name,
                         node_buffer_name, index);
  }
}
...
std::vector<Tensor> OutputImpl::get_output_tensor() const { return {}; }
...
OutputImpl::get_available_implementations(
    std::shared_ptr<CUDAContext> cuda_context, InputSpec input_spec) {
  std::shared_ptr<OutputImpl> impl =
