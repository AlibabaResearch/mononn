...
namespace op_impl {
    ...
    std::string ClampImpl::generate_impl() const {
        std::string functor_name = Functor::get_functor_name_for_op_type(OpType::clamp);
        ...
        if (this->is_instruction_parallelized()) {
            for (int ilp_id = 0; ilp_id < this->get_instruction_parallel_factor(); ++ilp_id) {
                ...
                invocation.add_arg(mononn_engine::helpers::get_node_ilp_name(this->input_spec.min_val.get_name(), ilp_id));
                invocation.add_arg(mononn_engine::helpers::get_node_ilp_name(this->input_spec.operand.get_name(), ilp_id));
                invocation.add_arg(mononn_engine::helpers::get_node_ilp_name(this->input_spec.max_val.get_name(), ilp_id));
    ...
    std::vector<Tensor> ClampImpl::get_input_tensor() const {
        return { this->input_spec.min_val, this->input_spec.operand, this->input_spec.max_val };
    }
