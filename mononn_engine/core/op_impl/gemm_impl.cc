...
namespace cutlass_wrapper{
    ...
    std::string GemmImpl::generate_impl() const {
        GemmUniversalArgument codegen_gemm_universal_argument = this->gemm_universal_arguments;
        codegen_gemm_universal_argument.ptr_A = BufferManager::get_buffer_name(this->gemm_universal_arguments.ptr_A);
        codegen_gemm_universal_argument.ptr_B = BufferManager::get_buffer_name(this->gemm_universal_arguments.ptr_B);
        codegen_gemm_universal_argument.ptr_D = BufferManager::get_buffer_name(this->gemm_universal_arguments.ptr_D);

        if (this->gemm_universal_arguments.ptr_C != "nullptr") {
            codegen_gemm_universal_argument.ptr_C = BufferManager::get_buffer_name(this->gemm_universal_arguments.ptr_C);
        }

        std::stringstream ss;

        Using epilogue = Using(this->output.get_name() + "_epilogue_linear_combination", "cutlass::epilogue::thread::LinearCombination");
        Using gemm_kernel = Using(this->output.get_name() + "_gemm_kernel", "cutlass::gemm::kernel::DefaultGemmUniversal");
        ...
        codegen_gemm_universal_argument.alpha = mononn_engine::helpers::string_format("%s(%s)",
             output_type.to_cutlass_type().to_string().c_str(), this->gemm_universal_arguments.alpha.c_str());
        codegen_gemm_universal_argument.beta = mononn_engine::helpers::string_format("%s(%s)",
             output_type.to_cutlass_type().to_string().c_str(), this->gemm_universal_arguments.beta.c_str());
        ...
        gemm_kernel.add_template_arg(this->cutlass_config.ThreadBlockShape.to_string());
        gemm_kernel.add_template_arg(this->cutlass_config.WarpShape.to_string());
        gemm_kernel.add_template_arg(this->cutlass_config.InstructionShape.to_string());
        gemm_kernel.add_template_arg(epilogue.get_name());
        gemm_kernel.add_template_arg(cutlass::Swizzle::GemmXThreadblockSwizzle.to_string());
        ...
        ss << codegen_gemm_universal_argument.define_variable(gemm_kernel.get_name(), gemm_arg_name) << "\n";

        ss << "typename " << gemm_kernel.get_name() << "::Params " << gemm_params_name << " = " << "cutlass::gemm::device::GemmUniversalBase<" << gemm_kernel.get_name() << ">" << "::Argument2Params(" << gemm_arg_name << ");\n";

        FunctionInvocation gemm_call("cutlass_wrapper::cutlass_device_gemm_universal");
    ...
    int GemmImpl::get_smem_usage_in_bytes() const {
        return SharedStorage::get_shared_storage_size(
            this->cutlass_config.ThreadBlockShape,
            this->cutlass_config.stages,
    ...
    GemmBackendConfig GemmImpl::parse_gemm_backend_config(std::string config_str) {
        ...
        if (config["dot_dimension_numbers"]["lhs_contracting_dimensions"].size() != 1) {
            LOG(FATAL) << "Lhs contracting dimension num is not one: " << config_str;
        }

        gemm_backend_config.lhs_contracting_dimensions = std::stoi(std::string(config["dot_dimension_numbers"]["lhs_contracting_dimensions"][0]));

        if (config["dot_dimension_numbers"]["rhs_contracting_dimensions"].size() != 1) {
            LOG(FATAL) << "Rhs contracting dimension num is not one: " << config_str;
        }

        gemm_backend_config.rhs_contracting_dimensions = std::stoi(std::string(config["dot_dimension_numbers"]["rhs_contracting_dimensions"][0]));
        gemm_backend_config.batch_size = std::stoi(std::string(config["batch_size"]));

        if (gemm_backend_config.batch_size > 1) {
            std::vector<std::string> lhs_batch_dimensions = config["dot_dimension_numbers"]["lhs_batch_dimensions"];
            std::vector<std::string> rhs_batch_dimensions = config["dot_dimension_numbers"]["rhs_batch_dimensions"];

            EXPECT_TRUE(lhs_batch_dimensions.size() == rhs_batch_dimensions.size(), "Batch dim size should be same");

            for (int idx = 0; idx < (int)lhs_batch_dimensions.size(); ++idx) {
                gemm_backend_config.lhs_batch_dimensions.push_back(std::stoi(lhs_batch_dimensions[idx]));
                gemm_backend_config.rhs_batch_dimensions.push_back(std::stoi(rhs_batch_dimensions[idx]));
            }
        }
        ...
        gemm_backend_config.selected_algorithm = std::stoi(std::string(config["selected_algorithm"]));
    ...
    void GemmImpl::setup() {
        ...
        if (this->gemm_backend_config.batch_size == 1) {
            ...
            this->gemm_universal_arguments.batch_count = std::to_string(this->gemm_backend_config.batch_size);
        }
        ...
        EXPECT_TRUE(A.get_shape(this->gemm_backend_config.lhs_contracting_dimensions) == B.get_shape(this->gemm_backend_config.rhs_contracting_dimensions), 
                "GEMM contracting dimension not match");

        EXPECT_TRUE(A.rank() == B.rank() && B.rank() == D.rank(), "Rank should be same");
        ...
        for (int idx = 0; idx < rank; ++idx) {
            if (idx != this->gemm_backend_config.lhs_contracting_dimensions && 
                std::find(this->gemm_backend_config.lhs_batch_dimensions.begin(), 
                            this->gemm_backend_config.lhs_batch_dimensions.end(), idx) == this->gemm_backend_config.lhs_batch_dimensions.end()) {
            ...
            if (idx != this->gemm_backend_config.rhs_contracting_dimensions && 
                std::find(this->gemm_backend_config.rhs_batch_dimensions.begin(), 
                            this->gemm_backend_config.rhs_batch_dimensions.end(), idx) == this->gemm_backend_config.rhs_batch_dimensions.end()) {
        ...
        // Parse ABCD layout
        this->LayoutA = m_dimension < this->gemm_backend_config.lhs_contracting_dimensions ? cutlass::Layout::RowMajor : cutlass::Layout::ColumnMajor;
        this->LayoutB = n_dimension > this->gemm_backend_config.rhs_contracting_dimensions ? cutlass::Layout::RowMajor : cutlass::Layout::ColumnMajor;
        ...
        // Colmajor example: %cublas-batch-gemm.1 = f16[4,2,128,128]{3,2,1,0} custom-call(
        // f16[4,2,64,128]{3,2,1,0} %fusion.36,
        // f16[4,2,128,64]{3,2,1,0} %get-tuple-element.8),
        // custom_call_target="__cublas$gemm", metadata={
        ...
        if (this->LayoutA == cutlass::Layout::RowMajor) {
            EXPECT_TRUE(this->gemm_backend_config.lhs_contracting_dimensions + 1 == A.rank(),
                        "Ill formed lhs contracting dim " + std::to_string(this->gemm_backend_config.lhs_contracting_dimensions));
        } else { // column major
            EXPECT_TRUE(this->gemm_backend_config.lhs_contracting_dimensions + 2 == A.rank(),
                        "Ill formed lhs contracting dim " + std::to_string(this->gemm_backend_config.lhs_contracting_dimensions));
        }

        if (this->LayoutB == cutlass::Layout::RowMajor) {
            EXPECT_TRUE(this->gemm_backend_config.rhs_contracting_dimensions + 1 == B.rank() - 1,
                        "Ill formed rhs contracting dim " + std::to_string(this->gemm_backend_config.rhs_contracting_dimensions));
        } else { // column major
            EXPECT_TRUE(this->gemm_backend_config.rhs_contracting_dimensions + 1 == B.rank(),
                        "Ill formed rhs contracting dim " + std::to_string(this->gemm_backend_config.rhs_contracting_dimensions));
        }
        ...
        if (this->LayoutD == cutlass::Layout::RowMajor) {
            ...
            this->gemm_universal_arguments.alpha = std::to_string(this->gemm_backend_config.alpha_real);
            this->gemm_universal_arguments.beta = std::to_string(this->gemm_backend_config.beta);
            ...
            this->gemm_universal_arguments.batch_stride_A = std::to_string(A.get_shape(m_dimension) * A.get_shape(this->gemm_backend_config.lhs_contracting_dimensions));
            this->gemm_universal_arguments.batch_stride_B = std::to_string(B.get_shape(n_dimension) * B.get_shape(this->gemm_backend_config.rhs_contracting_dimensions));
            this->gemm_universal_arguments.batch_stride_D = std::to_string(D.get_shape(-1) * D.get_shape(-2));
            this->gemm_universal_arguments.stride_a = std::to_string(this->LayoutA == cutlass::Layout::RowMajor ? k : m);
            this->gemm_universal_arguments.stride_b = std::to_string(this->LayoutB == cutlass::Layout::RowMajor ? n : k);
            this->gemm_universal_arguments.stride_d = std::to_string(this->LayoutD == cutlass::Layout::RowMajor ? n : m);

            EXPECT_TRUE(this->gemm_universal_arguments.batch_stride_A == std::to_string(this->gemm_backend_config.lhs_stride),
                        mononn_engine::helpers::string_format("Validation Failed, parsed %s, calculated %s",
                                                        std::to_string(this->gemm_backend_config.lhs_stride).c_str(),
                                                        this->gemm_universal_arguments.batch_stride_A.c_str()));
            EXPECT_TRUE(this->gemm_universal_arguments.batch_stride_B == std::to_string(this->gemm_backend_config.rhs_stride),
                        mononn_engine::helpers::string_format("Validation Failed, parsed %s, calculated %s",
            ...
            if (this->gemm_backend_config.beta != 0) {
                ...
                this->gemm_universal_arguments.batch_stride_C = std::to_string(C.get_shape(-1) * C.get_shape(-2));
                this->gemm_universal_arguments.stride_c = std::to_string(this->LayoutC == cutlass::Layout::RowMajor ? n : m);
            } else {
            ...
            this->LayoutD = cutlass::Layout::RowMajor;
            this->LayoutA = this->LayoutA == cutlass::Layout::RowMajor ? cutlass::Layout::ColumnMajor : cutlass::Layout::RowMajor;
            this->LayoutB = this->LayoutB == cutlass::Layout::RowMajor ? cutlass::Layout::ColumnMajor : cutlass::Layout::RowMajor;
            ...
            this->gemm_universal_arguments.ptr_D = D.get_name();
            this->gemm_universal_arguments.batch_stride_A = std::to_string(B.get_shape(n_dimension) * B.get_shape(this->gemm_backend_config.rhs_contracting_dimensions));
            this->gemm_universal_arguments.batch_stride_B = std::to_string(A.get_shape(m_dimension) * A.get_shape(this->gemm_backend_config.lhs_contracting_dimensions));
            this->gemm_universal_arguments.batch_stride_D = std::to_string(D.get_shape(-1) * D.get_shape(-2));
            this->gemm_universal_arguments.stride_a = std::to_string(this->LayoutA == cutlass::Layout::RowMajor ? k : n);
            this->gemm_universal_arguments.stride_b = std::to_string(this->LayoutB == cutlass::Layout::RowMajor ? m : k);
            this->gemm_universal_arguments.stride_d = std::to_string(this->LayoutD == cutlass::Layout::RowMajor ? m : n);

            EXPECT_TRUE(this->gemm_universal_arguments.batch_stride_A == std::to_string(this->gemm_backend_config.rhs_stride),
                        mononn_engine::helpers::string_format("Validation Failed, parsed %s, calculated %s",
                                                        std::to_string(this->gemm_backend_config.lhs_stride).c_str(),
                                                        this->gemm_universal_arguments.batch_stride_A.c_str()));
            EXPECT_TRUE(this->gemm_universal_arguments.batch_stride_B == std::to_string(this->gemm_backend_config.lhs_stride),
                        mononn_engine::helpers::string_format("Validation Failed, parsed %s, calculated %s",
                                                        std::to_string(this->gemm_backend_config.rhs_stride).c_str(),
                                                        this->gemm_universal_arguments.batch_stride_B.c_str()));
            ...
            if (this->gemm_backend_config.beta != 0) {
                ...
                this->gemm_universal_arguments.ptr_C = C.get_name();
                this->gemm_universal_arguments.batch_stride_C = std::to_string(C.get_shape(-1) * C.get_shape(-2));
                this->gemm_universal_arguments.stride_c = std::to_string(this->LayoutC == cutlass::Layout::RowMajor ? m : n);
            } else {
                this->gemm_universal_arguments.ptr_C = "nullptr";
                this->gemm_universal_arguments.batch_stride_C = "0";
                this->gemm_universal_arguments.stride_c = "0";
        ...
        while (std::stoi(this->gemm_universal_arguments.stride_a) % this->alignmentA != 0) {
            this->alignmentA >>= 1;
        }

        while (std::stoi(this->gemm_universal_arguments.stride_b) % this->alignmentB != 0) {
            this->alignmentB >>= 1;
        }

        while (std::stoi(this->gemm_universal_arguments.stride_c) % this->alignmentC != 0) {
            this->alignmentC >>= 1;
        }
    }
    ...
    std::vector<std::shared_ptr<OpImplBase>> GemmImpl::get_available_implementations(
        ...
        std::vector<cutlass::TileDescription> available_tile_description = 
            cutlass::TileDescription::get_available_tile_description(cuda_context->cuda_device_context.get_cutlass_arch_tag(), input_spec.A.get_dtype());

        std::vector<cutlass::TileDescription> valid_tile_description;
        GemmBackendConfig gemm_backend_config = GemmImpl::parse_gemm_backend_config(backend_config_str);
        ...
        for (auto const &desc : available_tile_description) {
            if (cutlass::SharedStorage::get_shared_storage_size(desc.get_ThreadblockShape(), desc.get_stages(), A_type.size_in_bytes(), B_type.size_in_bytes())
                > cuda_context->cuda_runtime_context.smem_size) {
                continue;
            }

            if (cuda_context->cuda_runtime_context.block_dim.XYZ() != desc.threads_per_block()) {
                continue;
            }
        ...
        if (valid_tile_description.empty()) {
            ...
            ss << "A: " << input_spec.A.to_string() << " B: " << input_spec.B.to_string() << " D: " << output.to_string() << "\n";
        ...
        for (auto const &desc : valid_tile_description) {
            ...
            std::shared_ptr<GemmImpl> gemm_impl = std::make_shared<GemmImpl>(cuda_context, input_spec, cutlass_config, gemm_backend_config, output);
