...
namespace op {
    ...
    OpType Concatenate::get_type() const {
        return OpType::concatenate;
    }

    std::vector<std::shared_ptr<OpImpl>> Concatenate::generate_candidate_implementation(std::shared_ptr<CUDAContext> context, Tier tier) const {
        ...
        std::vector<std::shared_ptr<OpImpl>> impls = ConcatenateImpl::get_available_implementations(context, input_spec, output);
    ...
    int Concatenate::get_dimension() const {
