...
namespace op {
    ...
    OpType Multiply::get_type() const {
        return OpType::multiply;
    }

    std::vector<std::shared_ptr<OpImpl>> Multiply::generate_candidate_implementation(std::shared_ptr<CUDAContext> context, Tier tier) const {
