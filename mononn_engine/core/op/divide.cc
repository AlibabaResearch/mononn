...
namespace op {
    ...
    OpType Divide::get_type() const {
        return OpType::divide;
    }

    std::vector<std::shared_ptr<OpImpl>> Divide::generate_candidate_implementation(std::shared_ptr<CUDAContext> context, Tier tier) const {
