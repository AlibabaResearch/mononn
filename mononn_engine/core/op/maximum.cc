...
namespace op {
    ...
    OpType Maximum::get_type() const {
        return OpType::maximum;
    }

    std::vector<std::shared_ptr<OpImpl>> Maximum::generate_candidate_implementation(std::shared_ptr<CUDAContext> context, Tier tier) const {
