...
namespace op {
    ...
    ClusterElewise::ClusterElewise(std::string _name, std::vector<std::shared_ptr<Op>> _operands, std::vector<TensorSpec> _output_specs) 
    ...
    TensorShape ClusterElewise::get_loop_shape() const {
        ...
        TensorShape loop_shape = this->get_graph()->get_node(output_node1_name)->get_output_spec(0).get_shape();

        for (auto const& output_node_name : this->get_graph()->get_output_nodes()) {
            if (loop_shape.element_count() != this->get_graph()->get_node(output_node_name)->get_output_spec(0).get_shape().element_count()) {
                LOG(WARNING) << "Mismatched output shape in cluster " << this->get_name() << " " 
                << output_node1_name << " " << loop_shape.to_string() << " " 
                << output_node_name << " " << this->get_graph()->get_node(output_node_name)->get_output_spec(0).get_shape().to_string();
            }
        }

        bool valid = true;
        for (auto const &node_name : this->get_graph()->get_output_nodes()) {
            if (loop_shape.element_count() < this->get_graph()->get_node(node_name)->get_output_spec(0).get_shape().element_count()) {
                loop_shape = this->get_graph()->get_node(node_name)->get_output_spec(0).get_shape();
            }
        }
    ...
    Schedule ClusterElewise::construct_schedule(LocalityTier::Tier tier) {
        EXPECT_TRUE(tier == LocalityTier::kT0, "Elewise cluster must have t0 locality tier");
        return ScheduleFactory::get_schedule(tier, this->get_loop_shape());
    }

    void ClusterElewise::trace_symbolic_index() {
        // trace symbolic index
        this->get_graph()->reverse_topology_order([&](std::shared_ptr<const Op> node, std::shared_ptr<const Op> next_node) -> void {
    ...
    // std::string emit_code_use_new_code_emitter(const Graph *graph, const std::string &loop_key, const std::string &loop_stride) {
    //     using CUDAEmitter = mononn_engine::codegen::CUDAEmitter;
    ...
    //             {IndexSymbols::linear_index, mononn_engine::helpers::string_format("((%s) + (%s * {ilp_index_id}))", loop_key.c_str(), loop_stride.c_str())},
    ...
    //     std::function<void(const Op *node)> codegen_dfs = [&](const Op *node) -> void {
    ...
    void ClusterElewise::setup_codegen() {
        ...
        for (auto const &node_name : this->get_graph()->get_node_list()) {
            ...
            node->instantiate_concrete_index({{"linear_index", loop1_key}}, loop1.get_loop_stride());

            if (this->is_instruction_parallelized()) {
                node->instantiate_ilp_concrete_index({{"linear_index", loop1_key}}, loop1.get_loop_ilp_stride(), loop1.get_loop_stride());
            }
        }

        int loop_element_count = this->get_schedule().get_inner_loop().get_loop_shape().element_count();
        int total_thread_count = this->cuda_context->cuda_runtime_context.grid_dim.XYZ() * this->cuda_context->cuda_runtime_context.block_dim.XYZ();

        if (this->is_instruction_parallelized() && 
            loop_element_count % (this->get_instruction_parallel_factor() * total_thread_count) != 0) {
            
            int step2_loop_element_count = loop_element_count % (this->get_instruction_parallel_factor() * total_thread_count);
            int step1_loop_element_count = loop_element_count - step2_loop_element_count;
            ...
            step1_loop = step1_loop.instruction_level_parallelism(this->get_instruction_parallel_factor());
            Loop step2_loop = Loop(
                    TensorShape(std::vector<int> {loop_element_count}),
                    existing_loop.get_loop_key(),
                    step2_loop_init,
                    existing_loop.get_loop_stride());
    ...
    std::string ClusterElewise::generate_cluster_code() const {
        ...
        if ((int)this->get_graph()->get_node_list().size() == 1) {
            ...
            if (node->get_type() == OpType::constant || node->get_type() == OpType::parameter) {
                return "";
            }

            LOG(WARNING) << "Single node cluster, cluster name, " << node->get_cluster_name() << " node name, " << node->get_name();
        }
        ...
        { // generate ilp loop
            ...
            std::string loop_stride = this->is_instruction_parallelized() ? loop.get_loop_ilp_stride() : loop.get_loop_stride();
            ...
            if (CUDAEmitter::should_use_new_code_emitter(this->graph.get())) {
                ss << CUDAEmitter::emit_code_use_new_code_emitter(this->graph.get(), loop.get_loop_key().get_name(), loop.get_loop_stride());
            } else {
                this->get_graph()->wave_front_order([&](std::shared_ptr<const Op> node, std::shared_ptr<const Op> next_node) -> void {
                    ...
                    if (node->need_generate_with_index()) {
                        ...
                        std::string prefetch_pred = mononn_engine::helpers::string_format("(%s) + (%s) < (%s)", loop1_key.c_str(), loop_stride.c_str(), loop_boundary.c_str());
                        impl->propagate_attributes({{OpAttribute::prefetch_predicate, prefetch_pred}});
                    }
        ...
        if (this->is_instruction_parallelized() && this->get_schedule().num_loop_schedule() > 1) {
            ...
            if (CUDAEmitter::should_use_new_code_emitter(this->graph.get())) {
                 CUDAEmitter::emit_code_use_new_code_emitter(this->graph.get(), loop.get_loop_key().get_name(), loop_stride);
            } else {
                this->get_graph()->wave_front_order([&](std::shared_ptr<const Op> node, std::shared_ptr<const Op> next_node) -> void {
                    std::string node_name = node->get_name();
                    std::shared_ptr<OpImplBase> impl = node->get_implementation();
            ...
            this->graph->set_instruction_parallel_factor(this->get_instruction_parallel_factor());
    ...
    bool ClusterElewise::is_cluster_elewise() const {
        return true;
    }
