...
namespace op {
    ...
    OpType ReduceWindow::get_type() const {
        return OpType::reduce_window;
    }

    std::vector<std::shared_ptr<OpImpl>> ReduceWindow::generate_candidate_implementation(std::shared_ptr<CUDAContext> context, Tier tier) const {
    ...
    const Scalar &ReduceWindow::get_init_value() const {
    ...
    void ReduceWindow::set_reduction_functor_generator(const ReductionFunctorGenerator * _reduction_functor_generator) {
        this->reduction_functor_generator = _reduction_functor_generator;
    }

    const ReductionFunctorGenerator *ReduceWindow::get_reduction_functor_generator() const {
    ...
    std::vector<std::vector<int>> ReduceWindow::get_window_positions() const {
        ...
        for (int idx = 1; idx < this->filter_size.size(); ++idx) {
            ...
            positions = mononn_engine::helpers::cartesian_join<std::vector<int>, int, std::vector<int>>
                (positions, new_pos, [](const std::vector<int> &vec, const int &pos) 
                -> std::vector<int> {
                auto res = vec;
                res.push_back(pos);
                return res;
