...
namespace op {
    ...
    public:
        ...
        Op(std::string _name, std::vector<std::shared_ptr<Op>> _operands, std::vector<TensorSpec> _output_specs)
        ...
        void set_symbolic_index(const std::vector<SymbolicIndexStamp> &_symbolic_index_list);
        ...
        virtual std::vector<std::shared_ptr<OpImpl>> generate_candidate_implementation(std::shared_ptr<CUDAContext> context, Tier tier) const = 0;
        ...
        virtual void replace_operand(std::string old_operand_name, std::shared_ptr<Op> new_operand);
        ...
        void import_attributes(std::unordered_map<std::string, std::string> _attributes);
        ...
        void instantiate_concrete_index(const std::map<std::string, std::string> &params, const std::string &loop_stride);
        void instantiate_ilp_concrete_index(const std::map<std::string, std::string> &params, const std::string &loop_stride, const std::string &ilp_stride);
    ...
    protected:
        ...
        // virtual int vectorization_propagate_to_input_impl(int vec_len, int input_id) const;
        // virtual int vectorization_propagate_to_output_impl(int vec_len) const;
    private:
        std::string name;
