...
namespace op {
    ...
    OpType Output::get_type() const {
        return OpType::output;
    }

    std::vector<std::shared_ptr<OpImplBase>>
    Output::generate_candidate_implementation(std::shared_ptr<CUDAContext> context, Tier tier) const {
    ...
    int Output::get_output_number() const {
