...
namespace op {
    ...
    OpType Parameter::get_type() const {
        return OpType::parameter;
    }

    std::vector<std::shared_ptr<OpImpl>> Parameter::generate_candidate_implementation(std::shared_ptr<CUDAContext> context, Tier tier) const {
        Tensor output = this->get_output_tensor(0);

        std::vector<std::shared_ptr<OpImpl>> impls = ParameterImpl::get_available_implementations(context, output);
    ...
    int Parameter::get_parameter_number() const {
