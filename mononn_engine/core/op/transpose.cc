...
namespace op {
    ...
    std::vector<int> Transpose::get_permute() const {
        return this->permute;
    }
    ...
    OpType Transpose::get_type() const {
        return OpType::transpose;
    }

    std::vector<std::shared_ptr<OpImpl>> Transpose::generate_candidate_implementation(std::shared_ptr<CUDAContext> context, Tier tier) const {
        ...
        std::vector<std::shared_ptr<OpImpl>> impls = TransposeImpl::get_available_implementations(context, input_spec, output);
