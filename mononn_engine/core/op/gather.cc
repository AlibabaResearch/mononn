...
OpType Gather::get_type() const { return OpType::gather; }

std::vector<std::shared_ptr<OpImpl>> Gather::generate_candidate_implementation(
  ...
  std::vector<std::shared_ptr<OpImpl>> impls =
      GatherImpl::get_available_implementations(context, input_spec, output);
...
std::vector<int> Gather::get_offset_dims() const { return this->offset_dims; }
