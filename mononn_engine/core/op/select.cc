...
namespace op {
    ...
    OpType Select::get_type() const {
        return OpType::select;
    }

    std::vector<std::shared_ptr<OpImpl>> Select::generate_candidate_implementation(std::shared_ptr<CUDAContext> context, Tier tier) const {
        ...
        std::vector<std::shared_ptr<OpImpl>> impls = ElewiseBinaryImpl::get_available_implementations_for_select(
        context,
        input_spec,
        output,
        pred
