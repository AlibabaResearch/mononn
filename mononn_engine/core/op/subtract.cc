...
namespace op {
    ...
    OpType Subtract::get_type() const {
        return OpType::subtract;
    }

    std::vector<std::shared_ptr<OpImpl>> Subtract::generate_candidate_implementation(std::shared_ptr<CUDAContext> context, Tier tier) const {
