...
OpType Reduce::get_type() const { return OpType::reduce; }

std::vector<std::shared_ptr<OpImpl>> Reduce::generate_candidate_implementation(
  ...
  // input_spec.operand = Tensor(operand->get_name(),
  // operand->get_output_spec(0)); input_spec.init_value =
  // this->get_init_value().get_value(); input_spec.reducer =
  // this->get_reducer();

  for (int operand_id = 0; operand_id < this->get_operand_count() / 2;
       ++operand_id) {
    input_spec.operands.emplace_back(
        this->get_operand(operand_id)->get_name(),
        this->get_operand(operand_id)->get_output_spec(0));
  }
  ...
  for (int operand_idx = this->get_operand_count() / 2;
       operand_idx < this->get_operand_count(); ++operand_idx) {
    init_type_list.push_back(this->get_operand(operand_idx)
                                 ->get_output_spec(0)
                                 .get_dtype()
                                 .get_primitive_type());
    // init_value_list.push_back(input_spec.operands[param_idx].get_name());
    init_value_list.push_back(
        input_spec.init_value
            .get_values_in_list()[operand_idx - this->get_operand_count() / 2]);
  }

  input_spec.reduce_accum =
      Scalar(output.get_name() + "_accum", init_type_list, init_value_list);

  std::vector<std::shared_ptr<OpImpl>> impls =
      ReduceImpl::get_available_implementations(context, input_spec, output);
...
void Reduce::set_dimension(int _dim) {
  if (_dim != this->operands[0]->get_output_spec(0).rank() - 1) {
    LOG(FATAL) << "Unsupported reduction in operator: " << this->get_name()
               << ". Reduction dimension " << _dim << " reduction operand "
               << this->operands[0]->get_name() << " shape "
               << this->operands[0]->get_output_spec(0).to_string();
  }

  this->dimension = _dim;
}

int Reduce::get_dimension() const { return this->dimension; }

void Reduce::set_init_value(const Scalar& _init_value) {
  this->init_value = _init_value;
}

const Scalar& Reduce::get_init_value() const { return this->init_value; }
...
bool Reduce::need_pre_inner_loop_generation() const { return true; }

std::string Reduce::generate_pre_inner_loop() const {
  Scalar reduce_accum =
      this->get_implementation()->as<ReduceImpl>()->get_reduce_accum();
  return reduce_accum.get_definition_with_value() + "\n";
}

void Reduce::set_reduction_functor_generator(
