...
namespace op {
    ...
    OpType Negate::get_type() const {
        return OpType::negate;
    }

    std::vector<std::shared_ptr<OpImpl>> Negate::generate_candidate_implementation(std::shared_ptr<CUDAContext> context, Tier tier) const {
