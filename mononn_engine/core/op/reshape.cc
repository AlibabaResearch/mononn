...
namespace op {
    ...
    OpType Reshape::get_type() const {
        return OpType::reshape;
    }

    std::vector<std::shared_ptr<OpImpl>> Reshape::generate_candidate_implementation(std::shared_ptr<CUDAContext> context, Tier tier) const {
        ...
        std::vector<std::shared_ptr<OpImpl>> impls = ReshapeImpl::get_available_implementations(context, input_spec, output);
