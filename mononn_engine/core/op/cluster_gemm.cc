...
std::string ClusterGemm::generate_cluster_code() const {
  EXPECT_TRUE(this->get_graph()->get_node_list().size() == 1,
              "GEMM cluster should have only one node");
  std::stringstream ss;
  ...
  for (auto const& operand : node->get_operands()) {
    std::string operand_buffer_name =
        BufferManager::get_buffer_name(operand->get_name());
    ss << mononn_engine::helpers::string_format(
        "void *%s = reinterpret_cast<void *>(%s);\n",
...
ClusterType ClusterGemm::get_cluster_type() const { return ClusterType::Gemm; }
}  // namespace op
}  // namespace core
}  // namespace mononn_engine