...
namespace op {
    ...
    OpType Rsqrt::get_type() const {
        return OpType::rsqrt;
    }

    std::vector<std::shared_ptr<OpImpl>> Rsqrt::generate_candidate_implementation(std::shared_ptr<CUDAContext> context, Tier tier) const {
