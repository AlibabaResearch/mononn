...
std::string ClusterOp::generate_cluster_invocation() const {
  ...
  for (auto const& operand : this->get_operands()) {
    ...
    invocation.add_arg(mononn_engine::helpers::string_format(
        "static_cast<void *>(%s)",
        BufferManager::get_buffer_name(operand_name).c_str()));
  }
  ...
  if (this->get_graph()->get_output_nodes().size() > 1) {
    if (TF_MONONN_ENABLED) {
      for (int idx = 0; idx < this->get_output_specs_count(); ++idx) {
        std::string arg_name = mononn_engine::helpers::string_format(
            "get_tuple_element_%s_%d", this->get_name().c_str(), idx);
      ...
      for (int idx = 0; idx < this->get_graph()->get_output_nodes().size();
           ++idx) {
    ...
    if (output_node->get_output_specs_count() > 1) {
      if (output_node->get_type() != OpType::reduce) {
        LOG(FATAL) << "Reduce node expected, got node " << output_node_name
                   << " with type " << output_node->get_type().to_string();
      }

      for (int tuple_idx = 0; tuple_idx < output_node->get_output_specs_count();
           ++tuple_idx) {
        invocation.add_arg(mononn_engine::helpers::string_format(
            "get_tuple_element_%s_%d", this->get_name().c_str(), tuple_idx));
      }
    } else {
...
void ClusterOp::set_instruction_parallel_factor(int _ilp_factor) {
  ...
  for (int idx = 0; idx < n_loop_schedule; ++idx) {
    if (idx == n_loop_schedule - 1) {
      Loop loop = this->get_schedule()
                      .get_loop_schedule(idx)
                      .instruction_level_parallelism(_ilp_factor);
      new_schedule.add_loop_schedule(loop);
    } else {
      new_schedule.add_loop_schedule(
          this->get_schedule().get_loop_schedule(idx));
    }
  }
...
void ClusterOp::trace_symbolic_index() { LOG(FATAL) << "Unimplemented"; }
...
void ClusterOp::append_sub_cluster_tags(
...
void ClusterOp::append_sub_cluster_types(
...
void ClusterOp::initialize_smem_manager() { LOG(FATAL) << "Unimplemented"; }

SmemManager* ClusterOp::get_smem_manager() { return this->smem_manager.get(); }

int ClusterOp::get_horizontal_fusion_count() const {
  return this->horizontal_fusion_count;
}

void ClusterOp::set_horizontal_fusion_count(
    const int& _horizontal_fusion_count) {
