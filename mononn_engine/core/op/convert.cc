...
namespace op {
    ...
    OpType Convert::get_type() const {
        return OpType::convert;
    }

    std::vector<std::shared_ptr<OpImpl>> Convert::generate_candidate_implementation(std::shared_ptr<CUDAContext> context, Tier tier) const {
