...
namespace op {
    ...
    void Op::set_symbolic_index(const std::vector<SymbolicIndexStamp> &_symbolic_index_list) {
    ...
    std::shared_ptr<OpImpl> Op::get_implementation() const {
        return this->impl;
    }
    ...
    int Op::get_operand_count() const {
        return (int)this->operands.size();
    }

    void Op::replace_operand(std::string old_operand_name, std::shared_ptr<Op> new_operand) {
        for (auto &operand : this->operands) {
            if (operand->get_name() == old_operand_name) {
                if (operand->get_output_specs() != new_operand->get_output_specs()) {
                    LOG(FATAL) << "Tensor spec between operand " << old_operand_name << " and " << new_operand->get_name() << " for node " << this->get_name();
                }
        ...
        LOG(FATAL) << "Operand " << old_operand_name << " not found in node " << this->get_name();
    }
    ...
    int Op::get_output_specs_count() const {
        return this->output_specs.size();
    }
    ...
    ClusterType Op::get_cluster_type() const {
        return this->cluster_type;
    }

    void Op::set_cluster_id(int _cluster_id) {
        this->cluster_id = _cluster_id;
    }

    int Op::get_cluster_id() const {
        return this->cluster_id;
    }

    std::string Op::get_cluster_name() const {
        return this->cluster_type.to_string() + std::string("_") + std::to_string(this->cluster_id); 
    }

    bool Op::is_elewise() const {
        if (this->get_type() != OpType::reduce && 
            this->get_type() != OpType::custom_call &&
            this->get_type() != OpType::convolution) return true;
        else return false;
    }

    bool Op::is_gemm() const {
        return false;
    }

    bool Op::is_conv() const {
        return false;
    }

    bool Op::is_elewise_unary() const {
        return false;
    }

    bool Op::is_elewise_binary() const {
        return false;
    }

    bool Op::is_tuple() const {
        return false;
    }
    ...
    bool Op::is_cluster_elewise() const {
        return false;
    }

    bool Op::is_cluster_reduce() const {
        return false;
    }
...
//        if (value == "") LOG(FATAL) << "Empty attribute of key " << key;
    ...
    std::string Op::get_attribute(const std::string &key) const {
        if (!this->has_attribute(key)) LOG(FATAL) << "Node " << this->get_name() << " do not have attribute " << key;

        return this->attributes.at(key);
    }

    void Op::del_attribute(const std::string &key) {
    ...
    void Op::import_attributes(std::unordered_map<std::string, std::string> _attributes) {
    ...
    void Op::set_read_from_global_memory() {
        this->read_from_global_memory = true;
    }
    ...
    void Op::set_write_to_global_memory() {
        this->write_to_global_memory = true;
    }
    ...
    bool Op::need_generate_with_index() const {
        ...
        if (this->get_type() == OpType::gather ||
            this->get_type() == OpType::iota ||
            this->get_type() == OpType::concatenate) return true;

        if (this->get_type() == OpType::custom_call ||
            this->get_type() == OpType::get_tuple_element) return false;

        // Write to global memory should not generate with index
        // For example: nodes write to global memory in clusters should not generate with index.
    ...
    void Op::set_hlo_text(std::string text) {
        this->hlo_dumped_text = text;
    }

    std::string Op::get_hlo_text() const {
        return this->hlo_dumped_text;
    }
    ...
    void Op::vectorize(int vec_len) {
        EXPECT_TRUE(this->get_output_specs().size() == 1, this->get_name() + " have multiple outputs.");
    ...
    void Op::assert_single_output() const {
        EXPECT_TRUE(this->get_output_specs().size() == 1, "Node " + this->get_name() + " should have exactly one output.");
    }

    // bool Op::can_vectorize_input_as(int vec_len, int input_id) const {
    //     this->assert_single_output();
    //     EXPECT_TRUE(input_id < this->get_operand_count(), "Input id out of range");

    //     if (this->get_operand(input_id)->get_output_spec(0).get_shape(-1) % vec_len != 0) return false;
    ...
    //                 "Cannot vectorize output of node " + this->get_name() + " to vector len " + std::to_string(vec_len));
    //     EXPECT_TRUE(input_id < this->get_operand_count(), "Input id out of range");
    //     return this->vectorization_propagate_to_input_impl(vec_len, input_id);
    // }
    ...
    //                 "Cannot vectorize input of node " + this->get_name() + " to vector len " + std::to_string(vec_len));
    //     return this->vectorization_propagate_to_output_impl(vec_len);
    // }

    void Op::set_value(std::string _value) {
        LOG(FATAL) << "Un implemented";
    }

    std::string Op::get_value() const {
        LOG(FATAL) << "Un implemented";
    }
    ...
    void Op::instantiate_concrete_index(const std::map<std::string, std::string> &params, const std::string &loop_stride) {
        this->impl->instantiate_concrete_index(this->symbolic_index_list, params, loop_stride);
    }

    void Op::instantiate_ilp_concrete_index(const std::map<std::string, std::string> &params, const std::string &loop_stride, const std::string &ilp_stride) {
        this->impl->instantiate_ilp_concrete_index(this->symbolic_index_list, params, loop_stride, ilp_stride);
    }
    ...
    //     new_op->impl = std::make_shared<OpImpl>(this->impl->deep_copy().release());
    ...
    //         LOG(FATAL) << "Node " << this->get_name() << "'s implementation has not been instruction parallelized yet";
    //     }

    //     if (this->get_instruction_parallel_factor() != this->impl->get_instruction_parallel_factor()) {
    //         LOG(FATAL) << "Node " << this->get_name() << " instruction parallel factor does not match";
    //     }

    //     for (int idx = 0; idx < this->ilp_factor; ++idx) {
    //         this->impl->set_ilp_traced_index(idx, this->get_ilp_traced_index(idx));
    //     }
    // }
    ...
    // int Op::vectorization_propagate_to_input_impl(int vec_len, int input_id) const {
    //     return vec_len;
    // }
