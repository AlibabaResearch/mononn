...
namespace op {
    ...
    OpType Bitcast::get_type() const {
        return OpType::bitcast;
    }

    std::vector<std::shared_ptr<OpImpl>> Bitcast::generate_candidate_implementation(std::shared_ptr<CUDAContext> context, Tier tier) const {
