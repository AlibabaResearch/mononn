...
namespace op {
    ...
    OpType CustomCall::get_type() const {
        return OpType::custom_call;
    }

    std::vector<std::shared_ptr<OpImpl>> CustomCall::generate_candidate_implementation(std::shared_ptr<CUDAContext> context, Tier tier) const {
        if (this->is_gemm()) {
            GemmImpl::InputSpec input_spec;
            input_spec.A = Tensor(this->get_operand(0)->get_name(), this->get_operand(0)->get_output_spec(0));
            input_spec.B = Tensor(this->get_operand(1)->get_name(), this->get_operand(1)->get_output_spec(0));
            if (this->get_operands().size() > 2) {
                input_spec.C = std::make_shared<Tensor>(this->get_operand(2)->get_name(), this->get_operand(2)->get_output_spec(0));
            }

            Tensor output(this->get_name(), this->get_output_spec(0));

            std::vector<std::shared_ptr<OpImpl>> impls = GemmImpl::get_available_implementations(context, input_spec, this->get_backend_config_str(), output);
            ...
            std::string output_tensor_name = EnvVar::is_true("TF_MONONN_ENABLED") ?
                this->conv_output_GET_node_name : // Output of conv is a tuple
                this->get_name();

            Tensor output(output_tensor_name, this->get_output_spec(0));

            std::vector<std::shared_ptr<OpImpl>> impls = ConvImpl::get_available_implementations(context, input_spec, this->get_backend_config_str(), output);
    ...
    std::string const CustomCall::Target::cudnn_conv_forward = "__cudnn$convForward";
}
}
}