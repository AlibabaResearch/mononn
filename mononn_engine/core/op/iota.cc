...
OpType Iota::get_type() const { return OpType::iota; }

std::vector<std::shared_ptr<OpImpl>> Iota::generate_candidate_implementation(
  ...
  std::vector<std::shared_ptr<OpImpl>> impls =
      IotaImpl::get_available_implementations(context, input_spec, output);
...
int Iota::get_iota_dimension() const { return this->iota_dimension; }
}  // namespace op
}  // namespace core
}  // namespace mononn_engine