...
namespace op {
    ...
    OpType Pad::get_type() const {
        return OpType::pad;
    }

    std::vector<std::shared_ptr<OpImpl>> Pad::generate_candidate_implementation(std::shared_ptr<CUDAContext> context, Tier tier) const {
        ...
        input_spec.padding_value = this->get_operand(1)->get_value();

        Tensor output = this->get_output_tensor(0);

        std::vector<std::shared_ptr<OpImpl>> impls = PadImpl::get_available_implementations(context, input_spec, output);
    ...
    std::vector<int> Pad::get_padding_low() const {
    ...
    std::vector<int> Pad::get_padding_high() const {
...
//        std::stringstream ss;
//        ss << this->get_output_spec(0).get_dtype().to_string() + " " + this->get_name() + ";\n";
//
//        std::string pad_index =  this->traced_index_list[0].index_before_trace;
//        std::vector<std::string> multi_index =
//                IndexTransform::offset_to_multi_index(this->get_output_spec(0).get_shape(), pad_index);
//
//        ss << this->get_implementation()->as<PadImpl>()->generate_if_statement_begin(multi_index);
//
//        return ss.str();
//    }
}
