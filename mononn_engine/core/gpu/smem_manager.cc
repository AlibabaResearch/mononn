...
namespace gpu {
    ...
    std::string SmemManager::define_root_buffer() const {
        return mononn_engine::helpers::string_format("extern __shared__ int8_t %s[];\n", this->root_buffer_ptr.c_str());
    }
    ...
    bool SmemManager::can_claim_buffer(const std::vector<size_t> &size_list_in_bytes) const {
        size_t current_size = (size_t)this->multi_buffer.get_total_size_in_bytes();

        for (auto size_in_bytes : size_list_in_bytes) {
            current_size = round_up_to(current_size, this->alignment_in_byte); // Ordering is correct, dont need to round last buffer.
    ...
    void SmemManager::claim_smem_buffer(std::string buffer_name, size_t size_in_bytes) {
        ...
        if (this->multi_buffer.get_total_size_in_bytes() > this->total_size_in_bytes) {
            LOG(FATAL) << "Current buffer size in bytes " << this->multi_buffer.get_total_size_in_bytes() << " exceeds buffer size limit " << this->total_size_in_bytes;
        }

        this->current_buffer_id += 1;
    };

    void SmemManager::claim_smem_buffer(std::string node_name, std::string buffer_name, size_t size_in_bytes) {
    ...
    std::string SmemManager::get_buffer_pointer(std::string buffer_name, std::string as_type) const {
        if (!this->buffer_id.count(buffer_name)) {
            LOG(FATAL) << "Buffer: " << buffer_name << " not found."; 
        }

        return this->multi_buffer.get_pointer_to_buffer(this->buffer_id.at(buffer_name), as_type);
    }
    ...
    void SmemManager::record_base_buffer_size(const std::string &buffer_name, size_t size_in_bytes) {
