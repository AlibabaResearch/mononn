...
namespace gpu {
    ...
    int64_t MultiBuffer::get_total_size_in_bytes() const {
        ...
        for (auto const &buffer_size : this->buffer_size_list) {
            total_size_in_bytes += round_up_to(buffer_size, (int64_t)this->alignment_in_byte);
        }

        return total_size_in_bytes;
    }

    std::string MultiBuffer::get_pointer_to_buffer(int buffer_id, std::string as_type) const {
        int64_t offset_in_bytes = 0;
        for (int idx = 0; idx < buffer_id; ++idx) {
            offset_in_bytes += round_up_to(this->buffer_size_list[idx], (int64_t)this->alignment_in_byte);
        }

        std::string ptr = mononn_engine::helpers::string_format("(&reinterpret_cast<int8_t *>(%s)[%s])",
                                           this->buffer_ptr.c_str(), std::to_string(offset_in_bytes).c_str());

        return mononn_engine::helpers::string_format("reinterpret_cast<%s>(%s)", as_type.c_str(), ptr.c_str());
    }
}
}
}