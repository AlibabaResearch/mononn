...
Element block_reduce_cub_WARP_REDUCTIONS(Element my_val) {
    ...
    std::vector<Reduction::FunctionInvocation> Reduction::get_invocations(std::shared_ptr<CUDAContext> context, const Tensor &operand, const Tier &tier, const Functor &reducer) {
        std::vector<Reduction::FunctionInvocation> invocations;
        bool vectorized_mem_access = operand.get_dtype().get_elements_per_access() > 1;
        Reduction::FunctionInvocation vector_to_scalar("to_scalar");
        if (vectorized_mem_access) {
            vector_to_scalar.add_template_arg(operand.get_dtype().get_primitive_type().to_string());
            vector_to_scalar.add_template_arg(std::to_string(operand.get_dtype().get_elements_per_access()));
        ...
        if (tier == LocalityTier::kT1) {
            ...
            if (vectorized_mem_access) warp_custom_reduce.add_arg(vector_to_scalar.to_string());
            else warp_custom_reduce.add_arg(operand.get_name());
            ...
            if (vectorized_mem_access) warp_reduce_cub.add_arg(vector_to_scalar.to_string());
            else warp_reduce_cub.add_arg(operand.get_name());
        ...
        if (tier == LocalityTier::kT2) {
            ...
            Reduction::FunctionInvocation block_reduce_cub_RAKING("block_reduce_cub_RAKING");
            Reduction::FunctionInvocation block_reduce_cub_RAKING_COMMUTATIVE_ONLY("block_reduce_cub_RAKING_COMMUTATIVE_ONLY");
            Reduction::FunctionInvocation block_reduce_cub_WARP_REDUCTIONS("block_reduce_cub_WARP_REDUCTIONS");
            ...
            for (auto &reduce_invocation : ret) {
                ...
                if (vectorized_mem_access) reduce_invocation.add_arg(vector_to_scalar.to_string());
                else reduce_invocation.add_arg(operand.get_name());
            }
