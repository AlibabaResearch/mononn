...
namespace gpu {
    ...
    std::string Functor::get_functor_name_for_op_type(OpType op_type, absl::optional<MathOp> math_op) {
    ...
    public:
        FunctorRegister(std::string name) {
            std::map<std::string, Functor> *functor_registry
                = Functor::registry();
            ...
            for (Functor::Dtype dtype : Functor::supported_types) {
                ...
                // bool operations.
                if (dtype == Dtype::from_string("bool") && name != "convert" && name != "equal" && name != "not_equal" && name != "sign") {
                    continue;
                }

                if ((name == "exponential" || name == "rsqrt" || name == "tanh" || name == "natural_log") && (dtype != Dtype::from_string("float32") && dtype != Dtype::from_string("float16"))) {
                    continue;
                }

                if ((name == "tanh" || name == "natural_log") && dtype != Dtype::from_string("float32")) {
                    continue;
                }
                ...
                for (int vec_len = 2; vec_len <= 8; vec_len <<= 1) {
                    ...
                    functor_registry->insert(std::make_pair(functor_vec.get_name(), functor_vec));
                }

//                for (int ilp_factor : Config::get()->candidate_ilp_factor) {
//                    Functor functor_ilp(name, dtype.instruction_parallelize(ilp_factor));
//                    functor_registry->insert(std::make_pair(functor_ilp.get_name(), functor_ilp));
//                }
            }
        }
    private:
    ...
    #define REGISTER_FUNCTOR(name) \
        FunctorRegister register_functor_##name(#name)
    ...
    std::string Functor::get_definition() const {
        return mononn_engine::helpers::string_format("__device__ %s %s;", this->get_functor_type().c_str(), this->get_name().c_str());
    }
    ...
    std::string Functor::get_raw_name() const {
        return this->name;
    }

    std::string Functor::get_functor_type() const {
        return mononn_engine::helpers::string_format("cutlass::%s<%s>", this->name.c_str(), this->dtype.to_string().c_str());
    }
