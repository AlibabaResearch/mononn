...
namespace core {
namespace gpu {
    // std::vector<std::string> BufferManager::variables_in_smem = std::vector<std::string>();
    std::vector<std::string> thread_local BufferManager::variables_in_global = std::vector<std::string>();
    ...
    std::string BufferManager::get_buffer_name(std::string var_name) {
        ...
        if (BufferManager::is_var_in_global(var_name)) return BufferManager::get_buffer_name_global(var_name);
    ...
    bool BufferManager::is_var_in_global(std::string var_name) {
        return std::find(BufferManager::variables_in_global.begin(), BufferManager::variables_in_global.end(), var_name) != BufferManager::variables_in_global.end();
    }
    ...
    void BufferManager::buffer_in_global(std::string var_name) {
        EXPECT_TRUE(!BufferManager::is_var_in_global(var_name), "Var " + var_name + " already exists in global buffer");
        BufferManager::variables_in_global.push_back(var_name);
    }

    void BufferManager::unbuffer_in_global(std::string var_name) {
        EXPECT_TRUE(BufferManager::is_var_in_global(var_name), "Var " + var_name + "not exists in global buffer");
        auto iter = std::find(BufferManager::variables_in_global.begin(), BufferManager::variables_in_global.end(), var_name);
