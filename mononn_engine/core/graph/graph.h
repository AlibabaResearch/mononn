...
namespace graph {
    ...
    public:
        ...
        void add_control_edge(const std::string &node_src, const std::string &node_dst);
        ...
        void replace_node(const std::vector<std::string> &node_list, std::shared_ptr<Op> new_node);

        bool has_node(const std::string &node_name) const;
        bool has_edge(const std::string &node_src, const std::string &node_dst) const;
        bool has_control_edge(const std::string &node_src, const std::string &node_dst) const;

        std::shared_ptr<Op> get_node(const std::string &node);
        std::shared_ptr<const Op> get_node(const std::string &node) const;
        std::vector<std::string> get_node(std::function<bool(const Op * op)> pred) const;

        std::shared_ptr<Edge> get_edge(const std::string &node_src, const std::string &node_dst);
        ...
        std::vector<std::shared_ptr<Edge>> get_node_output_edges(const std::string &node);
        std::vector<std::shared_ptr<Edge>> const get_node_output_edges(const std::string &node) const;
        std::vector<std::shared_ptr<ControlEdge>> get_node_output_control_edges(const std::string &node);
        std::vector<std::shared_ptr<ControlEdge>> const get_node_output_control_edges(const std::string &node) const;

        std::vector<std::shared_ptr<Edge>> get_node_input_edges(const std::string &node) const;
        std::vector<std::shared_ptr<ControlEdge>> get_node_input_control_edges(const std::string &node) const;
        ...
        void set_node_attribute(std::shared_ptr<Op> node, const std::string &key, const std::string &value);
        void set_node_attribute(const std::string &node_name, const std::string &key, const std::string &value);
        std::string get_node_attribute(std::shared_ptr<Op> node, const std::string &key) const;
        std::string get_node_attribute(const std::string &node_name, const std::string &key) const;
        bool node_has_attribute(std::shared_ptr<Op> node, const std::string &key) const;
        bool node_has_attribute(const std::string &node_name, const std::string &key) const;

        // Graph traversal algorithms
        std::unordered_set<std::string> bfs(const std::unordered_set<std::string> &start_nodes);
        std::unordered_set<std::string> bfs(const std::unordered_set<std::string> &start_nodes, std::function<void(std::shared_ptr<Op>)> func);

        std::unordered_set<std::string> post_order(const std::string &start_node);
        std::unordered_set<std::string> post_order(const std::string &start_node, std::function<void(std::shared_ptr<Op>)> func);
        ...
        void wave_front_order(std::function<void(std::shared_ptr<Op>, std::shared_ptr<Op>)> func);
        void wave_front_order(std::function<void(std::shared_ptr<const Op>, std::shared_ptr<const Op>)> func) const;
        void topology_order(std::function<void(std::shared_ptr<Op>, std::shared_ptr<Op>)> func);
        void topology_order(std::function<void(std::shared_ptr<const Op>, std::shared_ptr<const Op>)> func) const;
        void reverse_topology_order(std::function<void(std::shared_ptr<Op>, std::shared_ptr<Op>)> func);
        void reverse_topology_order(std::function<void(std::shared_ptr<const Op>, std::shared_ptr<const Op>)> func) const;
        ...
        bool topology_before(const std::string &node1, const std::string &node2) const;
        int distance(const std::string &node_src, const std::string &node_dst) const;
        bool reachable_under_constrain(const std::string &node_src, const std::string &node_dst, const OpType &op_type_constrain) const;
        ...
        std::vector<std::string> search_nodes_between_two_nodes(const std::string &node_src, const std::string &node_dst) const;

        void trace_index(const std::string &index, const std::string &node_name, std::string inverse_reduce_dimension = "");
//        void trace_ilp_index(int ilp_id, const std::string &index, const std::string &node_name, std::string inverse_reduce_dimension = "") override;

        const std::vector<SymbolicIndexStamp> &get_symbolic_index(const std::string &node_name) const;
        bool is_node_traced(const std::string &node_name) const;

        void reset_symbolic_index();

        std::vector<std::string> get_symbolic_index_before_trace(const std::string &node_name) const;
        std::vector<std::string> get_symbolic_index_after_trace(const std::string &node_name) const;

        bool is_simple_padding_scenario() const;
        bool is_dead_node(const std::string &node_name) const;

        bool remain_acyclic_after_node_merge(const std::vector<std::string> &node_list) const;

        void set_instruction_parallel_factor(int _ilp_factor);
        int get_instruction_parallel_factor() const;

        bool is_acyclic(std::vector<std::string> &nodes_in_cycle, bool include_control_edge = false) const;
    ...
    private:
        ...
        std::unordered_map<std::string, std::vector<std::shared_ptr<ControlEdge>>> control_edges;

        std::unordered_map<std::string, std::vector<SymbolicIndexStamp>> symbolic_index;
        ...
        std::unordered_map<std::pair<std::string, std::string>, int, pair_hash<std::string, std::string>> transitive_closure;

        void post_order_impl(std::string start_node, std::function<void(std::shared_ptr<Op>)> func, std::unordered_set<std::string> &visit);
        void post_order_impl(std::string start_node,
                             std::unordered_set<std::string> &visit,
                             std::function<void(std::shared_ptr<Op>)> pre_process,
                             std::function<void(std::shared_ptr<Op>)> func);

        void reverse_post_order_impl(std::string start_node,
                                     std::unordered_set<std::string> &visit,
                                     std::function<void(std::shared_ptr<Op>)> pre_process,
                                     std::function<void(std::shared_ptr<Op>)> func);
        ...
        bool remain_acyclic_after_add_node_to_cluster(std::shared_ptr<Op> node_to_add, ClusterType cluster_type, int cluster_id);
    };
}
}
}