...
namespace graph {
    ...
    const std::string &Graph::get_graph_name() const {
    ...
    void Graph::remove_node(std::shared_ptr<Op> node) {
        ...
        if (this->get_node_input_edges(node_name).size() != 0 ||
            ...
            "node input edge count: " << this->get_node_input_edges(node_name).size() << "\n" <<
            "node output edge count: " << this->get_node_output_edges(node_name).size() << "\n" << 
            "node input control edge count: " << this->get_node_input_control_edges(node_name).size() << "\n" << 
            "node output control edge count: " << this->get_node_output_control_edges(node_name).size();
        }

        this->nodes.erase(node_name);
        if (this->symbolic_index.count(node_name) > 0) this->symbolic_index.erase(node_name);
        std::vector<std::string>::iterator iter = std::find(this->input_nodes.begin(), this->input_nodes.end(), node_name);
        if (iter != this->input_nodes.end()) this->input_nodes.erase(iter);

        iter = std::find(this->extended_input_nodes.begin(), this->extended_input_nodes.end(), node_name);
        if (iter != this->extended_input_nodes.end()) this->extended_input_nodes.erase(iter);

        iter = std::find(this->output_nodes.begin(), this->output_nodes.end(), node_name);
    ...
    void Graph::add_edge(std::shared_ptr<Edge> edge) {
        EXPECT_TRUE(edge->get_src_name() != edge->get_dst_name(), "Self cycle is not allowed");
    ...
    void Graph::remove_edge(std::shared_ptr<Op> src, std::shared_ptr<Op> dst) {
        ...
        this->edges[src->get_name()].begin(),
        this->edges[src->get_name()].end(),
    ...
    void Graph::remove_edge_if(std::function<bool(std::shared_ptr<Edge> const)> pred) {
        ...
        for (auto const &[node_name, edge_list] : this->edges) {
            for (auto const &edge : edge_list) {
                if (pred(edge)) {
                    edge_to_be_removed.push_back(std::make_pair(edge->get_src_name(), edge->get_dst_name()));
                }
            }
        }
    ...
    void Graph::add_control_edge(std::shared_ptr<ControlEdge> control_edge) {
        EXPECT_TRUE(control_edge->get_src_name() != control_edge->get_dst_name(), "Self cycle is not allowed");

        if (this->nodes.find(control_edge->get_src()->get_name()) == this->nodes.end()) {
            LOG(FATAL) << "Node: " << control_edge->get_src()->get_name() << " not in graph";
        }

        if (this->nodes.find(control_edge->get_dst()->get_name()) == this->nodes.end()) {
            LOG(FATAL) << "Node: " << control_edge->get_dst()->get_name() << " not in graph";
        }

        if (this->control_edges.find(control_edge->get_src()->get_name()) == this->control_edges.end()) {
            this->control_edges[control_edge->get_src()->get_name()] = std::vector<std::shared_ptr<ControlEdge>>();
        }

        this->control_edges[control_edge->get_src()->get_name()].push_back(control_edge);
    }

    void Graph::add_control_edge(const std::string &node_src, const std::string &node_dst) {
        EXPECT_TRUE(node_src != node_dst, "Self cycle is not allowed");
    ...
    void Graph::add_control_edge(std::shared_ptr<Op> src, std::shared_ptr<Op> dst) {
        EXPECT_TRUE(src->get_name() != dst->get_name(), "Self cycle is not allowed");
        std::shared_ptr<ControlEdge> control_edge = std::make_shared<ControlEdge>(src, dst);
        this->add_control_edge(control_edge);
    }

    void Graph::remove_control_edge(std::shared_ptr<Op> src, std::shared_ptr<Op> dst) {
        if (this->control_edges.count(src->get_name()) == 0) {
            LOG(FATAL) << "Edge not found";
        }

        std::vector<std::shared_ptr<ControlEdge>>::iterator iter = std::find_if(
                this->control_edges[src->get_name()].begin(),
    ...
    void Graph::replace_node(const std::vector<std::string> &node_list, std::shared_ptr<Op> new_node) {
        LOG(WARNING) << "Deprecated, use ClusterUtil::replace_node instead";
        this->add_node(new_node);

        if (std::any_of(node_list.begin(), node_list.end(), [&](std::string const &node_name) -> bool {
        ...
        for (auto const &node_name : node_list) {
            ...
            EXPECT_TRUE(node->get_type() == OpType::cluster || node->get_type() == OpType::get_tuple_element, "Node " + node_name + " is neither cluster nor get tuple element node.");
        }

        std::function<bool(std::string, std::string)> edge_in_node_list = [&](std::string node_name_src, std::string node_name_dst) -> bool {
            return std::find(node_list.begin(), node_list.end(), node_name_src) != node_list.end() &&
            std::find(node_list.begin(), node_list.end(), node_name_dst) != node_list.end();
        };

        std::string new_node_name = new_node->get_name();
        int get_tuple_element_id = 0;

        std::function<std::string()> next_get_tuple_element_name = [&]() -> std::string {
            return "get_tuple_element_" + new_node_name + "_" + std::to_string(get_tuple_element_id++);
        };

        std::vector<std::string> get_tuple_element_node_to_be_removed;
        for (auto const &node_name : node_list) {
            if (this->get_node(node_name)->get_type() == OpType::get_tuple_element) continue;

            std::vector<std::shared_ptr<Edge>> node_input_edges = this->get_node_input_edges(node_name);
            std::vector<std::shared_ptr<Edge>> node_output_edges = this->get_node_output_edges(node_name);
            std::vector<std::shared_ptr<ControlEdge>> node_input_control_edges = this->get_node_input_control_edges(node_name);
            std::vector<std::shared_ptr<ControlEdge>> node_output_control_edges = this->get_node_output_control_edges(node_name);

            for (auto const &edge : node_input_edges) {
                this->remove_edge(edge->get_src(), edge->get_dst());

                if (edge_in_node_list(edge->get_src_name(), edge->get_dst_name())) continue;
                std::shared_ptr<Edge> new_edge = std::make_shared<Edge>(edge->get_src(), new_node);
            ...
            for (auto const &edge : node_input_control_edges) {
                ...
                if (edge_in_node_list(edge->get_src_name(), edge->get_dst_name())) continue;
                std::shared_ptr<ControlEdge> new_control_edge = std::make_shared<ControlEdge>(edge->get_src(), new_node);
            ...
            for (auto const &edge : node_output_control_edges) {
                this->remove_control_edge(edge->get_src(), edge->get_dst());
                if (edge_in_node_list(edge->get_src_name(), edge->get_dst_name())) continue;
                std::shared_ptr<ControlEdge> new_control_edge = std::make_shared<ControlEdge>(new_node, edge->get_dst());
                this->add_control_edge(new_control_edge);
            }

            if (this->get_node(node_name)->get_output_specs().size() > 1) {
                for (auto const &edge : node_output_edges) { // for each get tuple element
                    if (edge->get_dst()->get_type() != OpType::get_tuple_element) {
                        LOG(FATAL) << "Node " << node_name << " have multiple output spec but " << edge->get_dst()->get_name() << " is not get tuple element";
                    }

                    std::vector<std::shared_ptr<Edge>> old_get_tuple_element_output_edges = this->get_node_output_edges(edge->get_dst()->get_name());

                    if (std::all_of(old_get_tuple_element_output_edges.begin(), old_get_tuple_element_output_edges.end(), [&](std::shared_ptr<Edge> get_tuple_element_output_edge) -> bool {
                        return std::find(node_list.begin(), node_list.end(), get_tuple_element_output_edge->get_dst_name()) != node_list.end();
                    })) {
                        for (auto const &get_tuple_element_output_edge : old_get_tuple_element_output_edges) {
                        ...
                        // 2. first node in dependent merge with multiple output, as least one of them are output node
                        // 3. second node in dependemt merge with multiple output
                        // In all cases, new node will have multiple outputs
                        EXPECT_TRUE(new_node->get_output_specs_count() > 1, "Node " + new_node->get_name() + " should have multiple output specs");

                        std::shared_ptr<GetTupleElement> get_tuple_element = std::make_shared<GetTupleElement>(
                        ...
                        for (auto const &old_get_tuple_element_output_edge : old_get_tuple_element_output_edges) {
                            if (edge_in_node_list(old_get_tuple_element_output_edge->get_src_name(), old_get_tuple_element_output_edge->get_dst_name())) {
                                this->remove_edge(old_get_tuple_element_output_edge);
                                continue;
                            }

                            this->add_edge(get_tuple_element, old_get_tuple_element_output_edge->get_dst());
                            old_get_tuple_element_output_edge->get_dst()->replace_operand(old_get_tuple_element_output_edge->get_src()->get_name(), get_tuple_element);
                            this->remove_edge(old_get_tuple_element_output_edge);
                        }
                    }

                    this->remove_edge(edge);
                    if (std::find(node_list.begin(), node_list.end(), edge->get_dst_name()) == node_list.end()) {
                        this->remove_node(edge->get_dst());
                    }
                }
            } else {
                // all output edges are in the node list to be merged
                if (std::all_of(node_output_edges.begin(), node_output_edges.end(), [&](std::shared_ptr<Edge> edge) -> bool {
                    return std::find(node_list.begin(), node_list.end(), edge->get_dst_name()) != node_list.end();
                })) {
                    for (auto const &edge : node_output_edges) {
                        this->remove_edge(edge);
                    }
                } else {
                    // No get tuple element node is needed if two cluster is in below format:
                    // A
                    // |
                    // B
                    // All output of A goes in B and B only have one output tensor (i.e. not tuple)
                    if (new_node->get_output_specs_count() > 1) {
                        std::shared_ptr<GetTupleElement> get_tuple_element = std::make_shared<GetTupleElement>(
                            next_get_tuple_element_name(),
                            std::vector<std::shared_ptr<Op>>{new_node},
                            std::vector<TensorSpec>{this->get_node(node_name)->get_output_spec(0) });
                        get_tuple_element->set_tuple_index(get_tuple_element_id - 1);
                        this->add_node(get_tuple_element);
                        this->add_edge(new_node, get_tuple_element);
                        ...
                        for (auto const &edge : node_output_edges) {
                            EXPECT_FALSE(edge_in_node_list(edge->get_src_name(), edge->get_dst_name()), "Edge " + edge->to_string() + " should not in node list: " + mononn_engine::helpers::join(" ", node_list));
    ...
    bool Graph::has_edge(const std::string &node_src, const std::string &node_dst) const {
        if (this->edges.count(node_src) == 0) return false;
        if (std::find_if(this->edges.at(node_src).begin(), this->edges.at(node_src).end(), [&](std::shared_ptr<Edge> edge) -> bool {
            return edge->get_dst()->get_name() == node_dst;
        }) == this->edges.at(node_src).end()) return false;
        return true;
    }

    bool Graph::has_control_edge(const std::string &node_src, const std::string &node_dst) const {
        if (this->control_edges.count(node_src) == 0) return false;
        if (std::find_if(this->control_edges.at(node_src).begin(), this->control_edges.at(node_src).end(), [&](std::shared_ptr<ControlEdge> edge) -> bool {
            return edge->get_dst()->get_name() == node_dst;
        }) == this->control_edges.at(node_src).end()) return false;
        return true;
    }
    ...
    std::vector<std::string> Graph::get_node(std::function<bool(const Op *)> pred) const {
    ...
    std::shared_ptr<Edge> Graph::get_edge(const std::string &node_src, const std::string &node_dst) {
    ...
    std::vector<std::string> Graph::get_node_list_by_type(const OpType &op_type) const {
        std::vector<std::string> node_list;
    ...
    std::vector<std::shared_ptr<Edge>> Graph::get_node_output_edges(const std::string &node) {
        return this->edges[node];
    }

    std::vector<std::shared_ptr<Edge>> const Graph::get_node_output_edges(const std::string &node) const {
        return this->edges.at(node);
    }

    std::vector<std::shared_ptr<ControlEdge>> Graph::get_node_output_control_edges(const std::string &node) {
    ...
    std::vector<std::shared_ptr<ControlEdge>> const Graph::get_node_output_control_edges(const std::string &node) const {
        if (this->control_edges.find(node) == this->control_edges.end()) {
            return std::vector<std::shared_ptr<ControlEdge>>();
        }
    ...
    std::vector<std::shared_ptr<Edge>> Graph::get_node_input_edges(const std::string &node) const {
    ...
    std::vector<std::shared_ptr<ControlEdge>> Graph::get_node_input_control_edges(const std::string &node) const {
    ...
    void Graph::mark_as_input_node(const std::string &node) {
        ...
        if (std::find(this->input_nodes.begin(), this->input_nodes.end(), node) != this->input_nodes.end()) {
            return;
        }
    ...
    void Graph::sort_input_nodes() {
        std::sort(this->input_nodes.begin(), this->input_nodes.end(), [&](std::string const &node1, std::string const &node2) -> bool {
    ...
    void Graph::align_input_nodes() {
        for (int idx = 0; idx < (int)this->input_nodes.size(); ++idx) {
            this->get_node(this->input_nodes[idx])->as<Parameter>()->set_parameter_number(idx);
        }
    }
    ...
    void Graph::mark_as_extended_input_node(const std::string &node) {
        ...
        if (std::find(this->input_nodes.begin(), this->input_nodes.end(), node) != this->input_nodes.end()) {
            LOG(FATAL) << "Node " << node << " already been marked as input node, cannot be marked as extended input node.";
        }

        if (std::find(this->extended_input_nodes.begin(), this->extended_input_nodes.end(), node) != this->extended_input_nodes.end()) {
            return;
        }
...
//    void Graph::replace_input_node(std::string old_node_name, std::string new_node_name) {
...
//        }
//
//        LOG(FATAL) << "Cannot replace input node " << old_node_name << " with " << new_node_name << " not in graph";
//    }
    ...
    int Graph::get_input_node_count() const {
        return this->input_nodes.size();
    }

    bool Graph::is_input_node(const std::string &node_name) const {
        return std::find(this->input_nodes.begin(), this->input_nodes.end(), node_name) != this->input_nodes.end();
    }
    ...
    void Graph::mark_as_output_node(const std::string &node) {
        ...
        if (std::find(this->output_nodes.begin(), this->output_nodes.end(), node) != this->output_nodes.end()) {
            return;
        }
    ...
    int Graph::get_output_node_count() const {
        return this->output_nodes.size();
    }

    int Graph::find_output_node_idx(const std::string &node_name) const {
        std::vector<std::string>::const_iterator iter = std::find(this->output_nodes.begin(), this->output_nodes.end(), node_name);
    ...
    bool Graph::is_output_node(const std::string &node_name) const {
        return std::find(this->output_nodes.begin(), this->output_nodes.end(), node_name) != this->output_nodes.end();
    }
    ...
    void Graph::verify() {
        ...
        for (auto const &node : start_nodes) {
            if (in_degree[node] > 0) {
                LOG(FATAL) << "Node " << node << " have in degree of " << in_degree[node] << " cannot be set as input node";
            }
        }
        ...
        for (auto const &degree : in_degree) {
            if (degree.second == 0) {
                ...
                if (std::find(this->input_nodes.begin(), this->input_nodes.end(), degree.first) == this->input_nodes.end()
                    && std::find(this->extended_input_nodes.begin(), this->extended_input_nodes.end(), degree.first) == this->input_nodes.end()) {
    ...
    void Graph::set_node_attribute(std::shared_ptr<Op> node, const std::string &key, const std::string &value) {
        node->set_attribute(key, value);
    }

    void Graph::set_node_attribute(const std::string &node_name, const std::string &key, const std::string &value) {
        this->get_node(node_name)->set_attribute(key, value);
    }

    std::string Graph::get_node_attribute(std::shared_ptr<Op> node, const std::string &key) const {
        return node->get_attribute(key);
    }

    std::string Graph::get_node_attribute(const std::string &node_name, const std::string &key) const {
        return this->get_node(node_name)->get_attribute(key);
    }

    bool Graph::node_has_attribute(std::shared_ptr<Op> node, const std::string &key) const {
        return node->has_attribute(key);
    }

    bool Graph::node_has_attribute(const std::string &node_name, const std::string &key) const {
        return this->get_node(node_name)->has_attribute(key);
    }

    std::unordered_set<std::string> Graph::bfs(const std::unordered_set<std::string> &start_nodes) {
        return this->bfs(start_nodes, [](std::shared_ptr<Op> node) {
            // do nothing
        });
    }

    std::unordered_set<std::string> Graph::bfs(const std::unordered_set<std::string> &start_nodes, std::function<void(std::shared_ptr<Op>)> func) {
    ...
    std::unordered_set<std::string> Graph::post_order(const std::string &start_node) {
        return this->post_order(start_node, [](std::shared_ptr<Op> func) {
            // do nothing
        });
    }

    std::unordered_set<std::string> Graph::post_order(const std::string &start_node, std::function<void(std::shared_ptr<Op>)> func) {
        std::unordered_set<std::string> visit;
    ...
    std::vector<std::string> Graph::traverse_in_topology_order() const {
        ...
        while (!q.empty()) {
            ...
            if (step > steps[node_name]) {
                ...
                for (auto const &control_edge : this->get_node_output_control_edges(node_name)) {
        ...
        if (!dead_nodes.empty()) {
            LOG(FATAL) << "Found uneliminated dead node:" << mononn_engine::helpers::join(", ", dead_nodes);
        }

        std::sort(traverse_order.begin(), traverse_order.end(), [](std::pair<std::string, int> const &a, std::pair<std::string, int> const &b) -> bool {
    ...
    void Graph::wave_front_order(std::function<void(std::shared_ptr<Op>, std::shared_ptr<Op>)> func) {
        ...
        for (int idx = 0; idx < (int)traverse_order.size(); ++idx) {
            if (idx == (int)traverse_order.size() - 1) {
                ...
                func(this->get_node(traverse_order[idx]), this->get_node(traverse_order[idx + 1]));
            }
        }
    }

    void Graph::wave_front_order(std::function<void(std::shared_ptr<const Op>, std::shared_ptr<const Op>)> func) const {
        std::vector<std::string> traverse_order = this->traverse_in_topology_order();
    ...
    void Graph::topology_order(std::function<void(std::shared_ptr<Op>, std::shared_ptr<Op>)> func) {
        this->wave_front_order(func);
    }

    void Graph::topology_order(std::function<void(std::shared_ptr<const Op>, std::shared_ptr<const Op>)> func) const {
        this->wave_front_order(func);
    }

    void Graph::reverse_topology_order(std::function<void(std::shared_ptr<Op>, std::shared_ptr<Op>)> func) {
        std::vector<std::string> traverse_order = this->traverse_in_topology_order();
    ...
    void Graph::reverse_topology_order(std::function<void(std::shared_ptr<const Op>, std::shared_ptr<const Op>)> func) const {
        std::vector<std::string> traverse_order = this->traverse_in_topology_order();
    ...
    std::string Graph::summary() const {
        ...
        graph_summary << "Total edges: " << std::accumulate(this->edges.begin(), this->edges.end(), 0,
        [](int total_count, const std::pair<std::string, std::vector<std::shared_ptr<Edge>>> &edges) -> int {
            return total_count + (int)edges.second.size();
        }) << "\n";

        graph_summary << "Input nodes: " << std::accumulate(this->input_nodes.begin(), this->input_nodes.end(), std::string(""),
        [](std::string s, std::string r) -> std::string {
            return s + " " + r;
        }) << "\n";

        graph_summary << "Extended input nodes: " << std::accumulate(this->extended_input_nodes.begin(), this->extended_input_nodes.end(), std::string(""),
        [](std::string s, std::string r) -> std::string {
            return s + " " + r;
        }) << "\n";

        graph_summary << "Output nodes: " << std::accumulate(this->output_nodes.begin(), this->output_nodes.end(), std::string(""),
        [](std::string s, std::string r) -> std::string {
            return s + " " + r;
        }) << "\n";
...
//        }
    ...
    void Graph::post_order_impl(std::string start_node, std::function<void(std::shared_ptr<Op>)> func, std::unordered_set<std::string> &visit) {
        this->post_order_impl(start_node, visit, [&](std::shared_ptr<Op> op) -> void {}, func);
    }
    ...
    void Graph::post_order_impl(std::string start_node,
        ...
        for (auto const &edge : this->get_node_output_edges(start_node)) {
            this->post_order_impl(edge->get_src()->get_name(), visit, pre_process, func);
        }

        func(this->get_node(start_node));
    }

    void Graph::reverse_post_order_impl(std::string start_node,
                                std::unordered_set<std::string> &visit,
                                std::function<void(std::shared_ptr<Op>)> pre_process,
                                std::function<void(std::shared_ptr<Op>)> func) {
        if (visit.find(start_node) != visit.end()) return;
        ...
        for (auto const &operand : this->get_node(start_node)->get_operands()) {
            this->reverse_post_order_impl(operand->get_name(), visit, pre_process, func);
        }
    ...
    void Graph::clustering() {
        if (!std::all_of(this->nodes.begin(), this->nodes.end(), [](std::pair<std::string, std::shared_ptr<Op>> const &node) -> bool {
        ...
        LOG(INFO) << "  Gemm epilogue cluster: " << this->gemm_epilogue_cluster_id - 1;
        LOG(INFO) << "  Conv epilogue cluster: " << this->conv_epilogue_cluster_id - 1;
    ...
    void Graph::build_transitive_closure(bool including_control_dependency) {
        ...
        for (auto const &[node_name, edge_list] : this->edges) {
            for (auto const &edge : edge_list) {
//                this->transitive_closure[std::make_pair(edge->get_src_name(), edge->get_dst_name())] = 1;
    ...
    //                this->transitive_closure[std::make_pair(edge->get_src_name(), edge->get_dst_name())] = 1;
        ...
        for (int i = 0; i < cnt; ++i) {
            for (int j = 0; j < cnt; ++j) {
                if (dis[i][j] != -1) {
                    this->transitive_closure[std::make_pair(id_to_node_name[i], id_to_node_name[j])] = dis[i][j];
                }
            }
        }
...
//                    if (this->transitive_closure.find(i_k) != this->transitive_closure.end() &&
//                        this->transitive_closure.find(k_j) != this->transitive_closure.end()) {
//
//                        if (this->transitive_closure.find(i_j) == this->transitive_closure.end()) {
//                            this->transitive_closure[i_j] = this->transitive_closure[i_k] + this->transitive_closure[k_j];
//                        } else {
//                            this->transitive_closure[i_j] = std::min(this->transitive_closure[i_k] + this->transitive_closure[k_j],
...
//        }
    ...
    void Graph::trace_index(const std::string &index, const std::string &node_name, std::string inverse_reduce_dimension) {
        ...
        if (this->node_has_attribute(node_name, OpAttribute::sub_cluster_tag)) {
            sub_cluster_tag = this->get_node(node_name)->get_attribute(OpAttribute::sub_cluster_tag);
        }

        std::function<void(std::string, std::string)> trace_index_dfs = [&](std::string node_name, std::string traced_by) -> void {
            if (this->node_has_attribute(node_name, OpAttribute::sub_cluster_tag) && 
                    this->get_node_attribute(node_name, OpAttribute::sub_cluster_tag) != sub_cluster_tag) {
                return;
            }
            ...
            if (node->get_type() == OpType::gather) {
                ...
                LOG_ONCE(WARNING, __already_logged_gather, "May need take care of predictive propagation for gather op");

                // already traced
                if (std::find_if(this->symbolic_index[node_name].begin(), this->symbolic_index[node_name].end(), [&](SymbolicIndexStamp const &index_trace_stamp) -> bool {
                    return index_trace_stamp.index_before_trace == index_before_trace && index_trace_stamp.index_after_trace == index_after_trace;
                }) != this->symbolic_index[node_name].end()) {
                    return;
                }

                SymbolicIndexStamp index_trace_stamp { index_before_trace, index_after_trace, traced_by, pred_before_trace, pred_after_trace};
            ...
            if (node->get_type() == OpType::reduce) {
                index_tracer.trace_reduce(std::static_pointer_cast<Reduce>(node), inverse_reduce_dimension);
                std::string index_after_trace = index_tracer.get_index();
                std::string pred_after_trace = index_tracer.get_predictive();
            ...
            if (node->get_type() == OpType::concatenate) {
                // already traced
                if (std::find_if(this->symbolic_index[node_name].begin(), this->symbolic_index[node_name].end(), [&](SymbolicIndexStamp const &index_trace_stamp) -> bool {
                    return index_trace_stamp.index_before_trace == index_before_trace;
                }) != this->symbolic_index[node_name].end()) {
                    return;
                }

                SymbolicIndexStamp index_trace_stamp { index_before_trace, index_before_trace /*Use same after trace index for concat node*/, traced_by, pred_before_trace, pred_before_trace /*Use save after trace pred*/ };
                this->symbolic_index[node_name].push_back(index_trace_stamp);
                ...
                for (int idx = 0; idx < node->get_operand_count(); ++idx) {
                    ...
                    index_tracer.trace_concatenate(std::static_pointer_cast<Concatenate>(node), idx);
            ...
            if (node->get_type() == OpType::dynamic_update_slice) {
                ...
                index_tracer.trace_dynamic_update_slice_operand(std::static_pointer_cast<DynamicUpdateSlice>(node));

                SymbolicIndexStamp index_trace_stamp_operand { index_before_trace, index_tracer.get_index(), traced_by, pred_before_trace, index_tracer.get_predictive()};
                ...
                index_tracer.trace_dynamic_update_slice_update(std::static_pointer_cast<DynamicUpdateSlice>(node));
                trace_index_dfs(node->get_operand(1)->get_name(), node_name);
            ...
            for (int idx = 0; idx < node->get_operand_count(); ++idx) {
                ...
                if (node->get_type() == OpType::reduce_window && idx >= node->get_operand_count() / 2) {
                    index_tracer.set_index("0");
                    index_tracer.set_pred("true");
                }
        ...
        for (auto const &[traced_node_name, traced_index_list] : this->symbolic_index) {
            // Theoratically any node can be traced by multiple index.
            if (traced_index_list.size() > 1) {
            //     EXPECT_TRUE(this->get_node(traced_node_name)->get_type() == OpType::constant ||
            //         this->get_node(traced_node_name)->get_type() == OpType::parameter,
            //         "Node " + traced_node_name + " have multiple traced index however it's neither constant nor parameter");
            ...
            //                 "Node " + traced_node_name + " have multiple index but it's upstream traced node " + its.traced_by + " is not slice");
                        if (next_node->get_type() == OpType::slice) {
                            next_node->get_implementation()->add_operand_reuse_mask(traced_node_name, traced_node_name + "_reuse_" + next_node->get_name());
                        }    
                    }
                }
            }
...
//    void Graph::trace_ilp_index(int ilp_id, const std::string &index, const std::string &node_name, std::string inverse_reduce_dimension) {
//        IndexTracer index_tracer(index);
//
...
//
//            if (node->get_type() == OpType::gather) {
//                LOG_ONCE(WARNING, __already_logged__gather, "May need take care of predictive propagation for gather op");
//                index_tracer.trace_gather_operand_ilp(std::static_pointer_cast<Gather>(node), ilp_id);
//                std::string index_after_trace = index_tracer.get_index();
//                std::string pred_after_trace = index_tracer.get_predictive();
//
//                // already traced
//                if (std::find_if(this->ilp_traced_index[node_name][ilp_id].begin(), this->ilp_traced_index[node_name][ilp_id].end(), [&](IndexTraceStamp const &index_trace_stamp) -> bool {
//                    return index_trace_stamp.index_before_trace == index_before_trace && index_trace_stamp.index_after_trace == index_after_trace;
//                }) != this->ilp_traced_index[node_name][ilp_id].end()) {
//                    return;
//                }
//
//                IndexTraceStamp index_trace_stamp { index_before_trace, index_after_trace, traced_by, pred_before_trace, pred_after_trace};
//
...
//                }
//
//                IndexTraceStamp index_trace_stamp { index_before_trace, index_before_trace /*Use same after trace index for concat node*/, traced_by, pred_before_trace, pred_before_trace /*Use save after trace pred*/ };
//                this->ilp_traced_index[node_name][ilp_id].push_back(index_trace_stamp);
//
//                for (int idx = 0; idx < node->get_operand_count(); ++idx) {
//                    index_tracer.set_index(index_before_trace);
...
//        trace_index_dfs(node_name, "");
//
//        for (auto const &[traced_node_name, traced_index_list_for_each_ilp_id] : this->ilp_traced_index) {
//            auto const &traced_index_list = traced_index_list_for_each_ilp_id[ilp_id];
//
//            if (traced_index_list.size() > 1) {
//                EXPECT_TRUE(this->get_node(traced_node_name)->get_type() == OpType::constant ||
//                            this->get_node(traced_node_name)->get_type() == OpType::parameter,
...
//                        LOG(WARNING) << "Operand reuse mask may need further refactor";
//                    }
//
//                    next_node->get_implementation()->add_operand_reuse_mask(traced_node_name, traced_node_name + "_reuse_" + next_node->get_name());
//                }
//            }
//
//            this->get_node(traced_node_name)->set_ilp_traced_index(ilp_id, traced_index_list);
//            this->get_node(traced_node_name)->propagate_ilp_index_to_implementation();
//        }
//    }

    const std::vector<SymbolicIndexStamp> &Graph::get_symbolic_index(const std::string &node_name) const {
        EXPECT_TRUE(this->symbolic_index.count(node_name) > 0, "Index " + node_name + " not traced");
    ...
    void Graph::reset_symbolic_index() {
        this->symbolic_index.clear();
    }

    std::vector<std::string> Graph::get_symbolic_index_before_trace(const std::string &node_name) const {
        if (this->symbolic_index.find(node_name) == this->symbolic_index.end()) {
    ...
    std::vector<std::string> Graph::get_symbolic_index_after_trace(const std::string &node_name) const {
        if (this->symbolic_index.find(node_name) == this->symbolic_index.end()) {
            LOG(FATAL) << "Node " << node_name << " do not have traced index";
        }
    ...
    bool Graph::is_simple_padding_scenario() const {
        EXPECT_TRUE(!this->transitive_closure.empty(), "Should build transitive closure first");
        ...
        for (auto const &node_name : pad_list) {
            ...
            if (std::find(this->output_nodes.begin(), this->output_nodes.end(), node_name)
        ...
        if (pad_list.size() > 1) {
            for (int idx = 1; idx < pad_list.size(); ++idx) {
                if (this->symbolic_index.at(pad_list[idx]) != this->symbolic_index.at(pad_list[0])) return false;
                std::shared_ptr<const Pad> pad1 = std::static_pointer_cast<const Pad>(this->get_node(pad_list[0]));
                std::shared_ptr<const Pad> pad2 = std::static_pointer_cast<const Pad>(this->get_node(pad_list[idx]));
    ...
    bool Graph::is_dead_node(const std::string &node_name) const {
        EXPECT_TRUE(!this->transitive_closure.empty() , "Transitive closure not built yet");

        if (this->get_node(node_name)->get_type() == OpType::global_sync) return false;

        for (auto const &output_node_name : this->get_output_nodes()) {
            if (node_name == output_node_name ||
                this->topology_before(node_name, output_node_name)) return false;
        }

        return true;
    }

    bool Graph::remain_acyclic_after_node_merge(const std::vector<std::string> &node_list) const {
        std::unordered_set<std::string> visit;

        std::function<bool(std::string)> dfs_search = [&](std::string node_name) -> bool {
            ...
            for (auto const &edge : this->get_node_output_edges(node_name)) {
                ...
                if (std::find(node_list.begin(), node_list.end(), dst_name) != node_list.end()) {
                    if (std::find(node_list.begin(), node_list.end(), node_name) != node_list.end()) {
                        continue;
                    } else {
                        return false;
                    }
    ...
    int Graph::get_instruction_parallel_factor() const {
        return this->ilp_factor;
    }

    bool Graph::is_acyclic(std::vector<std::string> &nodes_in_cycle, bool include_control_edge) const {
        std::vector<std::string> node_list;

        std::unordered_set<std::string> visit;
        std::unordered_set<std::string> visit_in_path;

        std::function<bool(std::string node_name)> dfs = [&](std::string node_name) -> bool {
    ...
    void Graph::verify_is_acyclic(bool include_control_edge) const {
        ...
        if (!this->is_acyclic(node_in_cycle, true)) {
            LOG(FATAL) << "Acyclic check failed, detected cycle in graph: " << mononn_engine::helpers::join("->", node_in_cycle);
        }
    }
    ...
    //         new_graph->nodes[node_name] = std::shared_ptr<Op>(node->deep_copy().release());
    //     }

    //     for (auto const &[src_node_name, edges] : this->edges) {
    //         for (auto const& edge : edges) {
    //             std::shared_ptr<Edge> new_edge = std::make_shared<Edge>(edge->get_src(), edge->get_dst());
    //             new_edge->set_sync(edge->get_sync());
    ...
    //             new_graph->add_control_edge(control_edge->get_src_name(), control_edge->get_dst_name());
    //         }
    //     }
    ...
    bool Graph::topology_before(const std::string &node1, const std::string &node2) const {
        ...
        if (this->transitive_closure.find(std::make_pair(node1, node2)) != this->transitive_closure.end()) return true;
        else return false;
    }

    int Graph::distance(const std::string &node_src, const std::string &node_dst) const {
    ...
    bool Graph::reachable_under_constrain(const std::string &node_src, const std::string &node_dst, const OpType &op_type_constrain) const {
    ...
    std::vector<std::string> Graph::search_downstream_node_under_constrain(
            ...
            std::function<bool(const Op *, std::vector<std::string> const &)> dst_constrain,
        ...
        std::function<bool(std::string)> dfs_search = [&](std::string node_name) -> bool {
            ...
            for (auto const &edge : this->get_node_output_edges(node_name)) {
                if (dst_constrain(edge->get_dst().get(), nodes_in_path)) {
                    ...
                    if (return_nodes_in_path) result.insert(result.end(), nodes_in_path.begin(), nodes_in_path.end());
    ...
    std::vector<std::string> Graph::search_downstream_node_under_constrain(
            ...
            std::function<bool(const Op *, std::vector<std::string> const &)> dst_constrain,
            bool return_nodes_in_path) const {
        return this->search_downstream_node_under_constrain(node_src, 
            [&](const Op *op) -> bool { return true; },
            dst_constrain,
            return_nodes_in_path
        );
    }

    std::vector<std::string> Graph::search_nodes_between_two_nodes(const std::string &node_src, const std::string &node_dst) const {
        std::vector<std::string> result;
        for (auto const &node_name : this->get_node_list()) {
            std::vector<std::shared_ptr<Edge>> const &input_edges = this->edges.at(node_src);

            if (std::find_if(input_edges.begin(), input_edges.end(), [&](std::shared_ptr<Edge> const edge) -> bool {
                return edge->get_dst()->get_name() == node_name;
            }) != input_edges.end()) {
                std::vector<std::shared_ptr<Edge>> const &output_edges = this->edges.at(node_name);

                if (std::find_if(output_edges.begin(), output_edges.end(), [&](std::shared_ptr<Edge> const edge) -> bool {
                    return edge->get_dst()->get_name() == node_dst;
                }) != output_edges.end()) {
    ...
    void Graph::cluster_reduce(std::shared_ptr<Op> node) {
        std::function<void(std::shared_ptr<Op>)> cluster_epilogue = [&](std::shared_ptr<Op> n) -> void {
            if (n->get_type() == OpType::broadcast) return;
            if (n->get_cluster_type() != ClusterType::None) return;
            if (!n->is_elewise()) return;

            if (!this->remain_acyclic_after_add_node_to_cluster(n, ClusterType::Reduce, this->reduce_cluster_id)) return;
        ...
        std::function<void(std::shared_ptr<Op>)> cluster_prologue = [&](std::shared_ptr<Op> n) -> void {
            if (n->get_cluster_type() != ClusterType::None) return;
            if (this->transitive_closure.find(std::make_pair(node->get_name(), n->get_name())) != this->transitive_closure.end()) {
    ...
    void Graph::cluster_elewise(std::shared_ptr<Op> node) {
        std::function<void(std::shared_ptr<Op>)> cluster = [&](std::shared_ptr<Op> n) -> void {
            if (n->get_cluster_type() != ClusterType::None) return;
            if (!n->is_elewise()) return;
            if (!this->remain_acyclic_after_add_node_to_cluster(n, ClusterType::Elewise, this->elewise_cluster_id)) return;
    ...
    void Graph::cluster_gemm_epilogue(std::shared_ptr<Op> node) {
        std::function<bool(std::shared_ptr<Op>, ClusterType, int)> can_cluster = [&](std::shared_ptr<Op> n, ClusterType cluster_type, int cluster_id) -> bool {
        ...
        std::function<void(std::shared_ptr<Op>, ClusterType, int)> do_cluster = [&](std::shared_ptr<Op> n, ClusterType cluster_type, int cluster_id) -> void {
        ...
        std::shared_ptr<Op> epilogue_node = this->edges[node->get_name()][0]->get_dst();

        if (epilogue_node->get_cluster_type() != ClusterType::Elewise) return;

        if (!can_cluster(epilogue_node, epilogue_node->get_cluster_type(), epilogue_node->get_cluster_id())) return;

        do_cluster(epilogue_node, epilogue_node->get_cluster_type(), epilogue_node->get_cluster_id());
    }
    ...
    bool Graph::remain_acyclic_after_add_node_to_cluster(std::shared_ptr<Op> node_to_add, ClusterType cluster_type, int cluster_id) {
        std::function<bool(std::shared_ptr<Op>, std::unordered_set<std::string>&)> dfs = [&](std::shared_ptr<Op> node, std::unordered_set<std::string> &visit) -> bool {
            if (visit.find(node->get_name()) != visit.end()) return true;
            visit.insert(node->get_name());

            for (auto const &edge : this->edges[node->get_name()]) {
                if (edge->get_dst()->get_name() == node_to_add->get_name() || 
                    (edge->get_dst()->get_cluster_type() == cluster_type && edge->get_dst()->get_cluster_id() == cluster_id)) return false;
        ...
        for (auto const &[node_name, node] : this->nodes) {
            ...
            for (auto const &edge : this->edges[node_name]) {
                ...
                if (edge->get_dst()->get_cluster_type() == cluster_type && edge->get_dst()->get_cluster_id() == cluster_id) continue;
