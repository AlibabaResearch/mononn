...
namespace graph {
    ...
    void ClusteredGraph::from_graph(std::shared_ptr<Graph> graph) {
        ...
        for (auto const &node_name : graph->get_node_list()) {
            ...
            if (cluster_name_visit.find(cluster_name) != cluster_name_visit.end()) { continue; }
            ...
            std::function<void(std::shared_ptr<Op>)> find_input_output_nodes = [&](std::shared_ptr<Op> dfs_node) -> void {
                ...
                if (graph->get_node_output_edges(dfs_node->get_name()).size() == 0) {
                    ...
                    for (auto const &edge : graph->get_node_output_edges(dfs_node->get_name())) {
                ...
                for (auto const &operand : dfs_node->get_operands()) find_input_output_nodes(operand);
                for (auto const &edge : graph->get_node_output_edges(dfs_node->get_name())) find_input_output_nodes(edge->get_dst());
            }; 

            find_input_output_nodes(node);

            std::shared_ptr<Graph> cluster_subgraph = graph->get_subgraph(visit, cluster_input_nodes, cluster_output_nodes);
            ...
            for (auto const &node_name : cluster_input_nodes) {
                ...
                for (auto const &operand : node->get_operands()) {
                    if (operand->get_cluster_name() != cluster_name) operands_set.insert(operand->get_name());
                }
            }
            ...
            if (cluster_type == ClusterType::Elewise) {
                cluster_op = std::static_pointer_cast<ClusterOp>(std::make_shared<ClusterElewise>(cluster_name, operands_list, output_specs));
            } else if (cluster_type == ClusterType::Reduce) {
                cluster_op = std::static_pointer_cast<ClusterOp>(std::make_shared<ClusterReduce>(cluster_name, operands_list, output_specs));
            } else if (cluster_type == ClusterType::Gemm) {
                cluster_op = std::static_pointer_cast<ClusterOp>(std::make_shared<ClusterGemm>(cluster_name, operands_list, output_specs));
            } else if (cluster_type == ClusterType::Conv) {
                cluster_op = std::static_pointer_cast<ClusterOp>(std::make_shared<ClusterConv>(cluster_name, operands_list, output_specs));
            } else if (cluster_type == ClusterType::GemmEpilogue) {
                cluster_op = std::static_pointer_cast<ClusterOp>(std::make_shared<ClusterGemmEpilogue>(cluster_name, operands_list, output_specs));
            } else if (cluster_type == ClusterType::ConvEpilogue) {
                cluster_op = std::static_pointer_cast<ClusterOp>(std::make_shared<ClusterConvEpilogue>(cluster_name, operands_list, output_specs));
            } else {
        ...
        for (auto const &node_name : graph->get_node_list()) {
            ...
            for (auto const &edge : graph->get_node_output_edges(node_name)) {
                if (cluster_name != edge->get_dst()->get_cluster_name()) {
                    edge_list.push_back(std::make_pair(cluster_name, edge->get_dst()->get_cluster_name()));
                }
            }
        }
        ...
        for (auto const &edge : edge_list) {
            this->add_edge(std::make_shared<Edge>(this->get_node(edge.first), this->get_node(edge.second)));
        }
    ...
    std::vector<std::shared_ptr<Edge>> ClusteredGraph::get_node_output_edges(std::string node) {
        return this->edges[node];
    }

     void ClusteredGraph::wave_front_order(std::function<void(std::shared_ptr<ClusterOp>)> func) {
    ...
    std::string ClusteredGraph::summary() {
        ...
        for (auto const &clustered_node_name : this->get_node_list()) {
            ...
            for (auto const &node_name : this->get_node(clustered_node_name)->get_graph()->get_node_list()) {
                ss << " " << node_name;
            }
