...
namespace semantic {
    ...
    void FunctionInvocation::add_arg(std::string arg) {
        this->args.push_back(arg);
    }

    FunctionInvocation FunctionInvocation::get_ilp_function_invocation(int ilp_id) const {
        ...
        for (auto const &arg : this->args) {
            ilp_invocation.add_arg(mononn_engine::helpers::get_node_ilp_name(arg, ilp_id));
        }
    ...
    std::string FunctionInvocation::to_string() const {
        return mononn_engine::helpers::string_format("%s%s%s",
                this->get_func_name().c_str(), 
                this->template_args_to_string().c_str(),
                this->args_to_string().c_str());
    }
}
}
}