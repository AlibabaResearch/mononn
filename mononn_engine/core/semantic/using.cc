...
namespace semantic {
    ...
    std::string Using::get_name() const {
        return this->name;
    }
    ...
    std::string Using::to_string() const {
        ...
        if (this->is_typename()) ss << " " << "typename";
    ...
    bool Using::is_typename() const {
        return !this->with_type.empty();
    }

    void Using::with(std::string _type) {
        this->with_type.push_back(_type);
    }
}
}
}