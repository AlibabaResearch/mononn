...
namespace semantic {
    ...
    std::string CUDAInvocation::cuda_config_to_string() const {
        return mononn_engine::helpers::string_format("<<<dim3(%d, %d, %d), dim3(%d, %d, %d), %d, %s>>>",
            this->grid.x, this->grid.y, this->grid.z, this->block.x, this->block.y, this->block.z,
    ...
    std::string CUDAInvocation::to_string() const {
        return mononn_engine::helpers::string_format("%s%s%s%s",
                this->function_invocation.get_func_name().c_str(), 
