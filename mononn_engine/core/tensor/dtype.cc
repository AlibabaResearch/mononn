...
bool Dtype::is_vectorized() const { return this->vectorized; }

Dtype Dtype::vectorize(int N) const {
  return Dtype(this->type, this->get_elements_per_access() * N, this->bytes * N,
               this->vectorized || N > 1  // already vectorized or N > 1,
  );
}

Dtype Dtype::vectorize_to_bits(int bits) const {
  if (bits % this->size_in_bits() != 0)
    LOG(FATAL) << "Cannot vectorize: " << this->to_string() << " to " << bits
               << " bits";
...
std::string Dtype::to_string() const {
  if (this->vectorized) {
    return mononn_engine::helpers::string_format(
        "cutlass::AlignedArray<%s, %d>", this->type.c_str(),
        this->elements_per_access);
  } else {
    if (this->elements_per_access != 1)
      LOG(FATAL) << "Unvectorized element per access: "
                 << this->elements_per_access;
...
Dtype Dtype::get_primitive_type() const {
  return Dtype(this->type, 1, this->bytes / this->elements_per_access, false);
}

int Dtype::get_elements_per_access() const { return this->elements_per_access; }

int Dtype::size_in_bytes() const { return this->bytes; }

int Dtype::size_in_bits() const { return this->bytes * 8; }

Dtype Dtype::to_cutlass_type() const {
  return Dtype(this->type == "half" ? "cutlass::half_t" : this->type,
               this->elements_per_access, this->bytes, this->vectorized);
}
...
bool Dtype::operator==(const Dtype& rhs) const {
  return this->type == rhs.type &&
         ...
         this->bytes == rhs.bytes && this->vectorized == rhs.vectorized;
}

bool Dtype::operator!=(const Dtype& rhs) const { return !(*this == rhs); }
}  // namespace tensor
}  // namespace core
}  // namespace mononn_engine