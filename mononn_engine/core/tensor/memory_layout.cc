...
namespace tensor {
    ...
    int MemoryLayout::rank() const {
        return this->perm.size();
    }

    std::vector<int> MemoryLayout::get() const {
        return this->perm;
    }
    ...
    std::vector<int> MemoryLayout::get_layout() const {
        return this->get();
    }

    int MemoryLayout::get_layout(int index) const {
    ...
    MemoryLayout MemoryLayout::permute(std::vector<int> _perm) const {
        ...
        if (mask != ((1 << _perm.size()) - 1)) LOG(FATAL) << "Input is not permutation " << mononn_engine::helpers::to_string(_perm);
