...
namespace tensor {
    ...
    Scalar::Scalar(const std::string &_name, const std::vector<Dtype> &_dtype, const std::vector<std::string> &_value)
        : name(_name), dtype(_dtype), value(_value) {

        if (this->dtype.size() != this->value.size()) {
            LOG(FATAL) << "Tuple element number does not match: " << this->dtype.size() << " vs " << this->value.size();
        }
    }

    std::string Scalar::get_definition() const {
        return mononn_engine::helpers::string_format("%s %s;", this->get_type_string().c_str(), this->get_name().c_str());
    }
    ...
    std::string Scalar::get_definition_with_value() const {
        ...
        return mononn_engine::helpers::string_format("%s %s = %s;", this->get_type_string().c_str(), this->get_name().c_str(), this->get_value().c_str());
    }
    
    std::string Scalar::get_name() const {
        return this->name;
    }
    ...
    std::string Scalar::get_type_string() const {
        if (this->is_tuple()) {
            ...
            std::string type_string = "cuda::std::tuple<" + mononn_engine::helpers::join(", ", type_list) + ">";
    ...
    bool Scalar::is_tuple() const {
        return this->dtype.size() > 1;
    }
