...
namespace core {
namespace tensor {
int TensorShape::rank() const { return this->shape.size(); }
...
const std::vector<int>& TensorShape::get_shape() const { return this->shape; }
...
TensorShape TensorShape::concat_on_dim(const TensorShape& rhs, int dim) const {
  ...
  for (int idx = 0; idx < this->rank(); ++idx) {
    if (this->get_shape(idx) != rhs.get_shape(idx) && idx != dim)
      LOG(FATAL) << "Unable to concat_dim two shapes";
...
TensorShape TensorShape::slice_dim(int start, int end) const {
  ...
  if (start < 0 || start > this->rank())
    LOG(FATAL) << "Index " << start << " out of range";
  if (end < 0 || end > this->rank())
    LOG(FATAL) << "Index " << end << " out of range";
...
TensorShape TensorShape::permute(std::vector<int> perm) const {
  ...
  if (mask != ((1 << perm.size()) - 1))
    LOG(FATAL) << "Input is not permutation "
               << mononn_engine::helpers::to_string(perm);
