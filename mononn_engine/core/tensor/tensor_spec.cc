...
namespace core {
namespace tensor {
    TensorSpec::TensorSpec(Dtype _dtype, TensorShape _tensor_shape, MemoryLayout _memory_layout)
        : dtype(_dtype), tensor_shape(_tensor_shape), memory_layout(_memory_layout) {}
    ...
    Dtype TensorSpec::get_dtype() const {
        return this->dtype;
    }

    TensorShape TensorSpec::get_shape() const {
    ...
    MemoryLayout TensorSpec::get_layout() const {
    ...
    int TensorSpec::rank() const {
        EXPECT_TRUE(this->tensor_shape.rank() == this->memory_layout.rank(), "Rank does not match.");

        return this->tensor_shape.rank();
    }

    TensorSpec TensorSpec::flatten() const {
        return TensorSpec(this->get_dtype(), this->get_shape().flatten(), this->get_layout().flatten());
    }

    TensorSpec TensorSpec::concat(const TensorSpec &rhs) const {
        return TensorSpec(this->get_dtype(), this->get_shape().concat(rhs.get_shape()), this->get_layout().concat(rhs.get_layout()));
    }

    TensorSpec TensorSpec::slice_dim(int start, int end) const {
        return TensorSpec(
            this->get_dtype(),
            this->get_shape().slice_dim(start, end),
            this->get_layout().slice_dim(start, end)
        );
    }

    TensorSpec TensorSpec::reduce_dim(int index) const {
        return TensorSpec(this->get_dtype(), this->get_shape().reduce_dim(index), this->get_layout().reduce_dim(index));
    }
    ...
    TensorSpec TensorSpec::reshape(std::vector<int> to_shape) const {
        ...
        while (idx1 < (int)curr_shape.size() && idx2 < (int)to_shape.size()) {
            ...
            if (acc1 < acc2) {
                while (acc1 < acc2) {
                    ...
                    if (this->memory_layout.get_layout(idx1 + 1) != this->memory_layout.get_layout(idx1) - 1) LOG(FATAL) << "Failed";
        ...
        if (idx1 != (int)curr_shape.size() || idx2 != to_shape.size()) LOG(FATAL) << "Failed";

        if (new_shape.get_shape().size() != new_layout.size()) LOG(FATAL) << "Failed on transformation, new shape size " << new_shape.get_shape().size() << " new layout size " << new_layout.size();

        return TensorSpec(this->get_dtype(), new_shape, MemoryLayout(new_layout).normalize());
    }
        ...
        while (idx1 < (int)curr_shape.size() && idx2 < (int)to_shape.size()) {
            ...
            if (acc1 < acc2) {
                while (acc1 < acc2) {
                    ...
                    if (this->memory_layout.get_layout(idx1 + 1) != this->memory_layout.get_layout(idx1) - 1) return false;
    ...
    TensorSpec TensorSpec::vectorize(int vec_len) const {
        EXPECT_TRUE(this->get_shape(-1) % vec_len == 0,
                    "Cannot vectorize tensor with shape " + this->get_shape().to_string() + " to len " + std::to_string(vec_len) + " vector");
    ...
    TensorShape TensorSpec::get_tensor_shape_with_ordered_memory_layout() const {
        ...
        for (int idx = 0; idx < this->rank(); ++idx) {
            new_shape[this->rank() - this->memory_layout.get(idx) - 1] = this->tensor_shape.get_shape(idx);
        }
