...
void IndexTracer::trace(const std::shared_ptr<const Op>& op) {
  if (op->get_type() == OpType::broadcast) {
    ...
    LOG(FATAL) << "DynamicUpdateSlice operator should be traced explicitly for "
                  "operand or indices";
  } else if (op->get_type() == OpType::gather) {
    LOG(FATAL)
        << "Gather operator should be traced explicitly for operand or indices";
...
void IndexTracer::set_index(std::string _index) { this->index = _index; }

void IndexTracer::set_pred(std::string _pred) { this->pred = _pred; }

std::string IndexTracer::get_index() const { return this->index; }

std::string IndexTracer::get_predictive() const { return this->pred; }
...
void IndexTracer::trace_broadcast(const std::shared_ptr<const Broadcast>& op) {
  ...
  if (!__already_logged) {
    ...
    LOG(INFO) << "Trace broadcast can perform additional optimizations for "
                 "lesser index calculation";
  }
  ...
  TensorShape shape_before_broadcast =
      op->get_operand(0)->get_output_spec(0).get_shape();

  std::vector<std::string> per_dim_index =
      IndexTransform::offset_to_multi_index(shape, this->index);

  EXPECT_TRUE(
      broadcast_dims.size() == shape_before_broadcast.rank() ||
          (broadcast_dims.size() == 0 &&
           shape_before_broadcast.element_count() ==
               1 /*scalar to tensor broadcast*/),
      "Rank not match for node " + op->get_name() +
          ". Broadcast dimension size " +
          std::to_string(broadcast_dims.size()) +
          " shape before broadcast: " + shape_before_broadcast.to_string());
  ...
  if (broadcast_dims.empty()) {
    ...
    this->index = IndexTransform::multi_index_to_offset(
        shape_before_broadcast, per_dim_index_before_broadcast);
  }
}

void IndexTracer::trace_dynamic_slice(
    const std::shared_ptr<const DynamicSlice>& op) {
  ...
  if (op->get_operand(0)->get_output_spec(0).rank() !=
      start_indices_list.size()) {
    LOG(FATAL) << "Rank mismatch "
               << op->get_operand(0)->get_output_spec(0).rank() << " vs "
               << start_indices_list.size();
  }

  if (op->get_operand(0)->get_output_spec(0).rank() !=
      dynamic_slice_sizes.size()) {
    LOG(FATAL) << "Rank mismatch "
               << op->get_operand(0)->get_output_spec(0).rank() << " vs "
               << dynamic_slice_sizes.size();
  }
  ...
  TensorShape shape = op->get_output_spec(0).get_shape();
  TensorShape shape_before_dynamic_slice =
      op->get_operand(0)->get_output_spec(0).get_shape();
  std::vector<std::string> per_dim_index =
      IndexTransform::offset_to_multi_index(shape, this->index);

  for (int idx = 0; idx < per_dim_index.size(); ++idx) {
    per_dim_index[idx] = mononn_engine::helpers::string_format(
        "clamp(%s{ilp_variable_suffix}, 0, %d) + (%s)",
        start_indices_list[idx].c_str(),
        shape_before_dynamic_slice.get_shape(idx) - dynamic_slice_sizes[idx],
        per_dim_index[idx].c_str());
  }

  this->index = IndexTransform::multi_index_to_offset(
      shape_before_dynamic_slice, per_dim_index);
}

void IndexTracer::trace_dynamic_update_slice_operand(
  ...
  if (op->get_operand(1)->get_output_spec(0).rank() !=
      start_indices_list.size()) {
    LOG(FATAL) << "Rank mismatch "
               << op->get_operand(1)->get_output_spec(0).rank() << " vs "
               << start_indices_list.size();
  }
  ...
  for (int idx = 0; idx < start_indices_list.size(); ++idx) {
    per_dim_index[idx] = mononn_engine::helpers::string_format(
        "(%s) - clamp(%s{ilp_variable_suffix}, 0, %d)",
        per_dim_index[idx].c_str(), start_indices_list[idx].c_str(),
        shape.get_shape(idx) - update_shape.get_shape(idx));
    // Predicate here is opposite to trace_dynamic_update_slice_update
    this->pred = mononn_engine::helpers::string_format(
        "(%s) && (!(%s >= 0) && (%s < %d))", this->pred.c_str(),
        per_dim_index[idx].c_str(), per_dim_index[idx].c_str(),
        update_shape.get_shape(idx));
  }
}

void IndexTracer::trace_dynamic_update_slice_update(
    const std::shared_ptr<const DynamicUpdateSlice>& op) {
  std::vector<std::string> start_indices_list;
  for (int idx = 2; idx < op->get_operand_count(); ++idx) {
    start_indices_list.push_back(op->get_operand(idx)->get_name());
  ...
  this->index =
      IndexTransform::multi_index_to_offset(update_shape, per_dim_index);
}

void IndexTracer::trace_gather_operand(
    const std::shared_ptr<const Gather>& op) {
  ...
  TensorShape shape = op->get_output_spec(0).get_shape();
  TensorShape shape_before_gather =
      op->get_operand(0)->get_output_spec(0).get_shape();
  TensorShape start_indices_shape =
      op->get_operand(1)->get_output_spec(0).get_shape();
  ...
  for (int idx = 0; idx < shape.rank(); ++idx) {
    if (std::find(offset_dims.begin(), offset_dims.end(), idx) ==
        offset_dims.end()) {
  ...
  this->index =
      IndexTransform::multi_index_to_offset(shape_before_gather, operand_index);
}

void IndexTracer::trace_gather_operand_ilp(
    const std::shared_ptr<const Gather>& op, int ilp_id) {
  int index_vector_dim = op->get_index_vector_dim();
  std::vector<int> offset_dims = op->get_offset_dims();
  std::vector<int> slice_sizes = op->get_slice_sizes();
  std::vector<int> collapsed_slice_dims = op->get_collapsed_slice_dims();
  ...
  std::vector<std::string> operand_index;
  operand_index.push_back(mononn_engine::helpers::get_node_ilp_name(
      op->get_operand(1)->get_name(), ilp_id));
...
void IndexTracer::trace_gather_indices(
    const std::shared_ptr<const Gather>& op) {
  int index_vector_dim = op->get_index_vector_dim();
  std::vector<int> offset_dims = op->get_offset_dims();
  std::vector<int> slice_sizes = op->get_slice_sizes();
  ...
  this->index = IndexTransform::multi_index_to_offset(start_indices_shape,
                                                      batch_dim_index);
}
...
void IndexTracer::trace_pad(const std::shared_ptr<const Pad>& op) {
  ...
  for (int idx = 0; idx < (int)per_dim_index.size(); ++idx) {
    per_dim_index_before_pad[idx] = mononn_engine::helpers::string_format(
        "(%s - %s)", per_dim_index[idx].c_str(),
        std::to_string(padding_low[idx]).c_str());

    if (padding_low[idx] != 0) {
      this->pred = mononn_engine::helpers::string_format(
          "(%s) && (%s >= %d /*Pad dim %d*/)", this->pred.c_str(),
          per_dim_index[idx].c_str(), padding_low[idx], idx);
    }

    if (padding_high[idx] != 0) {
      this->pred = mononn_engine::helpers::string_format(
          "(%s) && (%s < %d /*Pad dim %d*/)", this->pred.c_str(),
          per_dim_index[idx].c_str(), shape.get_shape(idx) - padding_high[idx],
          idx);
    }
  }

  this->index = IndexTransform::multi_index_to_offset(shape_before_pad,
                                                      per_dim_index_before_pad);
}
...
void IndexTracer::trace_transpose(const std::shared_ptr<const Transpose>& op) {
  ...
  TensorShape shape = op->get_output_spec(0).get_shape();
  TensorShape shape_before_transpose =
      op->get_operand(0)->get_output_spec(0).get_shape();

  if (op->need_smem()) {
    LOG(WARNING) << "Transpose" << op->get_name()
                 << " cannot be efficiently handled with simple index mapping";
  }
  ...
  EXPECT_TRUE(perm.size() == per_dim_index_before_transpose.size(),
              "Rank mismatch");

  for (int idx = 0; idx < (int)perm.size(); ++idx) {
    // per_dim_index_before_transpose[perm[idx]] = per_dim_index[idx];
    per_dim_index_before_transpose[idx] = per_dim_index[perm[idx]];
  }

  this->index = IndexTransform::multi_index_to_offset(
      shape_before_transpose, per_dim_index_before_transpose);
}
...
void IndexTracer::trace_slice(const std::shared_ptr<const Slice>& op) {
  ...
  TensorShape shape = op->get_output_spec(0).get_shape();
  TensorShape shape_before_slice =
      op->get_operand(0)->get_output_spec(0).get_shape();
  ...
  for (int idx = 0; idx < (int)per_dim_index_before_slice.size(); ++idx) {
    per_dim_index_before_slice[idx] = mononn_engine::helpers::string_format(
        "(%s + %s)", per_dim_index[idx].c_str(),
        std::to_string(slice_starts[idx]).c_str());
  }

  this->index = IndexTransform::multi_index_to_offset(
      shape_before_slice, per_dim_index_before_slice);
}

void IndexTracer::trace_reduce(const std::shared_ptr<const Reduce>& op,
  ...
  this->index = mononn_engine::helpers::string_format(
      "((%s * %s) + %s)", this->index.c_str(),
      std::to_string(op->get_operand(0)->get_output_spec(0).get_shape(-1))
          .c_str(),
      inverse_reduce_dim.c_str());
}

void IndexTracer::trace_reduce_window(
    const std::shared_ptr<const ReduceWindow>& op) {
  TensorShape shape = op->get_output_spec(0).get_shape();
  TensorShape shape_before_reduce_window =
      op->get_operand(0)->get_output_spec(0).get_shape();
  ...
  for (int idx = 0; idx < per_dim_index.size(); ++idx) {
    if (filter_size[idx] != 1) {
      ...
      per_dim_index[idx] = mononn_engine::helpers::string_format(
          "((0 - %d) + (%s * %d) + {window_position_%d})", padding_low[idx],
          per_dim_index[idx].c_str(), stride[idx], idx);
    }

    if (padding_low[idx] != 0) {
      // index * stride - padding low >= 0
      this->pred = mononn_engine::helpers::string_format(
          "(%s) && (%s >= 0)", this->pred.c_str(), per_dim_index[idx].c_str());
    }

    if (padding_high[idx] != 0) {
      // index * stride + window <= size + padding high
      this->pred = mononn_engine::helpers::string_format(
          "(%s) && (%s < %d)", this->pred.c_str(), per_dim_index[idx].c_str(),
          shape_before_reduce_window.get_shape(idx));
    }
  }

  this->index = IndexTransform::multi_index_to_offset(
      shape_before_reduce_window, per_dim_index);
}

void IndexTracer::trace_concatenate(
    const std::shared_ptr<const Concatenate>& op, int operand_id) {
  ...
  for (int idx = 0; idx < op->get_operand_count(); ++idx) {
    operand_shapes.push_back(
        op->get_operand(idx)->get_output_spec(0).get_shape());
  }
  ...
  if (operand_id != 0) {
    for (int shape_id = 0; shape_id < operand_id; ++shape_id) {
      left_shape =
          left_shape.concat_on_dim(operand_shapes[shape_id], dimension);
    }

    branch_prediction = mononn_engine::helpers::string_format(
        "(%s) >= (%d)", per_dim_index[dimension].c_str(),
        left_shape.get_shape(dimension));
  }

  if (operand_id != op->get_operand_count() - 1) {
    if (!branch_prediction.empty()) branch_prediction += " && ";

    TensorShape left_shape_plus_one_shape =
        left_shape.concat_on_dim(operand_shapes[operand_id], dimension);

    branch_prediction += mononn_engine::helpers::string_format(
        "(%s) < (%d)", per_dim_index[dimension].c_str(),
        left_shape_plus_one_shape.get_shape(dimension));
  }

  this->pred = mononn_engine::helpers::string_format(
      "(%s) && (%s)", this->pred.c_str(), branch_prediction.c_str());

  ///////////////// Step 1: Get index for each branch /////////////////
  std::vector<std::string> multi_index = per_dim_index;
  multi_index[dimension] = mononn_engine::helpers::string_format(
      "((%s) - (%d))", per_dim_index[dimension].c_str(),
      left_shape.get_shape(dimension));
  index_for_current_concat_branch =
      IndexTransform::multi_index_to_offset(operand_shapes.back(), multi_index);
