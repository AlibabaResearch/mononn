...
namespace core {
namespace context {
    std::vector<std::string> IndexTransform::offset_to_multi_index(TensorShape tensor_shape, std::string offset) {
...
//            std::string div = mononn_engine::helpers::join(" * ", tensor_shape.slice_dim(idx + 1, -1).get_shape());
            ...
            if (idx == 0) {
                multi_index[idx] = mononn_engine::helpers::string_format("((%s) / (%d))", offset.c_str(), div);
            } else if (idx == (int)multi_index.size() - 1) {
                multi_index[idx] = mononn_engine::helpers::string_format("((%s) %% (%d))", offset.c_str(), mod);
            } else {
                multi_index[idx] = mononn_engine::helpers::string_format("(((%s) %% (%d)) / (%d))", offset.c_str(), mod, div);
            }
        }
    ...
    std::string IndexTransform::multi_index_to_offset(TensorShape tensor_shape, std::vector<std::string> multi_index) {
