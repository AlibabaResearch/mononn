...
namespace context {
    ...
    public:
        ...
        void trace_concatenate(const std::shared_ptr<const Concatenate> &op, int operand_id);
        void trace_dynamic_slice(const std::shared_ptr<const DynamicSlice> &op);
        void trace_dynamic_update_slice_operand(const std::shared_ptr<const DynamicUpdateSlice> &op);
        void trace_dynamic_update_slice_update(const std::shared_ptr<const DynamicUpdateSlice> &op);
        void trace_gather_operand(const std::shared_ptr<const Gather> &op);
        void trace_gather_operand_ilp(const std::shared_ptr<const Gather> &op, int ilp_id);
        void trace_gather_indices(const std::shared_ptr<const Gather> &op);
        void trace_pad(const std::shared_ptr<const Pad> &op);
        void trace_reduce(const std::shared_ptr<const Reduce> &op, std::string const &inverse_reduce_dim);
