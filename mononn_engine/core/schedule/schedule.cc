...
namespace schedule {
    ...
    Loop Schedule::get_inner_loop() const {
        return this->loop_schedule.back();
    }
    ...
    void Schedule::set_locality_tier(Tier _tier) {
        this->tier = _tier;
    }

    Tier Schedule::get_locality_tier() const {
        return this->tier;
    }
    ...
    Schedule Schedule::vectorize(int elements_per_access) const {
        ...
        for (int idx = 0; idx < (int)this->loop_schedule.size(); ++idx) {
            if (idx == (int)this->loop_schedule.size() - 1) {
                new_schedule.add_loop_schedule(this->loop_schedule[idx].vectorize(elements_per_access));
            } else {
