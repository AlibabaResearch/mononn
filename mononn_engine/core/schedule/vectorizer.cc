...
namespace schedule {
    ...
    void Vectorizer::vectorize(std::shared_ptr<ClusterOp> cluster_op) {
        ...
        [&](std::shared_ptr<Op> op) -> void {
            ...
            if (op->get_type() == OpType::reduce || op->get_type() == OpType::broadcast) trace.push(node_name);

            if (!trace.empty() && cluster_op->get_graph()->get_node(trace.top())->get_type() == OpType::reduce) {
            ...
            if (op->get_type() == OpType::reduce || op->get_type() == OpType::broadcast) {
        ...
        [&](std::shared_ptr<Op> op) -> void {
            ...
            if (op->get_type() == OpType::broadcast) {
                std::shared_ptr<Broadcast> broadcast_op = std::static_pointer_cast<Broadcast>(op);
                ...
                if (std::find(broadcast_dims.begin(), broadcast_dims.end(), output_shape.rank() - 1) == broadcast_dims.end()) {
                    if (node_can_vectorize.find(op->get_operand(0)->get_name()) != node_can_vectorize.end()) {
            ...
            if (op->get_type()== OpType::gather) {
                if (node_can_vectorize.find(op->get_operand(1)->get_name()) != node_can_vectorize.end()) {
                    node_can_vectorize.erase(op->get_operand(1)->get_name());
                }
            }

            if (op->get_type() != OpType::reduce && node_can_vectorize.find(node_name) == node_can_vectorize.end()) {
                for (auto const &operand : op->get_operands()) {
                    if (node_can_vectorize.find(operand->get_name()) != node_can_vectorize.end()) {
        ...
        cluster_op->get_graph()->wave_front_order([&](std::shared_ptr<Op> op) -> void {
            std::string node_name = op->get_name();

            if (node_can_vectorize.find(node_name) == node_can_vectorize.end()) return;

            std::vector<TensorSpec> output_specs = op->get_output_specs();
            EXPECT_TRUE(output_specs.size() == 1, "Node " + node_name + " have more than one outputs");
            Dtype type = output_specs[0].get_dtype();
            EXPECT_TRUE(128 % type.size_in_bits() == 0,
                        "Node " + node_name + "have output type size in bits " + std::to_string(type.size_in_bits()));
            ...
            if (op->get_type() == OpType::slice) {
                ...
                for (int candidate_vec_len = 16; ; candidate_vec_len >>= 1) {
                    ...
                    if (start_offset_in_bits % (candidate_vec_len * type.size_in_bits()) == 0) {
                ...
                for (int candidate_vec_len = 16; ; candidate_vec_len >>= 1) {
                    ...
                    if (end_offset_in_bits % (candidate_vec_len * type.size_in_bits()) == 0) {
                        vector_len = std::min(vector_len, candidate_vec_len);
                        break;
                    }
                }
