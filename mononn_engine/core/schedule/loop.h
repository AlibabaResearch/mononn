...
namespace schedule {
    ...
    public:
        ...
        public:
            Condition() {}
            Condition(std::string _left_statement, std::string _right_statement, MathOp _cmp)
            : left_statement(_left_statement), right_statement(_right_statement), cmp(_cmp) {}

            std::string get_left_statement() const {
                return this->left_statement;
            }

            std::string get_right_statement() const {
                return this->right_statement;
            }

            MathOp get_cmp() const {
                return this->cmp;
            }

            std::string to_string() const {
                return "(" + this->get_left_statement() + " " + this->get_cmp().to_string() + " " + this->get_right_statement() + ")";
            }

            static Condition less_than(std::string left_statement, std::string right_statement);
        ...
        Loop(TensorShape _loop_shape, Scalar _key, std::string _init, std::string _stride) :
            loop_shape(_loop_shape), key(_key), init(_init), stride(_stride) {
            this->condition = Condition(
                    this->key.get_name(),
                    std::to_string(this->loop_shape.element_count()),
                    MathOp::less_than
            );
        }
