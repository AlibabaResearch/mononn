...
ABSL_FLAG(bool, automatic_mixed_precision, false, "Weather tensor core is enabled");
ABSL_FLAG(bool, faster_tuning, false, "Prune tuning space for faster tuning, may get sub-optimal solution");
ABSL_FLAG(bool, fastest_tuning, false, "Minimize tuning space, result in slow solution, this is for fast prototyping");
ABSL_FLAG(bool, use_cached_tuning_result, true, "Use tuning cache to expedite tuning");
...
ABSL_FLAG(std::vector<std::string>, optimization_disabled_pass, {}, "Pass disabled during optimization");
...
std::shared_ptr<Graph> get_network_graph(std::string hlo_module_proto_file) {
    std::shared_ptr<Graph> graph = mononn_engine::parser::IRParserFused::from_hlo_module_proto_file(hlo_module_proto_file);
    ...
    // ADD_PASS_TO_PASS_MANAGER(pm_vectorization, OneTimePassRunner, VectorizationPass);
    // pm_vectorization.execute(graph);
...
int Main(mononn_engine::tuning::Options const &options) {
    ...
    Config::get()->hlo_module_proto_temp_path = mononn_engine::helpers::Directory::get_mononn_new_temp_dir();
    Config::get()->hlo_module_proto_temp_file = mononn_engine::helpers::Path::join(Config::get()->hlo_module_proto_temp_path, "hlo_module.pb");
    ...
    mononn_engine::helpers::Directory::create_recursive(Config::get()->hlo_module_proto_temp_path);
    HloModuleDumper hlo_module_dumper(options.input_file, options.automatic_mixed_precision, 
            options.feeds, options.input_data_files, options.fetches);
    hlo_module_dumper.dump(Config::get()->hlo_module_proto_temp_path, options.dump_text_hlo_dir);

    std::shared_ptr<Graph> graph = get_network_graph(Config::get()->hlo_module_proto_temp_file);

    std::vector<std::unique_ptr<tensorflow::mononn_extra::proto::GraphSpecification>> graph_spec_list =
            std::move(TuningSpaceGenerator::generate_tuning_space(graph.get(), Config::get()->hlo_module_proto_temp_file, options.feeds, options.input_data_files, options.fetches));

    LOG(INFO) << "Generate " << graph_spec_list.size() << " candidate graph specification";

    std::vector<tensorflow::mononn_extra::proto::GraphSpecification const *> graph_spec_list_ptr;

    GraphCachedTuner graph_tuner(options.num_threads, graph);
    std::unique_ptr<tensorflow::mononn_extra::proto::GraphSpecification> optimal_spec = graph_tuner.get_optimal_spec(
        ...
        >(graph_spec_list,
          [&](std::unique_ptr<tensorflow::mononn_extra::proto::GraphSpecification> &spec) -> tensorflow::mononn_extra::proto::GraphSpecification const * {
              return spec.get();
          })
    );

    LOG(INFO) << "Generate final program";
    std::unique_ptr<CUDAProgram> cuda_program = GraphSpecificationCodegen::generate(optimal_spec.get());
    ...
    mononn_engine::helpers::save_proto_to_json_file(optimal_spec.get(), mononn_engine::helpers::Path::join(options.output_dir, "graph_spec.json"));
    mononn_engine::helpers::save_proto_to_binary_file(optimal_spec.get(), mononn_engine::helpers::Path::join(options.output_dir, "graph_spec.pb"));

    mononn_engine::helpers::Directory::remove(Config::get()->hlo_module_proto_temp_path);
    return 0;
}
...
int main(int argc, char **argv) {
    ...
    options.automatic_mixed_precision = absl::GetFlag(FLAGS_automatic_mixed_precision);
    options.faster_tuning = absl::GetFlag(FLAGS_faster_tuning);
    options.fastest_tuning = absl::GetFlag(FLAGS_fastest_tuning);
    options.use_cached_tuning_result = absl::GetFlag(FLAGS_use_cached_tuning_result);
