  ...
  for (auto const& block_per_sm : block_per_sm_space) {
    for (auto const& thread_per_block : thread_per_block_space) {
      ...
      std::shared_ptr<CUDAContext> cuda_context =
          std::make_shared<CUDAContext>(CUDAContext::get_cuda_context(
              grid_dim, block_dim, "cuda_stream", cuda_device_context));

      if (cuda_context->cuda_runtime_context.smem_size < 0) {
        continue;
      }

      std::vector<std::unique_ptr<GraphSpecification>>
          tuning_space_for_context =
              std::move(TuningSpaceGenerator::generate_tuning_space(
                  graph, cuda_context, hlo_proto_file, feeds, input_data_files,
                  fetches));
...
TuningSpaceGenerator::generate_tuning_space(
    ...
    std::vector<std::string> fetches) {
  std::vector<std::unique_ptr<GraphSpecification>> tuning_space;
  std::unique_ptr<GraphSpecification> default_graph_spec =
  ...
  if (std::find(Config::get()->optimization_disabled_pass.begin(),
                ...
                PassName::IntraOpReschedulePass) !=
      Config::get()->optimization_disabled_pass.end()) {
    ilp_factor_space = {1};
    LOG(INFO) << "TuningSpaceGenerator: " << PassName::IntraOpReschedulePass
              << " disabled.";
  }
  ...
  for (auto const& cluster_node_name :
    ...
    if (cluster_node->is_cluster_elewise()) {
      cluster_node->as<ClusterOp>()->set_schedule(
          cluster_node->as<ClusterOp>()->construct_schedule(LocalityTier::kT0));
      for (auto const& ilp_factor : ilp_factor_space) {
        if (!IntraOpReschedulePass::can_rescheduled_with_ilp_factor(
                cuda_context, cluster_node->as<ClusterOp>(), ilp_factor))
          continue;
        ...
        std::unique_ptr<GraphSpecification> graph_spec =
            mononn_engine::helpers::deep_copy_graph_specification(
                default_graph_spec.get());

        (*graph_spec->mutable_cluster_elewise_spec())[cluster_node_name] =
            cluster_elewise_spec;

        TuningSpaceGenerator::set_allow_and_reject_list(graph_spec.get(), graph,
                                                        cluster_node_name);
      ...
      for (auto const& ilp_factor : ilp_factor_space) {
        auto tier1_schedule = cluster_node->as<ClusterOp>()->construct_schedule(
            LocalityTier::kT1);
        auto tier2_schedule = cluster_node->as<ClusterOp>()->construct_schedule(
            LocalityTier::kT2);
        ...
        if (IntraOpReschedulePass::can_rescheduled_with_ilp_factor(
          ...
          for (auto const& reduce_impl_id : reduce_warp_impl_space) {
            ...
            std::unique_ptr<GraphSpecification> graph_spec =
                mononn_engine::helpers::deep_copy_graph_specification(
                    default_graph_spec.get());
            (*graph_spec->mutable_cluster_reduce_spec())[cluster_node_name] =
                cluster_reduce_spec;

            TuningSpaceGenerator::set_allow_and_reject_list(
                graph_spec.get(), graph, cluster_node_name);
            tuning_space.push_back(std::move(graph_spec));
          }
        }
  ...
  for (auto const& node_name :
       graph->get_node_list_by_type(OpType::custom_call)) {
    std::shared_ptr<Op> node = graph->get_node(node_name);

    if (node->is_gemm()) {
      auto candidate_impl =
          node->generate_candidate_implementation(cuda_context, 3);
      ...
      for (auto const& gemm_impl : candidate_impl) {
        ...
        gemm_spec.set_allocated_cutlass_config(gemm_impl->as<GemmImpl>()
                                                   ->get_cutlass_config()
                                                   .ConvertToProto()
                                                   .release());
        gemm_spec.set_allocated_gemm_backend_config(
            gemm_impl->as<GemmImpl>()
                ->get_gemm_backend_config()
                .ConvertToProto()
                .release());
      ...
      for (auto const& conv_impl : candidate_impl) {
        ...
        conv_spec.set_allocated_cutlass_config(conv_impl->as<ConvImpl>()
                                                   ->get_cutlass_config()
                                                   .ConvertToProto()
                                                   .release());
        conv_spec.set_allocated_conv_backend_config(
            conv_impl->as<ConvImpl>()
                ->get_conv_backend_config()
                .ConvertToProto()
                .release());
...
TuningSpaceGenerator::get_default_graph_specification(
    ...
    std::vector<std::string> input_data_files,
    std::vector<std::string> fetches) {
  std::unique_ptr<GraphSpecification> default_graph_specification =
      std::make_unique<GraphSpecification>();

  default_graph_specification->set_allocated_cuda_context(
      cuda_context->ConvertToProto().release());
  ...
  for (auto const& cluster_node_name :
    ...
    if (cluster_node->is_cluster_elewise()) {
      ...
      (*default_graph_specification
            ->mutable_cluster_elewise_spec())[cluster_node_name] =
          cluster_elewise_spec;
    } else if (cluster_node->is_cluster_reduce()) {
      proto::ClusterReduceSpecification cluster_reduce_spec;
      cluster_reduce_spec.set_name(cluster_node_name);
      ...
      (*default_graph_specification
            ->mutable_cluster_reduce_spec())[cluster_node_name] =
          cluster_reduce_spec;
    } else {
      LOG(FATAL) << "";
    }
  ...
  for (auto const& node_name :
    ...
    if (node->is_gemm()) {
      ...
      gemm_spec.set_allocated_cutlass_config(
          gemm_impl->get_cutlass_config().ConvertToProto().release());
      gemm_spec.set_allocated_gemm_backend_config(
          gemm_impl->get_gemm_backend_config().ConvertToProto().release());

      (*default_graph_specification->mutable_gemm_spec_list())[node_name] =
          gemm_spec;
    } else if (node->is_conv()) {
      ...
      conv_spec.set_allocated_cutlass_config(
          conv_impl->get_cutlass_config().ConvertToProto().release());
      conv_spec.set_allocated_conv_backend_config(
          conv_impl->get_conv_backend_config().ConvertToProto().release());

      (*default_graph_specification->mutable_conv_spec_list())[node_name] =
          conv_spec;
    } else {
...
void TuningSpaceGenerator::set_allow_and_reject_list(
    GraphSpecification* graph_spec, Graph* graph, std::string allowed_node) {
