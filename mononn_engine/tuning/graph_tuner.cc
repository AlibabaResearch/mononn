...
namespace tuning {
    ...
    std::unique_ptr<GraphSpecification> GraphTuner::get_optimal_spec(std::vector<GraphSpecification const *> candidate_spec_list) {
        ...
        for (auto spec : candidate_spec_list) {
            profiling_result_futures.push_back(std::move(this->profiling_queue.post(spec, {"generate_memory_initialization", "generate_parameter_initialization"})));
        }
        ...
        for (auto &future : profiling_result_futures) {
            ...
            if (current_task_id % 10 == 0) {
                LOG(INFO) << "Finish " << current_task_id << " out of " << candidate_spec_list.size() << " tasks";
            }
        ...
        std::vector<std::unique_ptr<GraphSpecification>> best_spec_for_each_context = std::move(GraphTuner::get_optimal_spec_for_each_cuda_context(profiling_result, candidate_spec_list));
        ...
        if (Config::get()->save_candidate_specification) {
            ...
            for (auto &spec : best_spec_for_each_context) {
                proto::Dim3 grid_dim = spec->cuda_context().cuda_runtime_context().grid_dim();
                proto::Dim3 block_dim = spec->cuda_context().cuda_runtime_context().block_dim();

                std::string context_str = mononn_engine::helpers::string_format("%d__%d", grid_dim.x(), block_dim.x());
                ...
                mononn_engine::helpers::save_proto_to_json_file(spec.get(), json_file_name);
                mononn_engine::helpers::save_proto_to_binary_file(spec.get(), proto_file_name);
            }
        }

        for (auto &spec : best_spec_for_each_context) {
            e2e_profiling_result_future.push_back(std::move(this->profiling_queue.post(spec.get(), {"generate_memory_initialization", "generate_parameter_initialization"})));
        }
        ...
        for (int idx = 0; idx < best_spec_for_each_context.size(); ++idx) {
            const proto::CUDAContext &context = best_spec_for_each_context[idx]->cuda_context();
            ...
            std::string context_str = mononn_engine::helpers::string_format("%d_%d_%d__%d_%d_%d",
                                                                       grid_dim.x(), grid_dim.y(), grid_dim.z(), block_dim.x(), block_dim.y(), block_dim.z());

            LOG(INFO) << "Context: " << context_str << " best solution time: " <<  e2e_profiling_result[idx].time_in_us;
            ss << "Context: " << context_str << " best solution time: " <<  e2e_profiling_result[idx].time_in_us << "\n";
        }
        ...
        for (int idx = 1; idx < e2e_profiling_result.size(); ++idx) {
            if (e2e_profiling_result[idx].time_in_us < e2e_profiling_result[best_id].time_in_us) {
                best_id = idx;
            }
        }
    ...
    GraphTuner::get_optimal_spec_for_each_cuda_context(
        ...
        std::unordered_map<std::string, std::vector<const GraphSpecification *>> spec_by_context;
        std::unordered_map<std::string, std::vector<ProfilingResult>> profiling_result_by_context;
        ...
        for (auto &[context_str, spec_list] : spec_by_context) {
            std::unique_ptr<GraphSpecification> best_spec_for_context =
                    GraphTuner::get_optimal_spec_for_cuda_context(profiling_result_by_context[context_str], spec_list);
    ...
    GraphTuner::get_optimal_spec_for_cuda_context(
        ...
        std::unique_ptr<GraphSpecification> best_spec_for_cuda_context = mononn_engine::helpers::deep_copy_graph_specification(candidate_spec_list[0]);

        for (auto node_name : best_spec_for_cuda_context->codegen_reject_list()) {
            (*best_spec_for_cuda_context->mutable_codegen_allow_list()).Add(std::move(node_name));
        }

        (*best_spec_for_cuda_context->mutable_codegen_reject_list()).Clear();

        std::unordered_map<std::string, std::vector<const GraphSpecification *>> spec_by_node;
        std::unordered_map<std::string, std::vector<ProfilingResult>> profiling_result_by_node;
        ...
        for (auto &[node_name, spec_list_for_node] : spec_by_node) {
            std::unique_ptr<GraphSpecification> best_node_spec = GraphTuner::get_optimal_spec_for_node(profiling_result_by_node[node_name], spec_by_node[node_name]);

            if (best_node_spec->gemm_spec_list().contains(node_name)) {
                (*best_spec_for_cuda_context->mutable_gemm_spec_list())[node_name] = best_node_spec->gemm_spec_list().at(node_name);
            } else if (best_node_spec->cluster_elewise_spec().contains(node_name)) {
                (*best_spec_for_cuda_context->mutable_cluster_elewise_spec())[node_name] = best_node_spec->cluster_elewise_spec().at(node_name);
            } else if (best_node_spec->cluster_reduce_spec().contains(node_name)) {
                (*best_spec_for_cuda_context->mutable_cluster_reduce_spec())[node_name] = best_node_spec->cluster_reduce_spec().at(node_name);
            } else if (best_node_spec->conv_spec_list().contains(node_name)) {
                (*best_spec_for_cuda_context->mutable_conv_spec_list())[node_name] = best_node_spec->conv_spec_list().at(node_name);
            } else {
    ...
    GraphTuner::get_optimal_spec_for_node(
        ...
        for (int idx = 1; idx < profiling_result.size(); ++idx) {
            if (profiling_result[idx].time_in_us < profiling_result[best_id].time_in_us) {
                best_id = idx;
            }
        }

        return std::move(mononn_engine::helpers::deep_copy_graph_specification(candidate_spec_list[best_id]));
    }

    std::string GraphTuner::context_to_str(const tensorflow::mononn_extra::proto::CUDAContext *cuda_context) {
