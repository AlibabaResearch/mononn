    ...
    public:
        ...
        public:
            ...
            double get_time_in_ns(int idx) const {
                return this->metric_range_value.at("gpu__time_duration.sum").at(std::to_string(idx));
            }

            void add_data(const std::string &metricName, const std::string &rangeName, const double &value) {
                if (!this->metric_range_value.count(metricName)) {
                    this->metric_range_value[metricName] = std::unordered_map<std::string, double>();
                }
        ...
        private:
            std::unordered_map<std::string, std::unordered_map<std::string, double>> metric_range_value;
        };

        CuptiProfilingSession(const std::vector<std::string> &_metricNames, int _numRanges = 1);
