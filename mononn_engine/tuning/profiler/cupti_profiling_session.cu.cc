...
namespace tuning {
namespace profiler {
    inline void ParseMetricNameString(
        const std::string& metricName, 
        std::string* reqName, 
        bool* isolated, 
        bool* keepInstances);

    void GetRawMetricRequests(
        std::string chipName,
    ...
    void validate_device_supported_params(const CUpti_Profiler_DeviceSupported_Params &params, const int &deviceNum) {
        if (params.isSupported != CUPTI_PROFILER_CONFIGURATION_SUPPORTED)
        {
            ...
            {
                LOG(FATAL) << "\tdevice sli configuration is not supported" << ::std::endl;
            }

            if (params.vGpu == CUPTI_PROFILER_CONFIGURATION_UNSUPPORTED)
            {
                LOG(FATAL) << "\tdevice vgpu configuration is not supported" << ::std::endl;
            }
            else if (params.vGpu == CUPTI_PROFILER_CONFIGURATION_DISABLED)
            {
                LOG(FATAL) << "\tdevice vgpu configuration disabled profiling support" << ::std::endl;
            }

            if (params.confidentialCompute == CUPTI_PROFILER_CONFIGURATION_UNSUPPORTED)
            {
                LOG(FATAL) << "\tdevice confidential compute configuration is not supported" << ::std::endl;
            }

            if (params.cmp == CUPTI_PROFILER_CONFIGURATION_UNSUPPORTED)
            {
                LOG(FATAL) << "\tNVIDIA Crypto Mining Processors (CMP) are not supported" << ::std::endl;
            }
    ...
    CuptiProfilingSession::CuptiProfilingSession(const std::vector<std::string> &_metricNames, int _numRanges) 
    ...
    CuptiProfilingSession::ProfilingResult CuptiProfilingSession::profiling_context(std::function<void ()> kernel_func_wrapper) {
        
        if (this->already_profiled) {
            LOG(FATAL) << "Repeated profiling_context run is not allowed at this moment";
        }
        ...
        beginSessionParams.counterDataScratchBufferSize = this->counterDataScratchBuffer.size();
        beginSessionParams.pCounterDataScratchBuffer = &(this->counterDataScratchBuffer[0]);
    ...
    void CuptiProfilingSession::cupti_initialize() {
        ...
        this->counterAvailabilityImage.resize(getCounterAvailabilityParams.counterAvailabilityImageSize);
        getCounterAvailabilityParams.pCounterAvailabilityImage = this->counterAvailabilityImage.data();
        CUPTI_API_CALL(cuptiProfilerGetCounterAvailability(&getCounterAvailabilityParams));
        ...
        GetConfigImage(this->chipName, this->metricNames, this->configImage, this->counterAvailabilityImage.data());
        GetCounterDataPrefixImage(this->chipName, this->metricNames, this->counterDataImagePrefix);
        CreateCounterDataImage(this->counterDataImage, this->counterDataScratchBuffer, this->counterDataImagePrefix, this->numRanges);
    }

    const std::string CuptiProfilingSession::Metrics::gpu__time_duration_sum = "gpu__time_duration.sum";

    CuptiProfilingSession::ProfilingResult CuptiProfilingSession::decode_profiling_result() const {
        ...
        NVPW_API_CALL(NVPW_CUDA_MetricsEvaluator_CalculateScratchBufferSize(&calculateScratchBufferSizeParam));
        ...
        metricEvaluatorInitializeParams.pCounterDataImage = this->counterDataImage.data();
        metricEvaluatorInitializeParams.counterDataImageSize = this->counterDataImage.size();
        ...
        for (std::string metricName : this->metricNames) {
            ...
            NVPW_API_CALL(NVPW_MetricsEvaluator_ConvertMetricNameToMetricEvalRequest(&convertMetricToEvalRequest));

            if (getNumRangesParams.numRanges != this->numRanges) {
                LOG(FATAL) << "Got mismatch num range, this may indicate kernel launch/execution failure."
                    << " Expected " << this->numRanges << " got " << getNumRangesParams.numRanges;
            }
            ...
            for (size_t rangeIndex = 0; rangeIndex < getNumRangesParams.numRanges; ++rangeIndex) {
                ...
                NVPW_API_CALL(NVPW_Profiler_CounterData_GetRangeDescriptions(&getRangeDescParams));
                ...
                setDeviceAttribParams.counterDataImageSize = this->counterDataImage.size();
                ...
                evaluateToGpuValuesParams.pCounterDataImage = this->counterDataImage.data();
                evaluateToGpuValuesParams.counterDataImageSize = this->counterDataImage.size();
                ...
                NVPW_API_CALL(NVPW_MetricsEvaluator_EvaluateToGpuValues(&evaluateToGpuValuesParams));
    ...
    void GetRawMetricRequests(
        ...
        for (auto& metricName : metricNames)
        {
            ...
            NVPW_API_CALL(NVPW_MetricsEvaluator_GetMetricRawDependencies(&getMetricRawDependenciesParms));
    ...
    void GetConfigImage(
        ...
        NVPW_API_CALL(NVPW_CUDA_RawMetricsConfig_Create_V2(&rawMetricsConfigCreateParams));
        ...
        if(pCounterAvailabilityImage)
        {
            ...
            NVPW_API_CALL(NVPW_RawMetricsConfig_SetCounterAvailability(&setCounterAvailabilityParams));
        }
        ...
        NVPW_API_CALL(NVPW_RawMetricsConfig_GenerateConfigImage(&generateConfigImageParams));
        ...
        NVPW_API_CALL(NVPW_RawMetricsConfig_Destroy((NVPW_RawMetricsConfig_Destroy_Params *)&rawMetricsConfigDestroyParams));
    }
    ...
    void GetCounterDataPrefixImage(
        ...
        NVPW_API_CALL(NVPW_CUDA_CounterDataBuilder_Create(&counterDataBuilderCreateParams));
        ...
        NVPW_API_CALL(NVPW_CounterDataBuilder_GetCounterDataPrefix(&getCounterDataPrefixParams));
        ...
        NVPW_API_CALL(NVPW_CounterDataBuilder_Destroy((NVPW_CounterDataBuilder_Destroy_Params *)&counterDataBuilderDestroyParams));
    }
    ...
    __global__
    void power_kernel(float *in, float *out, int count) {
        for (int idx = threadIdx.x + blockIdx.x * blockDim.x; idx < count; idx += blockDim.x * gridDim.x) {
