...
std::future<ProfilingResult> ParallelProfilingQueue::post(
  ...
  return std::move(this->thread_pool.enqueue(
      [this, graph_spec, host_codegen_disabled_pass,
       optimization_disabled_pass]() -> ProfilingResult {
        ...
        std::string tmp_directory =
            mononn_engine::helpers::Directory::get_mononn_root_temp_dir();

        tmp_directory = mononn_engine::helpers::Path::join(
            tmp_directory, mononn_engine::helpers::UUID::new_uuid());
        ...
        std::string graph_spec_pb_file =
            mononn_engine::helpers::Path::join(tmp_directory, "graph_spec.pb");
        std::string graph_spec_json_file = mononn_engine::helpers::Path::join(
            tmp_directory, "graph_spec.json");

        mononn_engine::helpers::save_proto_to_binary_file(graph_spec,
                                                          graph_spec_pb_file);
        mononn_engine::helpers::save_proto_to_json_file(graph_spec,
                                                        graph_spec_json_file);

        for (int codegen_cnt = 0; codegen_cnt < 5; ++codegen_cnt) {
          std::vector<std::string> params = {
              "--graph_spec_file=" + graph_spec_pb_file,
          ...
          if (!host_codegen_disabled_pass.empty()) {
            ...
            params.push_back(
                "--host_codegen_disabled_pass=" +
                mononn_engine::helpers::join(",", host_codegen_disabled_pass));
          }

          if (!optimization_disabled_pass.empty()) {
            params.push_back(
                "--optimization_disabled_pass=" +
                mononn_engine::helpers::join(",", optimization_disabled_pass));
          }

          if (!Config::get()->optimization_disabled_pass.empty()) {
            std::string disabled_passes = mononn_engine::helpers::join(
          ...
          SubProcess codegen_process(Config::get()->graph_spec_compiler_path,
                                     params);
          codegen_process.start();
          codegen_process.wait();

          if (codegen_process.get_return_code() != 0) {
            profiling_result.output += mononn_engine::helpers::string_format(
                "Failed on codegen attempt %d===\n", codegen_cnt);
            profiling_result.output += codegen_process.get_output();
            this->thread_mutex.lock();
            LOG(WARNING) << "Attempt " << codegen_cnt
                         << ". Codegen failed for sample " << tmp_directory;
            this->thread_mutex.unlock();
            std::this_thread::sleep_for(
                std::chrono::seconds(60 * codegen_cnt));  // wait and retry;
            continue;
          } else {
        ...
        for (int profiling_cnt = 0; profiling_cnt < 5; ++profiling_cnt) {
          ...
          profiling_cmd += "CUDA_VISIBLE_DEVICES=\"" +
                           std::to_string(Config::get()->gpus[gpu_id]) + "\"";
          //                profiling_cmd += " ncu --csv --app-replay-buffer
          //                memory --metrics gpu__time_duration.sum ./onefuser";
          ...
          if (profiling_process.get_return_code() != 0) {
            profiling_result.output += mononn_engine::helpers::string_format(
                "Failed on profiling attempt %d===\n", profiling_cnt);
            profiling_result.output += profiling_process.get_output();
            this->thread_mutex.lock();
            LOG(WARNING) << "Attempt " << profiling_cnt
                         << ". Profiling failed for sample " << tmp_directory
                         << " on gpu " << Config::get()->gpus[gpu_id];
            this->thread_mutex.unlock();
            std::this_thread::sleep_for(std::chrono::seconds(
                60 * profiling_cnt * 10));  // wait and retry;
            continue;
          }
          ...
          //                this->parse_ncu_result(profiling_process.get_output());
          NCUResult ncu_result =
              this->parse_console_result(profiling_process.get_output());
          profiling_result.time_in_us = ncu_result.time_in_us;

          this->thread_mutex.lock();
          fs::remove_all(tmp_directory);
          LOG(INFO) << "Removing directory: " << tmp_directory
                    << " Wait latency "
...
NCUResult ParallelProfilingQueue::parse_ncu_result(std::string str) const {
  std::vector<std::string> lines =
      mononn_engine::helpers::string_split(str, '\n');
  NCUResult ncu_result;

  bool begin_section = false;
  for (auto& line : lines) {
    if (line ==
        R"("ID","Process ID","Process Name","Host Name","Kernel Name","Kernel Time","Context","Stream","Section Name","Metric Name","Metric Unit","Metric Value")") {
      begin_section = true;

      std::vector<std::string> cols =
          mononn_engine::helpers::string_split(line, ',');
