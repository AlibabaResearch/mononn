...
std::unique_ptr<GraphSpecification> GraphCachedTuner::get_optimal_spec(
    std::vector<const GraphSpecification*> candidate_spec_list) {
  std::unordered_map<
      std::string,
      std::unordered_map<std::string,
                         std::vector<std::pair<const GraphSpecification*,
                                               std::future<ProfilingResult>>>>>
      future_result_list_by_context_by_node;

  std::unordered_map<std::string,
                     std::unordered_map<std::string, const GraphSpecification*>>
      optimal_specs_by_context_by_node;
  std::unordered_map<std::string, std::unordered_map<std::string, float>>
      performance_report;  // mapping: context -> node -> time
  ...
  for (auto& spec : candidate_spec_list) {
    ...
    std::string context_str =
        GraphCachedTuner::context_to_str(&spec->cuda_context());
    std::string node_name = spec->codegen_allow_list(0);

    if (this->graph->get_node(node_name)->is_gemm() ||
        this->graph->get_node(node_name)->is_conv()) {
      continue;
    } else {
    ...
    if (future_result_list_by_context_by_node.count(context_str) == 0) {
      future_result_list_by_context_by_node[context_str] = std::unordered_map<
          std::string, std::vector<std::pair<const GraphSpecification*,
                                             std::future<ProfilingResult>>>>();
    }

    if (future_result_list_by_context_by_node[context_str].count(node_name) ==
        0) {
      future_result_list_by_context_by_node[context_str][node_name] =
          std::vector<std::pair<const GraphSpecification*,
                                std::future<ProfilingResult>>>();
    }

    future_result_list_by_context_by_node[context_str][node_name].push_back(
        std::move(std::make_pair(
            spec, this->profiling_queue.post(
                      spec, {"generate_memory_initialization",
                             "generate_parameter_initialization"}))));
  }

  for (auto& [context_str, future_result_by_node] :
       future_result_list_by_context_by_node) {
    for (auto& [node_name, spec_list] : future_result_by_node) {
      if (optimal_specs_by_context_by_node.count(context_str) == 0) {
        optimal_specs_by_context_by_node[context_str] =
            std::unordered_map<std::string, const GraphSpecification*>();
      }
      ...
      for (auto& [spec, future_result] : spec_list) {
        ...
        if (finished_non_gemm_node_optimization_count % 10 == 0) {
          LOG(INFO) << "Non gemm node profiling progress: "
                    << finished_non_gemm_node_optimization_count << " out of "
                    << non_gemm_nodes_optimization_count;
        }
  ...
  std::unordered_map<
      ...
      spec_by_node_by_context;
  std::unordered_map<std::string,
                     std::unordered_map<std::string, proto::CutlassConfig>>
      cached_cutlass_config_by_node_by_context;
  ...
  for (auto& spec : candidate_spec_list) {
    ...
    if (spec_by_node_by_context.count(node_name) == 0) {
      spec_by_node_by_context[node_name] =
          std::unordered_map<std::string,
                             std::vector<const GraphSpecification*>>();
    }

    if (spec_by_node_by_context[node_name].count(context_str) == 0) {
      spec_by_node_by_context[node_name][context_str] =
          std::vector<const GraphSpecification*>();
    }
  ...
  for (auto& [node_name, spec_by_context] : spec_by_node_by_context) {
    std::string gemm_problem_hash = this->get_gemm_or_conv_problem_hash(
        this->graph->get_node(node_name).get());
    if (Config::get()->use_cached_tuning_result &&
      ...
      LOG(INFO) << "Node " << node_name
                << " hit profiling cache, problem hash: " << gemm_problem_hash;
      for (auto& [context_str, spec_list] : spec_by_context) {
        auto best_spec = std::find_if(
            spec_list.begin(), spec_list.end(),
            [&](const GraphSpecification* spec) -> bool {
              return google::protobuf::util::MessageDifferencer::Equivalent(
                  spec->gemm_spec_list().at(node_name).cutlass_config(),
                  cached_cutlass_config_by_node_by_context[gemm_problem_hash]
                                                          [context_str]);
            });

        if (best_spec == spec_list.end()) {
          LOG(FATAL) << "Cannot find cached specification. Node " << node_name
                     << " context: " << context_str;
        }
        optimal_specs_by_context_by_node[context_str][node_name] = *best_spec;
      }
    } else {
      LOG(INFO) << "Tune GEMM node " << node_name
                << " problem hash: " << gemm_problem_hash;
      std::unordered_map<std::string,
                         std::vector<std::pair<const GraphSpecification*,
                                               std::future<ProfilingResult>>>>
          gemm_futures_by_context;
      ...
      for (auto& [context_str, spec_list] : spec_by_context) {
        ...
        for (auto& spec : spec_list) {
          gemm_futures_by_context[context_str].push_back(
              std::move(std::make_pair(
                  spec, this->profiling_queue.post(
                            spec, {"generate_memory_initialization",
                                   "generate_parameter_initialization"}))));
      ...
      for (auto& [context_str, future_list] : gemm_futures_by_context) {
        ...
        cached_cutlass_config_by_node_by_context
            [gemm_problem_hash][context_str] =
                optimal_specs_by_context_by_node[context_str][node_name]
                    ->gemm_spec_list()
                    .at(node_name)
                    .cutlass_config();
      }
    ...
    if (finished_gemm_node_optimization_count % 10 == 0) {
      LOG(INFO) << "Gemm node profiling progess: "
                << finished_gemm_node_optimization_count << " out of "
                << gemm_nodes_optimization_count;
    }
  }
  ...
  std::unordered_map<std::string, std::future<ProfilingResult>>
      future_result_for_each_context;
  ...
  for (auto& [context_str, specs_by_node] : optimal_specs_by_context_by_node) {
    ...
    future_result_for_each_context[context_str] =
        std::move(this->profiling_queue.post(optimal_spec_for_context.get()));
    optimal_spec_for_each_context[context_str] =
        std::move(optimal_spec_for_context);
  }

  std::string hlo_module_proto_file = mononn_engine::helpers::Path::join(
      Config::get()->output_dir, "hlo_module.pb");
  ...
  if (Config::get()->save_candidate_specification) {
    ...
    for (auto& [context_str, spec] : optimal_spec_for_each_context) {
      auto spec_to_save =
          mononn_engine::helpers::deep_copy_graph_specification(spec.get());
      ...
      mononn_engine::helpers::save_proto_to_json_file(spec_to_save.get(),
                                                      json_file_name);
      mononn_engine::helpers::save_proto_to_binary_file(spec_to_save.get(),
                                                        proto_file_name);
    }
  }
  ...
  for (auto& [context_str, result] : result_for_each_context) {
    LOG(INFO) << "Context: " << context_str
              << " best solution time: " << result.time_in_us;
  ...
  LOG(INFO) << "Copy " << Config::get()->hlo_module_proto_temp_file << " to "
  ...
  mononn_engine::helpers::File::copy(Config::get()->hlo_module_proto_temp_file,
                                     hlo_module_proto_file);

  return std::move(final_result);
}

std::string GraphCachedTuner::context_to_str(
...
std::string GraphCachedTuner::get_gemm_or_conv_problem_hash(
    const Op* op) const {
...
void GraphCachedTuner::check_profiling_result(
    const ProfilingResult& result) const {
...
void GraphCachedTuner::dump_performance_report(
    const std::unordered_map<
        std::string, std::unordered_map<std::string, float>>& report) const {
  std::stringstream ss;
  ...
  std::ofstream ofs(mononn_engine::helpers::Path::join(
      Config::get()->output_dir, "performance_report.csv"));
  ofs << ss.str();
  ofs.close();
}
}  // namespace tuning
