...
using ParallelProfilingQueue = mononn_engine::tuning::profiler::ParallelProfilingQueue;
using Config = mononn_engine::config::Config;
using ProfilingResult = mononn_engine::tuning::profiler::ProfilingResult;

void dump_perf_result(std::unordered_map<std::string, ProfilingResult> &perf_result, std::string output_file) {
    std::stringstream ss;
...
int Main(const Options &options) {
    ...
    if (options.format == "pb") {
        mononn_engine::helpers::load_proto_from_binary_file(graph_spec.get(), options.graph_spec_file);
    ...
    std::unordered_map<std::string, std::future<ProfilingResult>> perf_result_futures;
    ...
    for (auto node_name : graph_spec->codegen_allow_list()) {
        ...
        auto spec = std::move(mononn_engine::helpers::deep_copy_graph_specification(graph_spec.get()));
    ...
    for (auto &spec : specs) {
        ...
        perf_result_futures[node_name] = std::move(profiling_queue.post(spec.get(), {"generate_memory_initialization", "generate_parameter_initialization"}, options.optimization_disabled_pass));
    }
...
int main(int argc, char **argv) {
    ...
    if (options.graph_spec_file.empty() || ! mononn_engine::helpers::File::exists(options.graph_spec_file)) {
