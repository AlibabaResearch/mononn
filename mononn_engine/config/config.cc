    ...
    Config* Config::get() {
        if (config == nullptr) {
            ...
            if (std::getenv("ONEFUSER_GRAPH_SPEC_COMPILER_PATH")) {
                config->graph_spec_compiler_path = std::getenv("ONEFUSER_GRAPH_SPEC_COMPILER_PATH");
            } else {
                config->graph_spec_compiler_path = config->onefuser_home + "/bazel-bin/tensorflow/onefuser/codegen/graph_specification_codegen_main";
            }
            
            // if (!fs::exists(config->graph_spec_compiler_path)) {
            //     std::cerr << "At " << __FILE__ << ":" << __LINE__ << " " << config->graph_spec_compiler_path << " not exists.";
            //     abort();
            // }
