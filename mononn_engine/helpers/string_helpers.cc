...
namespace helpers {
    ...
    std::string string_named_format(const std::string &format, const std::map<std::string, std::string>& names_and_values) {
        std::string result;
        std::string buf;
        ...
        for (const char &ch : format) {
            ...
            if (ch == '}') {
                ...
                // auto name_and_value = std::find_if(names_and_values.begin(), names_and_values.end(), [&](const std::pair<std::string, std::string> &t) -> bool {
                //     return t.first == buf;
                // });
                ...
                if (!names_and_values.count(buf)) {
                    ...
                    // for (auto const &[name, value] : names_and_values) debug_str += string_format("{%s, %s} ", name.c_str(), value.c_str());
                    // LOG(FATAL) << "Symbol " << buf << " cannot be found. Mappings: " << debug_str;
    ...
    template std::string to_string<std::string>(std::vector<std::string> const &vec);
    ...
    template<>
    std::string join<std::string>(std::string sep, std::vector<std::string> const &vec) {
        std::string result;
    ...
    template std::string join<int32_t>(std::string sep, std::vector<int> const &vec);
    template std::string join<float>(std::string sep, std::vector<float> const &vec);
    template std::string join<int64_t>(std::string sep, std::vector<int64_t> const &vec);
}
}