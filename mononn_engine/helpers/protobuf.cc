...
namespace helpers {
    std::unique_ptr<GraphSpecification> deep_copy_graph_specification(const GraphSpecification *graph_spec) {
        std::unique_ptr<GraphSpecification> copied_graph_spec = std::make_unique<GraphSpecification>();
        std::string __buf;
    ...
    void save_proto_to_json_file(const google::protobuf::Message *message, std::string file_name) {
        ...
        google::protobuf::util::MessageToJsonString(*message, &json_string, json_options);
        std::ofstream ofs;
        ofs.open(file_name);
        ofs << json_string;
        ofs.close();
    ...
    void load_proto_from_json_file(google::protobuf::Message *message, std::string file_name) {
        ...
        google::protobuf::util::JsonStringToMessage(json_stream.str(), message, json_parse_options);
