...
namespace helpers {
    ...
    std::string Directory::get_mononn_new_temp_dir() {
        return mononn_engine::helpers::Path::join(Directory::get_mononn_root_temp_dir(), mononn_engine::helpers::UUID::new_uuid());
    }
    ...
    TempDirectoryRAII::TempDirectoryRAII(const std::string &_dir_name) : dir_name(_dir_name) {
    ...
    TempDirectoryRAII::~TempDirectoryRAII() {
        Directory::remove(this->dir_name);
    }
}
}