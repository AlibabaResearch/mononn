...
void MonoNNModule::set_tuning_spec(
...
void MonoNNModule::generate_code() {
  if (!this->graph) {
    this->graph = mononn_engine::parser::IRParserFused::from_hlo_module_unique(
        this->hlo_module);
  }
  ...
  if (!(this->optimizations_have_done &
        OptimizationRunner::GRPOU_PRE_IMPL_ASSIGNMENT)) {
    OptimizationRunner::run_group_pre_impl_assignment(graph.get(),
                                                      cuda_context);
  }

  OptimizationRunner::run_group_impl_assignment(graph.get(), cuda_context,
                                                this->tuning_spec.get());
  ...
  if (EnvVar::defined("MONONN_CODEGEN_ALLOW_LIST_OVERRIDE")) {
    std::string allowed_node_name =
        EnvVar::get("MONONN_CODEGEN_ALLOW_LIST_OVERRIDE");
    ...
    codegen_reject_list.insert(this->tuning_spec->codegen_reject_list().begin(),
                               this->tuning_spec->codegen_reject_list().end());
  }
  ...
  for (auto const& allocation : *this->allocation_list) {
    if (allocation.is_entry_computation_parameter()) {
      if (allocation.assigned_buffers().size() != 1) {
        LOG(FATAL) << "Parameter buffer " << allocation.parameter_number()
                   << " should have exactly logical buffer, got "
                   << allocation.assigned_buffers().size() << "\n"
                   << allocation.ToString();
      }

      std::string node_name = allocation.assigned_buffers()
                                  .begin()
                                  ->first->defining_instruction()
                                  ->name();
      std::string buffer_name =
          mononn_engine::helpers::get_canonicalized_node_name(node_name);
      argument_list[allocation.parameter_number()] = buffer_name;
      allocation_ptr_to_buffer_name[reinterpret_cast<uint64_t>(&allocation)] =
          buffer_name;
    }

    if (allocation.maybe_live_out()) {
      if (allocation.is_tuple()) {
        // We do not need to pass tuple buffer (typically last node in the entry
        // computation). The tuple info is not in buffer_allocations in
        // GetMlirAllocationInfo neither.
        continue;
      }

      std::string buffer_name =
          "liveout_buffer_" + std::to_string(liveout_buffer_index++);
  ...
  this->cuda_program = std::move(GraphCodegen::generate(params));
}
...
void MonoNNModule::build_assembly(CompileOutputType type) {
  ...
  helpers::TempDirectoryRAII tmp_dir_guard(
  ...
  if (type == CompileOutputType::COMPILE_OUTPUT_TYPE_CUBIN) {
    std::string cubin_file_name = mononn_engine::helpers::Path::join(
        tmp_dir_guard.get_dir_name(), "mononn.cubin");
    ...
    std::string ptx_file_name = mononn_engine::helpers::Path::join(
        tmp_dir_guard.get_dir_name(), "mononn.ptx");
...
bool MonoNNModule::has_cubin() const { return !this->cubin.empty(); }

bool MonoNNModule::has_ptx() const { return !this->ptx.empty(); }

void MonoNNModule::set_cubin(const std::vector<uint8_t>& _cubin) {
  this->cubin = _cubin;
}

void MonoNNModule::set_ptx(const std::string& _ptx) { this->ptx = _ptx; }
