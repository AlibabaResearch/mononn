...
ABSL_FLAG(
    ...
    "/home/zhuangdonglin.zdl/workspace/models/experiments/bert_tiny/frozen.pb",
    "The input frozen pb file");
ABSL_FLAG(std::vector<std::string>, feeds,
          std::vector<std::string>({"token_type_ids", "input_ids",
                                    "attention_mask"}),
          "Input nodes");
ABSL_FLAG(std::vector<std::string>, fetches,
          std::vector<std::string>({"Identity"}), "Output nodes");
...
int Main(const Options& options) {
  ...
  session_options.config.mutable_graph_options()
      ...
      ->set_global_jit_level(tensorflow::OptimizerOptions_GlobalJitLevel_ON_2);
  session_options.config.mutable_graph_options()
      ->mutable_optimizer_options()
      ->set_cpu_global_jit(true);
  session_options.config.mutable_graph_options()
      ->mutable_rewrite_options()
      ->set_auto_mixed_precision(tensorflow::RewriterConfig_Toggle_ON);
  std::unique_ptr<tensorflow::Session> session(
...
int main(int argc, char** argv) {
  ...
  if (options.input_file.empty())
    LOG(FATAL) << "Please specify model input file";
  if (options.feeds.empty())
    LOG(FATAL) << "Please specify model graph input node";
  if (options.fetches.empty())
    LOG(FATAL) << "Please specify model graph output node";
