...
class IRParserFused {
 public:
  ...
  static std::shared_ptr<Graph> from_hlo_module_proto_file(
      std::string path_to_proto);
  static std::shared_ptr<Graph> from_hlo_module(
      const xla::HloModule* hlo_module);
  static std::unique_ptr<Graph> from_hlo_module_unique(
      const xla::HloModule* hlo_module);
 ...
 private:
  ...
  static void from_hlo_module_impl(Graph* graph,
                                   const xla::HloModule* hlo_module);
...
#undef DECLARE_OP_PARSER
  ...
  template <typename T>
  static std::shared_ptr<T> get_sketchy_node(Graph* graph,
                                             xla::HloInstruction* instruction);

  static Scalar hlo_constant_to_scalar(
