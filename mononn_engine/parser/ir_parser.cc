...
Scalar hlo_constant_to_scalar(xla::HloConstantInstruction const* constant) {
  ...
  Scalar scalar(constant->name(), dtype,
...
std::shared_ptr<Graph> IRParser::from_text(std::string text) {
  ...
  for (xla::HloInstruction* instruction : entry_computation->instructions()) {
    std::shared_ptr<Op> node =
        get_node_from_hlo_instruction(graph, instruction, &fused_computation);
...
std::shared_ptr<T> IRParser::get_sketchy_node(
  ...
  if (shape.IsTuple()) {
    ...
    dtype = Dtype::from_string(
        hlo_primitive_type_to_string(tuple_shapes[0].element_type()));
    tensor_shape = get_tensor_shape_from_xla_shape(tuple_shapes[0]);
    memory_layout = get_memory_layout_from_xla_shape(tuple_shapes[0]);
  } else {
    dtype =
        Dtype::from_string(hlo_primitive_type_to_string(shape.element_type()));
  ...
  if (shape.IsTuple()) {
    ...
    for (int idx = 1; idx < (int)tuple_shapes.size(); ++idx) {
      ...
      dtype = Dtype::from_string(
          hlo_primitive_type_to_string(elem_shape.element_type()));
  ...
  std::shared_ptr<T> node =
      std::make_shared<T>(node_name, operands, output_tensor_spec_list);
...
#define INSTANCIATE_TEMPLATE(op_name, op_code, op_class_code, ...) \
  ...
  IRParser::get_sketchy_node<op_class_code>(                       \
      std::shared_ptr<Graph> graph, xla::HloInstruction * instruction);
...
#define SWITCH_NODE(op_name, op_code, op_class_code, hlo_code)                 \
  case xla::HloOpcode::k##hlo_code: {                                          \
    std::shared_ptr<op_class_code> sketchy_node =                              \
        get_sketchy_node<op_class_code>(graph, instruction);                   \
...
std::shared_ptr<Op> IRParser::initialize_pad(
  ...
  Scalar padding_value =
      hlo_constant_to_scalar(dynamic_cast<xla::HloConstantInstruction const*>(
...
std::shared_ptr<Op> IRParser::initialize_reduce(
  ...
  xla::HloInstruction* constant =
      dynamic_cast<xla::HloReduceInstruction*>(instruction)->init_values()[0];

  Scalar init_value = hlo_constant_to_scalar(
      dynamic_cast<xla::HloConstantInstruction*>(constant));
