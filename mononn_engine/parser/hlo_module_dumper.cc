...
#define COPY_DATA_TO_TENSOR(_type, _dst, _src)                        \
  using RealT =                                                       \
      typename tensorflow::Input::Initializer::RealType<_type>::type; \
...
tensorflow::DataType get_node_data_type(tensorflow::GraphDef& graph_def,
...
void HloModuleDumper::dump(std::string binary_path, std::string text_path) {
  if (!text_path.empty()) {
    setenv("XLA_FLAGS",
           mononn_engine::helpers::string_format(
               "--xla_dump_hlo_as_text --xla_dump_to=%s", text_path.c_str())
               .c_str(),
           1);
  }
  ...
  for (int idx = 0; idx < feed_count; ++idx) {
    ...
    if (input_tensor.NumElements() != data.num_vals) {
      LOG(FATAL) << "Num element not match, " << input_tensor.NumElements()
                 << " vs. " << data.num_vals;
    }
