...
namespace parser {
    ...
    Scalar IRParserFused::hlo_constant_to_scalar(xla::HloConstantInstruction const *constant) {
        ...
        Scalar scalar(mononn_engine::helpers::get_canonicalized_node_name(constant->name()),
                      dtype,
                      constant->literal().ToStringWithoutShapeOneline());
    ...
    Scalar get_reduction_init_value(xla::HloInstruction *instruction) {
        if (instruction->opcode() != xla::HloOpcode::kReduce && instruction->opcode() != xla::HloOpcode::kReduceWindow) {
            LOG(FATAL) << "Unexpected inst type: " << xla::HloOpcodeString(instruction->opcode());
        } 
        
        std::string node_name = mononn_engine::helpers::get_canonicalized_node_name(instruction->name());

        std::vector<Dtype> init_type_list;
        std::vector<std::string> init_value_list;

        for (int64_t param_idx = instruction->operand_count() / 2; param_idx < instruction->operand_count(); ++param_idx) {
            auto param_inst = instruction->operand(param_idx);
            if (param_inst->opcode() != xla::HloOpcode::kConstant) {
                LOG(FATAL) << "Operand " << param_inst->name() << " of reduction inst " << instruction->name() << " is not constant";
            }

            Dtype type = Dtype::from_string(hlo_primitive_type_to_string(param_inst->shape().element_type()));
            std::string value = static_cast<const xla::HloConstantInstruction *>(param_inst)->literal().ToStringWithoutShapeOneline();
...
//             LOG(INFO) << "Sync can be optimized, however, additional optimization depends on implementation which is not available at parsing stage";
// //            if (ptr_src->as<ClusterReduce>()->get_reduce_node()->get_implementation()->as<ReduceImpl>()->get_tier() == LocalityTier::kT1 ||
// //                ptr_dst->as<ClusterReduce>()->get_reduce_node()->get_implementation()->as<ReduceImpl>()->get_tier() == LocalityTier::kT1) {
// //
// //                return Synchronization::Warp;
// //            }
// //
// //            if (ptr_src->as<ClusterReduce>()->get_reduce_node()->get_implementation()->as<ReduceImpl>()->get_tier() == LocalityTier::kT2 ||
// //                ptr_dst->as<ClusterReduce>()->get_reduce_node()->get_implementation()->as<ReduceImpl>()->get_tier() == LocalityTier::kT2) {
// //
// //                return Synchronization::ThreadBlock;
// //            }
//         }
...
//     }
    ...
    std::shared_ptr<Graph> IRParserFused::from_hlo_module_proto_file(std::string path_to_proto) {
        std::unique_ptr<xla::HloModuleProto> hlo_module_proto = std::make_unique<xla::HloModuleProto>();
        mononn_engine::helpers::load_proto_from_binary_file(hlo_module_proto.get(), path_to_proto);

        xla::DebugOptions debug_options = xla::GetDebugOptionsFromFlags();
        xla::HloModuleConfig module_config = xla::HloModule::CreateModuleConfigFromProto(*hlo_module_proto, debug_options).ValueOrDie();

        std::unique_ptr<xla::HloModule> hlo_module = xla::HloModule::CreateFromProto(*hlo_module_proto, module_config).ValueOrDie();

        if (!Config::get()->dump_text_hlo_dir.empty()) {
            std::string dump_file = mononn_engine::helpers::Path::join(Config::get()->dump_text_hlo_dir, "hlo_module.txt");
    ...
    std::shared_ptr<Op> IRParserFused::get_node_from_hlo_instruction(
        ...
        { \
            std::shared_ptr<op_class_code> sketchy_node = IRParserFused::get_sketchy_node<op_class_code>(graph, instruction); \
    ...
    void IRParserFused::from_hlo_module_impl(Graph *graph, const xla::HloModule *hlo_module) {
        std::unordered_map<std::string, xla::HloComputation *> fused_computation;
        ...
        for (xla::HloInstruction *instruction : entry_computation->MakeInstructionPostOrder()) {
            if (instruction->opcode() == xla::HloOpcode::kTuple) {
                // LOG(INFO) << "Skip tuple node " << instruction->name();
                continue;
            }

            std::shared_ptr<Op> node = IRParserFused::get_node_from_hlo_instruction(graph, instruction, &fused_computation);
            graph->add_node(node);

            for (xla::HloInstruction *input_instruction : instruction->operands()) {
                std::string input_node_name = mononn_engine::helpers::get_canonicalized_node_name(input_instruction->name());
        ...
        if (root_inst->opcode() == xla::HloOpcode::kTuple) {
            for (auto const &operand : root_inst->operands()) {
                std::string operand_name = mononn_engine::helpers::get_canonicalized_node_name(operand->name());
                graph->mark_as_output_node(operand_name);
            }
        } else {
            std::string root_node_name = mononn_engine::helpers::get_canonicalized_node_name(root_inst->name());
    ...
    std::shared_ptr<Graph> IRParserFused::from_hlo_module(const xla::HloModule *hlo_module) {
    ...
    std::unique_ptr<Graph> IRParserFused::from_hlo_module_unique(const xla::HloModule *hlo_module) {
    ...
    std::vector<std::shared_ptr<Op>> IRParserFused::get_operands(
        ...
        for (xla::HloInstruction *operand_inst : instruction->operands()) {
            std::string operand_node_name = mononn_engine::helpers::get_canonicalized_node_name(operand_inst->name());

            if (!graph->has_node(operand_node_name)) {
                LOG(FATAL) << "Get operand " << operand_node_name << " for node " <<
                    mononn_engine::helpers::get_canonicalized_node_name(instruction->name()) << ", operand not in graph." <<
                    " graph node list " << mononn_engine::helpers::to_string(graph->get_node_list());
            }
    ...
    std::vector<TensorSpec> IRParserFused::get_output_spec_list(
        ...
        if (instruction_shape.IsTuple()) {
            ...
            for (auto const &shape : tuple_shapes) {
                ...
                if (shape.IsTuple()) {
                    LOG(FATAL) << "Nested tuple shape is not supported. But inst " << instruction->name()
                            << " has shape " << instruction_shape.ToString();
                }
                
                Dtype dtype = Dtype::from_string(hlo_primitive_type_to_string(shape.element_type()));
            ...
            Dtype dtype = Dtype::from_string(hlo_primitive_type_to_string(instruction_shape.element_type()));
            TensorShape tensor_shape = get_tensor_shape_from_xla_shape(instruction_shape);
            MemoryLayout memory_layout = get_memory_layout_from_xla_shape(instruction_shape);
            output_tensor_spec_list.push_back(TensorSpec(dtype, tensor_shape, memory_layout));
        }
    ...
    struct OpFactory<ClusterOp> {
        static std::shared_ptr<ClusterOp> make_shared(
                ...
                std::vector<TensorSpec> output_spec_list,
                xla::HloInstruction *instruction) {
            xla::HloFusionInstruction *fusion_instruction = dynamic_cast<xla::HloFusionInstruction *>(instruction);

            bool root_reduce_node = false;

            if (fusion_instruction->fused_instructions_computation()->root_instruction()->opcode() == xla::HloOpcode::kReduce) {
                root_reduce_node = true;
            }

            bool found_reduce_node = false;

            for (auto const &inst :instruction->fused_instructions_computation()->MakeInstructionPostOrder()) {
                if (inst->opcode() == xla::HloOpcode::kReduce) {
                    found_reduce_node = true;
                }
            }

            if (fusion_instruction->fusion_kind() == xla::HloInstruction::FusionKind::kLoop && !root_reduce_node) {
                return std::make_shared<ClusterElewise>(node_name, operands, output_spec_list);
            } else if (fusion_instruction->fusion_kind() == xla::HloInstruction::FusionKind::kInput && !found_reduce_node) {
                LOG(WARNING) << "Fusion " << instruction->name() << " computation " << instruction->fused_instructions_computation()->name() << " is kInput fusion but no reduce node present. MonoNN will treat it as loop fusion.";
                return std::make_shared<ClusterElewise>(node_name, operands, output_spec_list);
            } else if (fusion_instruction->fusion_kind() == xla::HloInstruction::FusionKind::kInput || root_reduce_node) {
                return std::make_shared<ClusterReduce>(node_name, operands, output_spec_list);
            } else {
                LOG(FATAL) << "Unsupported fusion instruction " << instruction->name() << " type " << fusion_instruction->fusion_kind();
            }
        }
    };
    ...
    std::shared_ptr<T> IRParserFused::get_sketchy_node(
        ...
        std::vector<TensorSpec> output_tensor_spec_list = get_output_spec_list(instruction);

        std::shared_ptr<T> node = OpFactory<T>::make_shared(node_name, operands, output_tensor_spec_list, instruction);
    ...
    #define INSTANCIATE_TEMPLATE(op_name, op_code, op_class_code, ...) \
        template std::shared_ptr<op_class_code> IRParserFused::get_sketchy_node<op_class_code>( \
            Graph *graph, \
            xla::HloInstruction *instruction \
        ); \
    ...
    std::shared_ptr<Op> IRParserFused::initialize_constant(
        ...
        if (node->is_scalar()) {
            ...
            auto TF_MONONN_ENABLED = mononn_engine::helpers::EnvVar::is_true("TF_MONONN_ENABLED");
    ...
    //            std::vector<int> shape = node->get_output_spec(0).get_shape().get_shape();
    //            int rank = node->get_output_spec(0).rank();
    //            int element_count = node->get_output_spec(0).element_count();

                std::vector<float>  float_arr;
                std::vector<Eigen::half> half_arr;

                bool MONONN_STANDALONE_ENABLED = mononn_engine::helpers::EnvVar::is_true("MONONN_STANDALONE_ENABLED");
...
//                for (auto r_iter = shape.rbegin(); r_iter != shape.rend(); ++r_iter) {
//                    multi_index.push_back((int64_t)remaining_index % *r_iter);
//                    remaining_index /= *r_iter;
//                }
//
//                std::reverse(multi_index.begin(), multi_index.end());
//
//                if (dtype == Dtype::FLOAT32) {
//                    auto val = instruction->literal().Get<float>(multi_index);
//                    float_arr.push_back(val);
//                } else if (dtype == Dtype::FLOAT16) {
//                    auto val = instruction->literal().Get<Eigen::half>(multi_index);
//                    half_arr.push_back(val);
//                } else {
//                    LOG(FATAL) << "Unsupported dtype " << dtype.to_string();
//                }
//            }

//            std::string save_file = mononn_engine::helpers::Path::join(Config::get()->build_path, node_name + ".npy");
//            if (dtype == Dtype::FLOAT32) {
...
//            }
    ...
    std::shared_ptr<Op> IRParserFused::initialize_custom_call(
        ...
        if (node->is_conv()) {
            if (custom_call->feature_group_count() != 1) {
                LOG(FATAL) << "Group Convolution is not supported at this moment, get feature group: " << custom_call->feature_group_count() << " in node " << node->get_name();
            }
            ...
            for (int idx = 0; idx < dim_size; ++idx) {
                ...
                filter_stride.push_back((int)instruction->window().dimensions(idx).stride());
                padding_low.push_back((int)instruction->window().dimensions(idx).padding_low());
                padding_high.push_back((int)instruction->window().dimensions(idx).padding_high());

                if (instruction->window().dimensions(idx).window_dilation() != 1) {
                    LOG(FATAL) << "Dilated conv not supported for node: " << node->get_name() << " dilation: " 
            ...
            for (auto const &user_inst : instruction->users()) {
                if (user_inst->opcode() != xla::HloOpcode::kGetTupleElement) {
                    LOG(FATAL) << "Conv node have non-GTE user, conv node: " << instruction->name()
                        << " Invalid user name: " << user_inst->name();
                }

                if (user_inst->tuple_index() == 0) {
                    setted = true;
                    node->set_conv_output_GTE_node_name(mononn_engine::helpers::get_canonicalized_node_name(user_inst->name()));
                }
            }

            if (!setted) {
                LOG(FATAL) << "GTE output node for conv: " << node->get_name() << " not set.";
            }
        }
    ...
    std::shared_ptr<Op> IRParserFused::initialize_dynamic_slice(
        std::shared_ptr<DynamicSlice> node,
        xla::HloInstruction *instruction,
        std::unordered_map<std::string, xla::HloComputation *> *fused_computation
    ) {
        LOG(DEBUG) << "Initialize dynamic slice: " << instruction->name() << " operand count " << instruction->operand_count();

        const std::vector<int64_t> &dynamic_slice_sizes 
            = static_cast<xla::HloDynamicSliceInstruction *>(instruction)->dynamic_slice_sizes();
    ...
    std::shared_ptr<Op> IRParserFused::initialize_dynamic_update_slice(
        std::shared_ptr<DynamicUpdateSlice> node,
        xla::HloInstruction *instruction,
        std::unordered_map<std::string, xla::HloComputation *> *fused_computation
    ) {
        return std::static_pointer_cast<Op>(node);
    ...
    std::shared_ptr<Op> IRParserFused::initialize_cluster(
        ...
        for (xla::HloInstruction *instruction : fusion_instruction->fused_instructions_computation()->MakeInstructionPostOrder()) {
            if (instruction->opcode() == xla::HloOpcode::kTuple) {
                LOG(INFO) << "Skip tuple node " << instruction->name();
                continue;
            }

            std::shared_ptr<Op> node = IRParserFused::get_node_from_hlo_instruction(graph.get(), instruction, fused_computation);
            node->set_attribute(OpAttribute::initial_cluster_tag, cluster_node->get_name());
            node->set_attribute(OpAttribute::sub_cluster_tag, cluster_node->get_name());
            node->set_attribute(OpAttribute::sub_cluster_type, cluster_node->get_cluster_type().to_string());
            node->set_attribute(OpAttribute::initial_cluster_type, cluster_node->get_cluster_type().to_string());
        ...
        for (xla::HloInstruction *instruction : fusion_instruction->fused_instructions_computation()->instructions()) {
            if (instruction->opcode() == xla::HloOpcode::kParameter) {
                std::string node_name = mononn_engine::helpers::get_canonicalized_node_name(instruction->name());
                graph->mark_as_input_node(node_name);
            }
        ...
        xla::HloInstruction *root_inst = fusion_instruction->fused_instructions_computation()->root_instruction();
        if (root_inst->opcode() == xla::HloOpcode::kTuple) {
            for (auto const &operand : root_inst->operands()) {
                std::string operand_name = mononn_engine::helpers::get_canonicalized_node_name(operand->name());
                graph->mark_as_output_node(operand_name);
        ...
        if (has_dynamic_slice || has_dynamic_update_slice || has_gather) {
            ...
            for (auto const &node_name : graph->get_node_list()) {
                ...
                if (node->get_type() == OpType::dynamic_slice) {
                    for (int operand_id = 1; operand_id < node->get_operand_count(); ++operand_id) {
                        for (auto const &input_node_name : graph->get_input_nodes()) {
                            if (graph->topology_before(input_node_name, node->get_operand(0)->get_name())) {
                                graph->add_control_edge(node->get_operand(operand_id)->get_name(), input_node_name);
                            }
                        }
                    }
                }
                ...
                if (node->get_type() == OpType::dynamic_update_slice) {
                    if (!graph->is_output_node(node_name)) {
                        LOG(FATAL) << "DynamicUpdateSlice node " << node_name << " is not output node in cluster " << cluster_node->get_name();
                    }

                    for (int operand_id = 2; operand_id < node->get_operand_count(); ++operand_id) {
                        for (auto const &input_node_name : graph->get_input_nodes()) {
                            if (graph->topology_before(input_node_name, node->get_operand(0)->get_name()) ||
                                graph->topology_before(input_node_name, node->get_operand(1)->get_name())) {
                                graph->add_control_edge(node->get_operand(operand_id)->get_name(), input_node_name);
                            }
                        }
                    }
                ...
                if (node->get_type() == OpType::gather) {
                    for (auto const &input_node_name : graph->get_input_nodes()) {
                        if (graph->topology_before(input_node_name, node->get_operand(0)->get_name()) || input_node_name == node->get_operand(0)->get_name()) {
                            graph->add_control_edge(node->get_operand(1)->get_name(), input_node_name);
                        }
                    }
                }
            }
    ...
    std::shared_ptr<Op> IRParserFused::initialize_reduce(
        ...
        // EXPECT_TRUE(instruction->operand_count() == 2, mononn_engine::helpers::string_format("Reduce node %s have %d operands", node->get_name().c_str(), instruction->operand_count()).c_str());
        EXPECT_TRUE(instruction->dimensions().size() == 1, "Multiple dimension not supported yet");
        ...
        // node->set_init_value(init_value);
        // Reducer reducer;
        ...
        ReductionFunctorGenerator::Registry()->add_generator(node->get_name(), instruction->to_apply());

        node->set_reduction_functor_generator(ReductionFunctorGenerator::Registry()->get_generator(instruction->to_apply()->name()));
    ...
    std::shared_ptr<Op> IRParserFused::initialize_reduce_window(
        ...
        EXPECT_TRUE(instruction->operand_count() == 2, 
            mononn_engine::helpers::string_format("ReduceWindow node %s have %d operands. Expect 2.", node->get_name().c_str(), instruction->operand_count()).c_str());
        ...
        for (int idx = 0; idx < dim_size; ++idx) {
            if (filter_stride[idx] == 0) {
                if (filter_size[idx] != 0) {
                    LOG(FATAL) << "Invalid filter stride and size: idx " << idx << " stride " << filter_stride[idx] << " size " << filter_size[idx] << " node name " << node->get_name();
                }

                if (padding_low[idx] != 0) {
                    LOG(FATAL) << "Invalid filter stride and padding low: idx " << idx << " stride " << filter_stride[idx] << " size " << padding_low[idx] << node->get_name();
                }

                if (padding_high[idx] != 0) {
                    LOG(FATAL) << "Invalid filter stride and padding high: idx " << idx << " stride " << filter_stride[idx] << " size " << padding_high[idx] << node->get_name();
                }
            }
        }
    ...
    std::shared_ptr<Op> IRParserFused::initialize_slice(
        ...
        EXPECT_TRUE(std::all_of(slice_strides.begin(), slice_strides.end(), [&](int stride) -> bool {
            return stride == 1;
        }), "Slice stride should be one"); // vectorizer and index tracer should be modified accordingly for non-one stride
