...
template <typename T>
void npy_save(std::string fname, const T* data, const std::vector<int> shape,
              std::string mode = "w") {
  std::vector<size_t> size_t_shape;
  for (auto const& s : shape) size_t_shape.push_back((size_t)s);

  npy_save(fname, data, size_t_shape, mode);
}
...
void parse_npy_header(FILE* fp, size_t& word_size, std::vector<size_t>& shape,
  ...
  if (loc1 == std::string::npos) exit(-1);
  loc1 += 16;
  fortran_order = (header.substr(loc1, 4) == "True" ? true : false);
  ...
  loc1 = header.find("(");
  loc2 = header.find(")");
  if (loc1 == std::string::npos || loc2 == std::string::npos) exit(-1);
...
NpyArray load_the_npy_file(FILE* fp) {
  ...
  if (nread != arr.num_bytes()) exit(-1);
  return arr;
}

NpyArray load_the_npz_array(FILE* fp, uint32_t compr_bytes,
                            uint32_t uncompr_bytes) {
  std::vector<unsigned char> buffer_compr(compr_bytes);
  std::vector<unsigned char> buffer_uncompr(uncompr_bytes);
  size_t nread = fread(&buffer_compr[0], 1, compr_bytes, fp);
  if (nread != compr_bytes) exit(-1);
...
npz_t npz_load(std::string fname) {
  ...
  while (1) {
    ...
    if (extra_field_len > 0) {
      ...
      if (efield_res != extra_field_len) exit(-1);
    }
