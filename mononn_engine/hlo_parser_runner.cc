...
std::string emit_node(std::string node_name, std::string label, std::string properties) {
...
std::string emit_edges(std::string node_name, const xla::HloInstruction::InstructionVector &ops) {
    std::stringstream cmd;
    for (const xla::HloInstruction *op : ops) {
        cmd << "CREATE(" + to_neo4j_node_name(op->name()) + ")-[:INPUT]->(" + node_name + ")\n";
        // cmd << "CREATE(" + node_name + ")-[:REVERSE_INPUT]->(" + to_neo4j_node_name(op->name()) + ")\n";
    }

    return cmd.str();
}

std::string to_properties_string(const std::map<std::string, std::string>& properties) {
    std::stringstream prop;

    bool first = true;
    for (const std::pair<std::string, std::string> &kv : properties) {
        if (first) { prop << "{"; first = false; }
        else prop << ", ";
...
std::string emit_fused_computation(xla::HloInstruction *fused_instruction, std::string model_name) {
    std::stringstream cmd;

    std::map<std::string, std::string> param_mapping;
    for (size_t op_id = 0; op_id < fused_instruction->operands().size(); ++op_id) {
        param_mapping["param_" + std::to_string(op_id)] = fused_instruction->operands()[op_id]->name();
    }

    for (xla::HloInstruction *instruction : fused_computations[fused_instruction->fused_instructions_computation()->name()]->instructions()) {
        ...
        properties["fused_computation"] = fused_instruction->fused_instructions_computation()->name();


        cmd << "CREATE(" + node_name + ":" + node_label + " " + to_properties_string(properties) << ")\n";
        ...
        for (const xla::HloInstruction *op : instruction->operands()) {
            ...
            cmd << "CREATE(" + to_neo4j_node_name(input_op_name) + ")-[:INPUT]->(" + node_name + ")\n";
            // cmd << "CREATE(" + node_name + ")-[:REVERSE_INPUT]->(" + to_neo4j_node_name(input_op_name) + ")\n";
        }
    }

    xla::HloInstruction *root_instruction = fused_computations[fused_instruction->fused_instructions_computation()->name()]->root_instruction();
    ...
    cmd << "CREATE(" + node_name + ":ROOT" + " " + to_properties_string(properties) << ")\n";

    cmd << "CREATE(" + to_neo4j_node_name(root_instruction->name()) + ")-[:INPUT]->(" + node_name + ")\n";
    // cmd << "CREATE(" + node_name + ")-[:REVERSE_INPUT]->(" + to_neo4j_node_name(root_instruction->name()) + ")\n";

    return cmd.str();
}

std::string emit_computation(xla::HloInstruction *instruction, std::string model_name) {
    std::string name = instruction->name();
    std::string node_label = get_node_label(name);
    std::string node_name = to_neo4j_node_name(name);
    std::string shape = instruction->shape().ToString(true);
    ...
    if (name.find("custom-call", 0) == 0) {
        ...
        return emit_node(node_name, node_label, to_properties_string(properties)) + emit_edges(node_name, instruction->operands());
    }
...
int main(int argc, char const *argv[])
{
    ...
    tensorflow::StatusOr<std::unique_ptr<xla::HloModule>> ret = xla::ParseAndReturnUnverifiedModule(absl::string_view(hlo_text_stream.str()));
    ...
    for (xla::HloInstruction *instruction : entry_computation->instructions()) {
        if (instruction->IsFusedComputation()) {
            std::cerr << "Unexpected fused instruction: " <<  instruction->name() << std::endl;
